test_that("MV5-AH ranks exact ties canonically without labels", {
  rows <- expand.grid(
  ...
  ranked <- mv05ah_rank_group_v1(rows, group)
...
test_that("MV5-AH blocked inference freezes 24 intervals and four tests", {
  registry <- mv05ag_estimand_registry_v1()
  ...
  result <- mv05ah_block_inference_v1(
...
test_that("MV5-AH boundary exceedances conservatively retain numerical ties", {
  observed <- 0.25
  null <- c(observed, -observed, observed - 2 * .Machine$double.eps, 0)
  expect_equal(.mv05ah_boundary_exceedances(null, observed), 3L)
})
