test_that("MV5-AL ranks exact ties canonically without labels", {
  rows <- expand.grid(
  ...
  group <- data.frame(robustness_group_id = "g", fold_id = "large_loso_v1:S",
    ...
    coordinate_source_identity_exact = TRUE,
    nested192_prefix_identity_exact = TRUE,
    nested256_subset_384_identity_exact = TRUE,
    method_rows = nrow(rows), stringsAsFactors = FALSE)
  ranked <- mv05al_rank_group_v1(rows, group)
  ...
  expect_true(all(part$tie_break_policy ==
    "ascending_distance_then_canonical_training_sample_id_radix_v1"))
  expect_false(any(ranked$labels_opened))

  group$nested192_prefix_identity_exact <- FALSE
  expect_error(mv05al_rank_group_v1(rows, group), "malformed")
})

test_that("MV5-AL blocked inference freezes 24 intervals and four tests", {
  registry <- mv05ak_estimand_registry_v1()
  ...
  result <- mv05al_block_inference_v1(
...
test_that("MV5-AL boundary exceedances conservatively retain numerical ties", {
  observed <- 0.25
  null <- c(observed, -observed, observed - 2 * .Machine$double.eps, 0)
  expect_equal(.mv05al_boundary_exceedances(null, observed), 3L)
})
