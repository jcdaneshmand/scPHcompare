make_mv05k_ranking_fixture <- function() {
  labels <- data.frame(
    sample_id = c("a", "b", "c", "d"),
    study = c("s1", "s2", "s1", "s2"),
  ...
  methods <- c("integrated_cell_landscape_h0_v1", "integrated_cell_landscape_h1_v1")
  seeds <- 1:2
  rows <- list()
  index <- 0L
    ...
    for (seed in seeds) {
      for (method in methods) {
        for (query in queries) {
          ...
          if (method == "integrated_cell_landscape_h0_v1") {
...
test_that("MV5-K uses immutable ranks for MRR and fixed 1-NN", {
  fixture <- make_mv05k_ranking_fixture()
  observed <- mv05k_evaluate_retrieval_v1(
  ...
  h0 <- observed[observed$method_id == "integrated_cell_landscape_h0_v1", ]
  h1 <- observed[observed$method_id == "integrated_cell_landscape_h1_v1", ]
...
test_that("MV5-K rejects noncanonical rank order and premature label opening", {
  fixture <- make_mv05k_ranking_fixture()
  bad <- fixture$rankings
  bad$neighbor_rank[c(1, 2)] <- bad$neighbor_rank[c(2, 1)]
  expect_error(
    mv05k_evaluate_retrieval_v1(
  ...
  expect_error(
    mv05k_open_frozen_labels_v1("missing.csv", data.frame(
...
test_that("MV5-K records prespecified non-estimability without scoring an error", {
  fixture <- make_mv05k_ranking_fixture()
...
test_that("MV5-K summaries average technical seeds before tissue macro use", {
  fixture <- make_mv05k_ranking_fixture()
  observed <- mv05k_evaluate_retrieval_v1(
    fixture$rankings, fixture$labels,
    expected_rows = nrow(fixture$rankings),
  ...
  summary <- mv05k_summarize_retrieval_v1(observed)
...
make_mv05k_inference_fixture <- function() {
  tissue_counts <- c(
...
test_that("MV5-K inference is paired, blocked, deterministic, and Holm adjusted", {
  fixture <- make_mv05k_inference_fixture()
  set.seed(42)
  rng_before <- .Random.seed
  first <- mv05k_block_inference_v1(
    fixture, bootstrap_replicates = 50L, bootstrap_seed = 7L,
    randomization_replicates = 99L, randomization_seed = 8L
  )
  expect_identical(.Random.seed, rng_before)
  second <- mv05k_block_inference_v1(
    fixture, bootstrap_replicates = 50L, bootstrap_seed = 7L,
    randomization_replicates = 99L, randomization_seed = 8L
  )
...
test_that("MV5-K randomization boundary ties are conservative and stable", {
  observed <- 0.5
  null <- c(observed - .Machine$double.eps, observed + .Machine$double.eps,
            0.25)
  expect_equal(.mv05k_randomization_exceedance_count_v1(null, observed), 2L)
  serialized_observed <- as.numeric(format(observed, digits = 15))
  expect_equal(
    .mv05k_randomization_exceedance_count_v1(null, serialized_observed), 2L
  )
})
