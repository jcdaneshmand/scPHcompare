test_that("MV5-AE recovers nested 192 as the sole canonical next setting", {
  ...
  order <- mv05ae_configuration_order_v1(queue)
  ...
  expect_error(mv05ae_configuration_order_v1(broken), "ambiguous or drifted")
})

test_that("MV5-AE binds complete unsliced robustness panels", {
  ...
  evidence <- mv05ae_bind_complete_evidence_v1(
    "cosine_chord_vs_euclidean", production, primary,
    data.frame(x = 1:24), registry)
  expect_true(evidence$complete_evidence_bound)
  expect_false(evidence$cell_count_sensitivity_answered)
  production$macro_estimands <- 23
  expect_error(mv05ae_bind_complete_evidence_v1(
    "cosine_chord_vs_euclidean", production, primary,
    data.frame(x = 1:24), registry), "missing or drifted")
})

test_that("MV5-AE requires every criterion and cannot skip nested 192", {
  ids <- c("cells384_pc20_euclidean_v1",
           "cells384_pc30_cosine_chord_v1",
           "nested_cells_192_pc30_euclidean_v1",
           "nested_cells_256_pc30_euclidean_v1")
  order <- data.frame(configuration_order = 1:4, configuration_id = ids,
    position_state = c("complete", "complete", "next_eligible", "later_closed"))
  evidence <- data.frame(
    analysis_id = c("pc20_vs_pc30", "cosine_chord_vs_euclidean"),
    complete_evidence_bound = TRUE, cell_count_sensitivity_answered = FALSE)
  pass <- mv05ae_continuation_criteria_v1()["criterion_id"]; pass$passed <- TRUE
  decision <- mv05ae_decide_v1(order, evidence, pass)
  expect_identical(decision$authorized_configuration_id,
                   "nested_cells_192_pc30_euclidean_v1")
  expect_false(decision$nested_256_authorized)
  pass$passed[[7L]] <- FALSE
  expect_error(mv05ae_decide_v1(order, evidence, pass), "cannot be authorized")
})

test_that("MV5-AE nested queue remains label, rank, and outcome closed", {
  ...
  queue$execution_order <- 301:450; queue$cells <- 192L
  ...
  result <- mv05ae_nested_192_queue_v1(queue, decision)
