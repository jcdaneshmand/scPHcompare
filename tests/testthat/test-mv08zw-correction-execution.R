...
test_that("MV8-ZX independently closes all correction pairs", {
  audit <- test_path("..", "..", "docs", "audits",
                     "mv08zx-correction-closure-v1")
  expect_true(dir.exists(audit))
  read_audit <- function(name) read.csv(file.path(audit, name),
                                        stringsAsFactors = FALSE,
                                        check.names = FALSE)
  groups <- read_audit("mv08zx-group-summary.csv")
  rehash <- read_audit("mv08zx-private-rehash.csv")
  reconstruction <- read_audit("mv08zx-reconstruction.csv")
  resources <- read_audit("mv08zx-resource-summary.csv")
  validation <- read_audit("mv08zx-validation.csv")
  decision <- read_audit("mv08zx-decision.csv")
  manifest <- read_audit("mv08zx-artifact-manifest.csv")
  expect_equal(nrow(groups), 2L)
  expect_identical(groups$homology_dimension, c("H0", "H1"))
  expect_true(all(groups$view_kind == "gene_topology_v1"))
  expect_equal(sum(groups$pairs), 56L)
  expect_true(all(groups$independently_reconstructed))
  expect_true(all(groups$exact))
  expect_true(all(groups$all_active_levels))
  expect_true(all(groups$essential_h0_excluded))
  expect_false(any(groups$grid_used))
  expect_false(any(groups$level_cap_used))
  expect_true(all(groups$scientific_engine_version == 2L))
  expect_equal(nrow(rehash), 2L)
  expect_true(all(rehash$independently_reconstructed))
  expect_equal(sum(reconstruction$pairs_recomputed), 56L)
  expect_true(all(reconstruction$passed))
  expect_true(resources$elapsed_cap_passed)
  expect_true(resources$rss_cap_passed)
  expect_true(resources$storage_cap_passed)
  expect_equal(resources$workers, 1L)
  expect_equal(resources$retries, 0L)
  expect_true(all(validation$passed))
  expect_equal(nrow(validation), 20L)
  expect_equal(decision$groups_closed, 2L)
  expect_equal(decision$pairs_closed, 56L)
  expect_false(decision$comparisons_authorized)
  expect_false(decision$clustering_authorized)
  expect_false(decision$fusion_authorized)
  expect_false(decision$labels_authorized)
