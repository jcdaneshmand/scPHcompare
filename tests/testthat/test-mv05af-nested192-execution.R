mv05af_fixture_queue <- function() {
  ...
  queue$configuration_id <- "nested_cells_192_pc30_euclidean_v1"
  queue$configuration_order <- 3L
  queue$cells <- 192L; queue$coordinates <- 30L
...
test_that("MV5-AF accepts only the complete nested-192 axis", {
  queue <- mv05af_fixture_queue()
  expect_invisible(mv05af_validate_nested_queue_v1(queue))
...
test_that("MV5-AF rejects configuration, order, and firewall drift", {
  queue <- mv05af_fixture_queue()
  broken <- queue; broken$configuration_id[[1L]] <- "cells384_pc20_euclidean_v1"
  expect_error(mv05af_validate_nested_queue_v1(broken), "150 unopened nested-192")
...
test_that("MV5-AF rejects incomplete counts and resource-axis drift", {
  queue <- mv05af_fixture_queue()
  expect_error(mv05af_validate_nested_queue_v1(queue[-1L, ]),
               "150 unopened nested-192")
  ...
  expect_error(mv05af_validate_nested_queue_v1(broken),
               "150 unopened nested-192")
})

test_that("MV5-AF scope freezes the deterministic nested point policy", {
  scope <- mv05af_nested_scope_v1(mv05af_fixture_queue())
  expect_identical(scope$point_selection,
                   "sha256_sample_seed_cell_nested_v1")
  expect_false(scope$nested_256_execution_authorized)
