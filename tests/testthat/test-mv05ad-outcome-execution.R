test_that("MV5-AD ranks exact ties canonically without labels", {
  rows <- expand.grid(
  ...
  ranked <- mv05ad_rank_group_v1(rows, group)
...
test_that("MV5-AD blocked inference freezes 24 intervals and four tests", {
  ...
  result <- mv05ad_block_inference_v1(
...
test_that("MV5-AD boundary exceedances conservatively retain numerical ties", {
  observed <- 0.25
  null <- c(observed, -observed, observed - 2 * .Machine$double.eps, 0)
  expect_equal(.mv05ad_boundary_exceedances(null, observed), 3L)
})
