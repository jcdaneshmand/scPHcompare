test_that("MV5-AG freezes complete method, endpoint, and estimand panels", {
  methods <- mv05ag_method_map_v1()
  endpoints <- mv05ag_endpoint_registry_v1()
  estimands <- mv05ag_estimand_registry_v1(methods, endpoints)
  ...
  expect_true(all(methods$nested192_coordinate_count == 30L))
  expect_true(all(methods$baseline_cells == 384L))
  expect_true(all(methods$nested192_cells == 192L))
  expect_true(all(methods$baseline_point_metric == "euclidean"))
  expect_true(all(methods$nested192_point_metric == "euclidean"))
  expect_equal(nrow(endpoints), 2L)
  expect_equal(nrow(estimands), 24L)
  expect_equal(sum(estimands$estimand_role ==
                     "confirmatory_nested192_sensitivity"), 4L)
  expect_false(any(estimands$equivalence_or_noninferiority_claim_authorized))
  expect_silent(mv05ag_validate_estimand_registry_v1(estimands))
})

test_that("MV5-AG rejects pre-outcome result leakage", {
  expect_error(.mv05ag_assert_preoutcome(
    data.frame(reciprocal_rank = 1)), "result columns")
  expect_error(.mv05ag_assert_preoutcome(
    data.frame(outcomes_computed = TRUE)), "prohibited completed")
  expect_silent(.mv05ag_assert_preoutcome(
    data.frame(outcomes_computed = FALSE, execution_authorized = FALSE)))
})

test_that("MV5-AG queue is exact and remains unauthorized", {
  ...
  scope$configuration_id <- "nested_cells_192_pc30_euclidean_v1"
  ...
  scope$nested192_group_manifest_sha256 <- paste(rep("b", 64L), collapse = "")
  scope$baseline_coordinate_source_sha256 <- paste(rep("c", 64L), collapse = "")
  scope$nested192_coordinate_source_sha256 <- paste(rep("c", 64L), collapse = "")
  ...
  queue <- mv05ag_build_evaluation_queue_v1(
  ...
  expect_silent(mv05ag_validate_evaluation_queue_v1(queue))
})

test_that("MV5-AG queue validator detects authorization and axis drift", {
  folds <- paste0("large_loso_v1:S", sprintf("%02d", 1:15))
  scope <- expand.grid(
    fold_id = folds, seed = 20260805:20260809,
    representation = c("sct_whole", "inductive_integrated"),
  ...
  expect_error(mv05ag_validate_evaluation_queue_v1(queue),
