test_that("MV5-Y freezes complete method, endpoint, and estimand panels", {
  methods <- mv05y_method_map_v1()
  endpoints <- mv05y_endpoint_registry_v1()
  estimands <- mv05y_estimand_registry_v1(methods, endpoints)
  ...
  expect_equal(sum(estimands$estimand_role ==
                     "confirmatory_pc20_sensitivity"), 4L)
  expect_false(any(estimands$equivalence_or_noninferiority_claim_authorized))
  expect_silent(mv05y_validate_estimand_registry_v1(estimands))
})

test_that("MV5-Y rejects pre-outcome result leakage", {
  expect_error(.mv05y_assert_preoutcome(
    data.frame(reciprocal_rank = 1)), "result columns")
  expect_error(.mv05y_assert_preoutcome(
    data.frame(outcomes_computed = TRUE)), "prohibited completed")
  expect_silent(.mv05y_assert_preoutcome(
    data.frame(outcomes_computed = FALSE, execution_authorized = FALSE)))
})

test_that("MV5-Y queue is exact and remains unauthorized", {
  ...
  scope$configuration_id <- "cells384_pc20_euclidean_v1"
  ...
  scope$pc20_group_manifest_sha256 <- paste(rep("b", 64L), collapse = "")
  ...
  queue <- mv05y_build_evaluation_queue_v1(
  ...
  expect_silent(mv05y_validate_evaluation_queue_v1(queue))
})

test_that("MV5-Y queue validator detects authorization and axis drift", {
  folds <- paste0("large_loso_v1:S", sprintf("%02d", 1:15))
  scope <- expand.grid(
    fold_id = folds, seed = 20260805:20260809,
    representation = c("sct_whole", "inductive_integrated"),
  ...
  expect_error(mv05y_validate_evaluation_queue_v1(queue),
