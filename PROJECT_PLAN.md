...
| 6. Biological/practical validation | HCA exact-annotation/common-475 sensitivity complete; MV8-H exact 48-file acquisition, Ensembl-93 inputs, Cell Ranger 8.0.1 runtime/reference, one-unit count execution, matrix/QC review, eight-unit common-475 dual-view topology/all-active-level validation, MV8-J claim map, and the HCA_BM_002 exact-500 feasibility gate are auditable; all five HCA exact-500 SCT seed checks remain panel-blocked on the historical H5 axis (497–499 retained genes), while the raw-read Cell Ranger 8 sentinel restores all 500 targets | G6 not evaluated; remaining-unit exact500 processing is conditional on a later claim-specific authorization |
