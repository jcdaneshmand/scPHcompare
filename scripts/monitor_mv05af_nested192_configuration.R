...
for (package in c("processx", "ps", "digest")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for monitored MV5-AF execution.", call. = FALSE)
  }
}
args <- commandArgs(trailingOnly = TRUE)
if (length(args) != 13L) {
  stop(paste(
    "usage: monitor_mv05af_nested192_configuration.R EXECUTION_QUEUE PRIVATE_INVENTORY",
...
if (is.na(max_units) || max_units != 150L) {
  stop("MV5-AF nested192 monitor requires exactly 150 groups.", call. = FALSE)
}
head <- trimws(system2("git", c("rev-parse", "HEAD"), stdout = TRUE))
if (!identical(head, expected_head)) {
  stop("MV5-AF monitor is not at its bound prospective HEAD.", call. = FALSE)
}
...
if (nrow(queue) != max_units ||
  ...
  stop("MV5-AF bound nested192 identity is stale.", call. = FALSE)
}
...
write_checkpoint <- function(value) {
  ...
  if (!file.rename(temporary, resource_output)) {
    stop("MV5-AF resource checkpoint publication failed.", call. = FALSE)
  }
}
...
if (file.exists(resource_output)) {
  ...
  if (!all(required_metrics %in% names(metrics)) ||
    ...
    stop("MV5-AF existing resource checkpoint is invalid.", call. = FALSE)
  }
}
...
if (completed_count < nrow(queue)) for (index in seq.int(completed_count + 1L,
  ...
  process <- processx::process$new(
    ...
    args = c(
      "--vanilla", "scripts/run_mv05af_nested192_group.R", queue_path,
  ...
  while (process$is_alive()) {
    ...
    if (elapsed > 600 || stage_elapsed > 6 * 60 * 60) {
      guard <- "timeout_guard"
      process$kill_tree()
  ...
  row <- data.frame(
    contract_id = "mv05af_nested192_resource_metric_v1",
    ...
    program_elapsed_cap_seconds = 6 * 60 * 60,
    storage_cap_bytes = 4 * 1024^3, maximum_workers = 1L,
...
if (any(!metrics$disposition %in% c("completed", "reused_validated")) ||
    ...
    sum(metrics$elapsed_seconds) > 6 * 60 * 60 ||
    ...
    any(as.logical(metrics$labels_opened)) ||
    any(as.logical(metrics$outcomes_computed))) {
  stop("MV5-AF nested192 execution completed outside its contract.", call. = FALSE)
}
message("Completed 150 MV5-AF nested192 groups (", run_mode, ").")
