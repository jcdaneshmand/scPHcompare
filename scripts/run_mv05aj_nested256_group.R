...
if (length(args) != 11L) {
  stop(paste(
    "usage: run_mv05w_full_group.R EXECUTION_QUEUE PRIVATE_INVENTORY",
    "D1_ROOT G_ROOT RESULT_ROOT ROBUSTNESS_GROUP_ID EXPECTED_HEAD",
...
if (!identical(head, expected_head)) {
  stop("MV5-W runner is not at its bound prospective HEAD.", call. = FALSE)
}
implementation_files <- c(
  provenance = "R/provenance_utils.R",
  toy = "R/toy_baseline.R",
  ...
  mv05aj = "R/mv05aj_nested_execution.R",
  landscape = "scripts/mv05w_exact_landscape_group.py",
  group_runner = "scripts/run_mv05aj_nested256_group.R",
  monitor = "scripts/monitor_mv05aj_nested256_configuration.R"
)
...
if (!identical(implementation_sha, expected_implementation_sha) ||
    file_sha("scripts/mv05w_exact_landscape_group.py") != python_script_sha) {
  stop("MV5-W implementation identity is stale.", call. = FALSE)
}

queue <- read_public(queue_path)
mv05aj_validate_nested_queue_v1(queue)
unit <- queue[queue$robustness_group_id == group_id, , drop = FALSE]
if (nrow(unit) != 1L || unit$implementation_sha256 != implementation_sha ||
    !as.logical(unit$execution_authorized)) {
  stop("MV5-W requested smoke group is absent, stale, or unauthorized.",
       call. = FALSE)
}
...
if ((!is.null(environment_status) && environment_status != 0L) ||
  ...
  stop("MV5-W Python runtime identity is stale.", call. = FALSE)
}
...
final_dir <- file.path(result_root, safe_name(group_id))
validate_published <- function(path) {
  ...
  if (nrow(status) != 1L || status$status != "completed" ||
      status$robustness_group_id != group_id ||
...
if (dir.exists(final_dir)) {
  ...
  message("MV5-W group reused: ", group_id)
...
partial <- tempfile(pattern = paste0(".mv05w_", safe_name(group_id), "_"),
                    tmpdir = result_root)
...
expected_nested_ids <- lapply(source_views, function(view) list(
  nested192 = mv05t_nested_point_ids_v1(
    view$sample_id, seed, view$point_ids, 192L),
  nested256 = mv05t_nested_point_ids_v1(
    view$sample_id, seed, view$point_ids, 256L)))
if (length(views) != 90L || any(vapply(views, function(view) {
  !identical(dim(view$payload), c(as.integer(unit$cells),
    ...
    !identical(
      view$point_ids,
      expected_nested_ids[[view$sample_id]]$nested256
    ) ||
    !identical(view$point_ids[seq_len(192L)],
               expected_nested_ids[[view$sample_id]]$nested192) ||
    view$transformations$mv05u_robustness$point_selection !=
      "sha256_sample_seed_cell_nested_v1"
}, logical(1L)))) {
  stop("MV5-AJ transformed nested-256 view axis is incomplete.", call. = FALSE)
}

pairs <- mv05w_full_pair_coverage_v1(
  d1, group_id, unit$base_pair_axis_sha256
)
if (nrow(pairs) != as.integer(unit$biological_pairs)) {
  stop("MV5-W complete pair count disagrees with its queue.", call. = FALSE)
}
pair_path <- file.path(partial, "pair_scope.csv")
...
energy$contract_id <- "mv05w_energy_pair_v1"
energy$robustness_group_id <- group_id
...
energy <- energy[c(
  "contract_id", "robustness_group_id", "fold_id", "seed", "representation",
...
for (index in seq_along(views)) {
  ...
  view_rows[[index]] <- data.frame(
    contract_id = "mv05w_view_ph_metric_v1", robustness_group_id = group_id,
    ...
    selected_point_ids_sha256 = digest::digest(
      view$point_ids, algo = "sha256", serialize = TRUE
    ),
    nested192_point_ids_sha256 = digest::digest(
      expected_nested_ids[[sample_id]]$nested192,
      algo = "sha256", serialize = TRUE
    ),
    nested192_is_exact_prefix = identical(
      view$point_ids[seq_len(192L)],
      expected_nested_ids[[sample_id]]$nested192
    ),
    point_selection =
      view$transformations$mv05u_robustness$point_selection,
  ...
  if (nrow(finite)) {
    ...
    interval_rows[[interval_cursor]] <- data.frame(
      contract_id = "mv05w_finite_interval_staging_v1",
      robustness_group_id = group_id, sample_id = sample_id,
...
source_identity <- data.frame(
  contract_id = "mv05w_group_source_identity_v1",
  robustness_group_id = group_id, fold_id = unit$fold_id, seed = seed,
  representation = unit$representation,
  configuration_id = unit$configuration_id,
...
python_stdout <- system2(
  python_executable,
  c(
    "scripts/mv05w_exact_landscape_group.py",
    ...
    "--robustness-group-id", shQuote(group_id),
    "--expected-pairs", as.character(unit$biological_pairs),
    "--max-rows-per-subchunk", "250",
    "--script-sha256", shQuote(python_script_sha)
  ), stdout = TRUE, stderr = TRUE
)
...
if (nrow(landscape_summary) != 180L ||
    nrow(landscape_pairs) != as.integer(unit$landscape_request_rows) ||
    length(unique(landscape_pairs$subchunk_id)) !=
      as.integer(unit$landscape_subchunks) ||
    any(!is.finite(landscape_pairs$distance)) ||
    any(landscape_pairs$distance < 0) ||
    any(bool(landscape_pairs$outcomes_computed))) {
  stop("MV5-W exact landscape outputs are incomplete.", call. = FALSE)
}

method_rows <- mv05w_assemble_method_rows_v1(
  landscape_pairs, energy, group_id
)
if (nrow(method_rows) != as.integer(unit$assembled_method_rows)) {
  stop("MV5-W assembled method row count is incomplete.", call. = FALSE)
}
write_provenance_csv(method_rows, file.path(partial, "method_rows.csv"))

resource <- data.frame(
  contract_id = "mv05w_internal_resource_metric_v1",
  robustness_group_id = group_id, transformed_views = 90L,
  pair_coverage_rows = nrow(pairs), landscape_summary_rows = 180L,
  landscape_pair_rows = nrow(landscape_pairs), energy_pair_rows = nrow(energy),
  method_rows = nrow(method_rows),
...
artifact_files <- c(
  ...
  "landscape_pairs.csv", "method_rows.csv", "internal_resources.csv"
)
manifest <- data.frame(
  contract_id = "mv05w_group_artifact_manifest_v1",
  robustness_group_id = group_id, artifact_file = artifact_files,
...
status <- data.frame(
  contract_id = "mv05w_group_status_v1", robustness_group_id = group_id,
...
message("MV5-W group built: ", group_id)
