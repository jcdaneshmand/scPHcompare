...
if (length(args) != 2L) {
  stop("usage: snapshot_mv05aj_results.R RESULT_ROOT OUTPUT_CSV",
       call. = FALSE)
}
if (!requireNamespace("digest", quietly = TRUE)) {
  stop("digest is required for MV5-AJ snapshots.", call. = FALSE)
}
...
if (length(groups) != 150L || any(grepl("^\\.", basename(groups)))) {
  stop("MV5-AJ snapshot requires exactly 150 groups.", call. = FALSE)
}
...
if (length(files) != 1650L) {
  stop("MV5-AJ snapshot requires exactly 1,650 files.", call. = FALSE)
}
result <- data.frame(
  contract_id = "mv05aj_result_snapshot_v1",
...
message("Snapshotted 1,650 MV5-AJ files with timestamps.")
