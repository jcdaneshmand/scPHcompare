...
for (package in c("processx", "ps", "digest")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for monitored MV5-D3 execution.",
         call. = FALSE)
  }
}
...
if (length(args) != 11L) {
  stop(
    "usage: monitor_mv05d3_ph_groups.R MANIFEST FOLD_CACHE_DIR RESULT_ROOT ",
...
if (is.na(max_groups) || max_groups < 1L || max_groups > 75L ||
  ...
  stop("MV5-D3 monitor guards are invalid.", call. = FALSE)
}
...
groups <- unique(manifest[c(
  "group_id", "group_order", "fold_id", "seed"
)])
...
if (!is.null(existing) &&
  ...
  stop("Existing MV5-D3 group metrics are not safely resumable.",
       call. = FALSE)
}
...
launch_group <- function(group) {
  ...
  process <- processx::process$new(
    ...
    args = c(
      "--vanilla", "scripts/run_mv05d3_ph_group.R", manifest_path,
      group$group_id, fold_cache_dir, result_dir, audit_path,
      "build_or_resume"
    ),
...
while (nrow(pending) > 0L || length(running) > 0L) {
  ...
  for (id in names(running)) {
    ...
    if (!is.null(view_metrics)) {
      mv05d3_validate_view_metrics_v1(view_metrics, expected_jobs = 90L)
    }
    rows[[length(rows) + 1L]] <- data.frame(
      contract_id = "mv05d3_group_resource_metric_v1",
...
mv05d3_validate_group_metrics_v1(
  ...
  stage_cap_seconds = 12 * 3600
)
message("Completed ", nrow(metrics), " MV5-D3 fold-seed groups.")
