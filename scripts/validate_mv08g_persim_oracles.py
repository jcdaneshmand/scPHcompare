#!/usr/bin/env python3
"""Validate MV8-G landscape oracle rows with the accepted grouped Persim engine."""
...
def load_engine(path):
    spec = importlib.util.spec_from_file_location("mv08g_persim", path)
...
def atomic_csv(path, rows):
    if not rows:
        raise ValueError("Refusing to publish empty MV8-G Persim evidence.")
...
def main():
    ...
    for row in read_csv(args.r_oracles):
        ...
        keys = ((row["first_diagram_id"], degree),
                (row["second_diagram_id"], degree))
        for key in keys:
            if key not in landscapes:
                landscapes[key] = engine.build_landscape(grouped.get(key[0], []), key[1])
        ...
        tolerance = max(1e-10 + 1e-10 * abs(squared),
        ...
        passed = (math.isfinite(squared) and squared >= 0 and
                  rust_error <= tolerance and r_error <= tolerance)
        output.append({
            "contract_id": "mv08g_persim_oracle_v1",
            "oracle_id": row["oracle_id"],
            "component_id": row["component_id"],
            "oracle_scope": row["oracle_scope"],
    ...
    if len(output) != 12 or any(row["passed"] != "TRUE" for row in output):
        raise RuntimeError("MV8-G corrected-Persim oracle validation failed.")
    atomic_csv(args.output, output)
    print("Validated MV8-G landscapes: 12/12 corrected-Persim oracle rows.")
