#!/usr/bin/env python3
"""Exact all-active landscape staging for one MV5-W full-pair group."""
...
CONTRACT_ID = "mv05w_exact_all_active_landscape_group_v1"
ENGINE_ID = "persim_exact_critical_pairs_mv05w_v1"
...
def validate_inputs(views, intervals, pairs, group_id, expected_pairs):
    view_required = {
        "robustness_group_id", "sample_id", "diagram_sha256", "h0_intervals",
        "h1_intervals", "outcome_label_state", "outcomes_computed",
    }
    interval_required = {
        "robustness_group_id", "sample_id", "homology_dimension", "birth",
    ...
    if len(pairs) != expected_pairs or not pair_required.issubset(pairs[0]):
        raise ValueError("MV5-W complete pair coverage is incomplete.")
    sample_ids = [row["sample_id"] for row in views]
    if len(set(sample_ids)) != 90:
        raise ValueError("MV5-U sample identities are duplicated.")
    if len({row["pair_request_id"] for row in pairs}) != expected_pairs:
        raise ValueError("MV5-W pair identities are duplicated.")
    if any(row["pair_scope"] != "held_out_query_to_training_reference"
           for row in pairs):
        raise ValueError("MV5-W pair scope is invalid.")
    ...
    if any(row["robustness_group_id"] != group_id for row in views):
        raise ValueError("MV5-W view group identity drifted.")
    if any(row["robustness_group_id"] != group_id for row in intervals):
        raise ValueError("MV5-W interval group identity drifted.")
    for row in pairs:
...
def main():
    ...
    parser.add_argument("--robustness-group-id")
    parser.add_argument("--expected-pairs", type=int)
    parser.add_argument("--max-rows-per-subchunk", type=int, default=250)
    ...
    required = (args.views, args.intervals, args.pairs, args.summary_output,
                args.pair_output, args.robustness_group_id,
                args.expected_pairs)
    ...
    sample_ids = validate_inputs(
        views, intervals_rows, pairs, args.robustness_group_id,
        args.expected_pairs
    )
    ...
    for sample_id in sorted(sample_ids):
        for dimension in ("H0", "H1"):
            ...
            summary.append({
                ...
                "engine_id": ENGINE_ID,
                "robustness_group_id": args.robustness_group_id,
                "sample_id": sample_id,
    ...
    for dimension in ("H0", "H1"):
        rows = sorted(
            (row for row in output if row["homology_dimension"] == dimension),
            key=lambda row: row["pair_request_id"],
        )
        for offset, row in enumerate(rows):
            local_index = offset // args.max_rows_per_subchunk + 1
            row["subchunk_index"] = local_index
            row["subchunk_offset"] = offset % args.max_rows_per_subchunk + 1
            row["subchunk_id"] = "mv05w_subchunk_v1:" + canonical_sha({
                "group": args.robustness_group_id,
                "dimension": dimension,
                "index": local_index,
            })
            ordered.append(row)
    output = ordered
    expected_subchunks = 2 * math.ceil(
        args.expected_pairs / args.max_rows_per_subchunk
    )
    if (len(summary) != 180 or len(output) != 2 * args.expected_pairs or
            len({row["subchunk_id"] for row in output}) != expected_subchunks or
            any(sum(1 for item in output if item["subchunk_id"] == chunk) >
                args.max_rows_per_subchunk
                for chunk in {row["subchunk_id"] for row in output})):
        raise RuntimeError("MV5-W landscape output cardinality is invalid.")
    atomic_csv(args.summary_output, summary)
    atomic_csv(args.pair_output, output)
    print({"group": args.robustness_group_id, "landscapes": len(summary),
