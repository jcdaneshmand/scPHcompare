...
if (length(args) != 15L) {
  stop(
    "usage: validate_mv05d4_production.R PAIRS CHUNKS INPUT_AUDIT ",
    "GROUP_METRICS OUTPUT_ROOT STATUS_ROOT REPEAT_OUTPUT REPEAT_STATUS ",
    "SUPERSEDED_OUTPUT COMPLETION_OUT CHUNK_OUT GROUP_OUT REPEAT_OUT ",
    "CORRECTION_OUT COMBINED_GZ_OUT", call. = FALSE
  )
}
...
read_tree <- function(root, pattern) {
  ...
  if (!length(paths)) stop("No MV5-D4 artifacts matched ", pattern,
                           call. = FALSE)
...
superseded_root <- normalizePath(args[[9L]], winslash = "/", mustWork = TRUE)
mv05d4_validate_pair_manifest_v1(pairs)
expected_chunks <- mv05d4_chunk_summary_v1(pairs)
if (!identical(chunks, expected_chunks)) {
  stop("MV5-D4 public chunk manifest is not reproducible.", call. = FALSE)
}
implementation_sha <- file_sha("scripts/mv05d4_landscape_group.py")
...
if (nrow(inputs) != 75L || nrow(groups) != 75L ||
  ...
  stop("MV5-D4 group input or production metrics are invalid.",
       call. = FALSE)
}
...
if (length(outputs$paths) != 360L || length(statuses$paths) != 360L ||
    ...
    any(observed$contract_id !=
          "mv05d4_all_active_exact_landscape_distance_v1") ||
  ...
  stop("MV5-D4 all-record independent validation failed.", call. = FALSE)
}

chunk_validation <- data.frame(
  contract_id = "mv05d4_independent_chunk_validation_v1",
...
group_validation <- do.call(rbind, lapply(names(group_sets), function(stem) {
  ...
  if (is.na(group_id)) stop("MV5-D4 output directory has no frozen group.",
                            call. = FALSE)
  group_rows <- observed[observed$group_id == group_id, , drop = FALSE]
  data.frame(
    contract_id = "mv05d4_independent_group_validation_v1",
    group_id = group_id,
...
if (nrow(group_validation) != 75L ||
  ...
  stop("MV5-D4 group validation cardinality failed.", call. = FALSE)
}
...
if (nrow(h0) != 35350L || nrow(h1) != 35350L || anyNA(h1_index) ||
  ...
  stop("MV5-D4 H0/H1 component pairing failed.", call. = FALSE)
}
h1 <- h1[h1_index, , drop = FALSE]
combined_squared <- h0$squared_distance + h1$squared_distance
combined <- data.frame(
  contract_id = "mv05d4_cell_landscape_component_distance_v1",
...
dir.create(dirname(args[[15L]]), recursive = TRUE, showWarnings = FALSE)
connection <- gzfile(args[[15L]], open = "wt", compression = 9)
...
if (length(production_paths) != length(repeat_paths)) {
  stop("MV5-D4 repeat artifact count differs.", call. = FALSE)
}
repeat_validation <- data.frame(
  contract_id = "mv05d4_exact_group_repeat_v1",
...
if (any(!repeat_validation$file_bytes_identical) ||
  ...
  stop("MV5-D4 exact group repeat failed.", call. = FALSE)
}

superseded <- read_tree(superseded_root, "[.]csv$")$data
old_match <- match(observed$pair_request_id, superseded$pair_request_id)
comparison_fields <- c(
  "pair_request_id", "homology_dimension", "difference_active_levels",
  "difference_segments", "difference_critical_points", "distance",
  "squared_distance", "exact", "all_active_levels",
  "absolute_error_estimate"
)
accepted_scientific <- observed[
  order(observed$pair_request_id), comparison_fields, drop = FALSE
]
superseded_scientific <- superseded[
  order(superseded$pair_request_id), comparison_fields, drop = FALSE
]
rownames(accepted_scientific) <- NULL
rownames(superseded_scientific) <- NULL
correction <- data.frame(
  contract_id = "mv05d4_timing_separation_correction_v1",
  superseded_rows = nrow(superseded), accepted_rows = nrow(observed),
  matched_pair_request_ids = sum(!is.na(old_match)),
  superseded_embedded_pair_timing = "pair_seconds" %in% names(superseded),
  accepted_embedded_pair_timing = "pair_seconds" %in% names(observed),
  scientific_fields_identical = !anyNA(old_match) &&
    identical(accepted_scientific, superseded_scientific),
  superseded_disposition = "rejected_nondeterministic_timing_in_scientific_csv",
  accepted_disposition = "accepted_timing_separated_v3",
  outcome_label_state = "closed", biological_outcomes_computed = FALSE,
  stringsAsFactors = FALSE
)
if (nrow(superseded) != 70700L ||
    !correction$scientific_fields_identical ||
    !correction$superseded_embedded_pair_timing ||
    correction$accepted_embedded_pair_timing) {
  stop("MV5-D4 v2-to-v3 correction validation failed.", call. = FALSE)
}

completion <- data.frame(
  contract_id = "mv05d4_landscape_distance_completion_v1",
...
write_provenance_csv(completion, args[[10L]])
write_provenance_csv(chunk_validation, args[[11L]])
write_provenance_csv(group_validation, args[[12L]])
write_provenance_csv(repeat_validation, args[[13L]])
write_provenance_csv(correction, args[[14L]])
message("Independently validated all 70,700 accepted MV5-D4 distances.")
