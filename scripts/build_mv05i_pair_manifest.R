...
if (length(args) != 4L) {
  stop(
    "usage: build_mv05i_pair_manifest.R PH_MANIFEST VIEW_METRICS ",
...
pairs <- mv05i_build_pair_manifest_v1(ph_manifest, view_metrics)
chunks <- mv05i_chunk_summary_v1(pairs)
write_provenance_csv(pairs, args[[3L]])
write_provenance_csv(chunks, args[[4L]])
message(
  "Wrote ", nrow(pairs), " MV5-I dimension-pair rows in ",
  nrow(chunks), " chunks."
)
