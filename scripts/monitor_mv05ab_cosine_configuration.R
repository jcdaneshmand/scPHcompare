...
if (length(args) != 13L) {
  stop(paste(
    "usage: monitor_mv05ab_cosine_configuration.R EXECUTION_QUEUE PRIVATE_INVENTORY",
...
if (is.na(max_units) || max_units != 150L) {
  stop("MV5-AB cosine monitor requires exactly 150 groups.", call. = FALSE)
}
head <- trimws(system2("git", c("rev-parse", "HEAD"), stdout = TRUE))
if (!identical(head, expected_head)) {
  stop("MV5-AB monitor is not at its bound prospective HEAD.", call. = FALSE)
}
...
if (nrow(queue) != max_units ||
  ...
  stop("MV5-AB bound cosine identity is stale.", call. = FALSE)
}
...
if (completed_count < nrow(queue)) for (index in seq.int(completed_count + 1L,
  ...
  process <- processx::process$new(
    ...
    args = c(
      "--vanilla", "scripts/run_mv05ab_cosine_group.R", queue_path,
  ...
  while (process$is_alive()) {
    ...
    if (elapsed > 600 || stage_elapsed > 8 * 60 * 60) {
      guard <- "timeout_guard"
      process$kill_tree()
  ...
  row <- data.frame(
    contract_id = "mv05ab_cosine_resource_metric_v1",
    ...
    program_elapsed_cap_seconds = 8 * 60 * 60,
...
if (any(!metrics$disposition %in% c("completed", "reused_validated")) ||
    ...
    sum(metrics$elapsed_seconds) > 8 * 60 * 60 ||
    ...
    any(as.logical(metrics$labels_opened)) ||
    any(as.logical(metrics$outcomes_computed))) {
  stop("MV5-AB cosine execution completed outside its contract.", call. = FALSE)
}
message("Completed 150 MV5-AB cosine groups (", run_mode, ").")
