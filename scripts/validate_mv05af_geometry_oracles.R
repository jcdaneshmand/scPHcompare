...
for (package in c("digest", "ripserr")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for MV5-AF independent validation.",
         call. = FALSE)
  }
}
args <- commandArgs(trailingOnly = TRUE)
if (length(args) != 6L) {
  stop(paste("usage: validate_mv05af_geometry_oracles.R QUEUE D1_ROOT G_ROOT",
...
if (nrow(queue) != 150L || !is.finite(tolerance) || tolerance <= 0) {
  stop("MV5-AF independent geometry inputs are invalid.", call. = FALSE)
}
...
for (index in seq_len(nrow(queue))) {
  ...
  if (length(ids) != 90L || !setequal(ids, metrics$sample_id) ||
    ...
    stop("MV5-AF independent sample axis failed at group ", index,
         call. = FALSE)
  }
  ...
  for (sample_id in ids) {
    ...
    if (!identical(dim(x), c(384L, 30L)) || any(!is.finite(x)) ||
      ...
      stop("MV5-AF independent source view failed: ", sample_id,
           call. = FALSE)
    }
    ordering <- point_order(sample_id, seed, rownames(x))
    selected_ids <- ordering[seq_len(192L)]
    nested_256_ids <- ordering[seq_len(256L)]
    if (!all(selected_ids %in% nested_256_ids)) {
      stop("MV5-AF independent nested inclusion failed: ", sample_id,
           call. = FALSE)
    }
    ...
    if (nrow(metric) != 1L ||
      ...
      stop("MV5-AF independent selected identity failed: ", sample_id,
           call. = FALSE)
    }
    ...
    if (!close(expected, observed)) {
      stop("MV5-AF independent H0 MST failed: ", sample_id,
           call. = FALSE)
    }
    ...
    if (sample_id %in% selected_pair_ids) {
      ...
      if (ncol(direct) != 3L) {
        stop("MV5-AF independent H1 diagram shape failed.", call. = FALSE)
      }
      ...
      if (!identical(dim(expected_h1), dim(observed_h1)) ||
        ...
        stop("MV5-AF independent H1 diagram failed: ", sample_id,
             call. = FALSE)
      }
      h1_cursor <- h1_cursor + 1L
      h1_rows[[h1_cursor]] <- data.frame(
        contract_id = "mv05af_independent_h1_diagram_oracle_v1",
  ...
  if (any(!is.finite(metric_min)) || any(!is.finite(metric_max)) ||
    ...
    stop("MV5-AF stored Euclidean norm metric failed at group ", index,
         call. = FALSE)
  }
  if (unit$fold_id %in% selected_folds) {
    ...
    if (length(observed) != 1L || !close(expected, observed)) {
      stop("MV5-AF independent energy oracle failed at group ", index,
           call. = FALSE)
    }
    energy_cursor <- energy_cursor + 1L
    energy_rows[[energy_cursor]] <- data.frame(
      contract_id = "mv05af_independent_energy_oracle_v1",
      robustness_group_id = unit$robustness_group_id,
      fold_id = unit$fold_id, seed = seed,
  ...
  group_rows[[index]] <- data.frame(
    contract_id = "mv05af_independent_geometry_group_v1",
    robustness_group_id = unit$robustness_group_id,
    fold_id = unit$fold_id, seed = seed,
    representation = unit$representation, views = length(ids),
    h0_mst_edges = sum(intervals$homology_dimension == "H0"),
    selected_points_per_view = 192L,
    nested_256_superset_verified = TRUE,
    ...
    labels_opened = FALSE, rankings_computed = FALSE,
    outcomes_computed = FALSE, stringsAsFactors = FALSE)
  if (index %% 10L == 0L) message("MV5-AF independent geometry groups: ", index)
}
...
if (nrow(groups) != 150L || total_views != 13500L ||
    total_h0_edges != 13500L * 191L || nrow(energies) != 30L ||
    nrow(h1_oracles) != 60L ||
    length(unique(energies$fold_id)) != 3L ||
    length(unique(energies$seed)) != 5L ||
    length(unique(energies$representation)) != 2L) {
  stop("MV5-AF independent oracle totals failed.", call. = FALSE)
}
summary <- data.frame(
  contract_id = "mv05af_independent_geometry_summary_v1",
  ...
  labels_opened = FALSE, rankings_computed = FALSE,
  outcomes_computed = FALSE, stringsAsFactors = FALSE)
write_once(groups, "mv05af-independent-geometry-groups-2026-08-11.csv")
write_once(energies, "mv05af-independent-energy-oracles-2026-08-11.csv")
write_once(h1_oracles, "mv05af-independent-h1-diagram-oracles-2026-08-11.csv")
write_once(summary, "mv05af-independent-geometry-summary-2026-08-11.csv")
message("MV5-AF independent geometry passed 13,500 views and 30 energy oracles.")
