...
if (length(args) != 11L) {
  stop(paste(
    "usage: validate_mv05ab_configuration.R QUEUE INVENTORY RESULT_ROOT",
...
for (package in c("digest")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for MV5-X validation.", call. = FALSE)
  }
}
...
if (!identical(current_head, execution_head) || nrow(authorized) != 150L ||
    ...
    sum(resources$elapsed_seconds) > 8 * 60 * 60 ||
  ...
  stop("MV5-X aggregate execution/resume contract failed.", call. = FALSE)
}
...
for (index in seq_len(nrow(authorized))) {
  ...
  if (!dir.exists(path) || length(files) != 11L || any(file.info(
    ...
    )$isdir)) stop("MV5-X group file cardinality failed.", call. = FALSE)
  ...
  if (nrow(status) != 1L || nrow(source_identity) != 1L ||
    ...
    stop("MV5-X group identity/manifest failed at row ", index,
         call. = FALSE)
  }
  ...
  if (!all(manifest_ok) || file_sha(file.path(path, "artifact_manifest.csv")) !=
    ...
    stop("MV5-X artifact hash validation failed at row ", index,
         call. = FALSE)
  }
  ...
  if (nrow(expected_d1) != 1L || nrow(expected_g) != 1L ||
    ...
    stop("MV5-X private source identity failed at row ", index,
         call. = FALSE)
  }
  ...
  if (nrow(views) != 90L || any(views$point_count != unit$cells) ||
      ...
      any(abs(views$minimum_row_norm - 1) > 1e-10) ||
      any(abs(views$maximum_row_norm - 1) > 1e-10) ||
    ...
    stop("MV5-X scientific/cardinality contract failed at row ", index,
         call. = FALSE)
  }
  ...
  for (method_id in names(expected)) {
    ...
    if (anyNA(observed$pair_request_id) ||
      ...
      stop("MV5-X four-method reconstruction failed at row ", index,
           call. = FALSE)
    }
  }

  completion[[index]] <- data.frame(
    contract_id = "mv05ab_cosine_unit_completion_v1",
...
if (total_views != 13500L || total_pairs != 70700L ||
  ...
  stop("MV5-X aggregate scientific cardinalities failed.", call. = FALSE)
}

repeat_units <- authorized[strict(authorized$deterministic_repeat_required),
                           , drop = FALSE]
if (nrow(repeat_units) != 2L) {
  stop("MV5-X requires two prospectively marked repeat groups.", call. = FALSE)
}
repeat_rows <- list()
cursor <- 0L
...
validation <- data.frame(
  contract_id = "mv05ab_independent_validation_v1",
  category = c(
    "queue_authorization", "execution_identity", "private_sources",
    "group_publications", "manifest_hashes", "view_ph_mst",
  ...
  passed = TRUE,
  observed = c(
    "150_cosine_only", execution_head, "150_hash_bound",
...
resume_evidence <- data.frame(
  contract_id = "mv05ab_resume_validation_v1",
...
decision <- data.frame(
  contract_id = "mv05ab_cosine_configuration_decision_v1",
  configuration_id = "cells384_pc30_cosine_chord_v1",
  ...
  next_action = "prefreeze_cosine_prediction_locked_outcome_evaluation",
  labels_opened = FALSE, outcomes_computed = FALSE,
  stringsAsFactors = FALSE
)
resources$contract_id <- "mv05ab_cosine_production_resource_metric_v1"
write_once(resources, file.path(
  output_dir, "mv05ab-cosine-production-resources-2026-08-11.csv"
))
write_once(completion, file.path(
  output_dir, "mv05ab-cosine-unit-completion-2026-08-11.csv"
))
write_once(validation, file.path(
  output_dir, "mv05ab-independent-validation-2026-08-11.csv"
))
write_once(repeat_evidence, file.path(
  output_dir, "mv05ab-deterministic-repeat-2026-08-11.csv"
))
write_once(resume_evidence, file.path(
  output_dir, "mv05ab-resume-validation-2026-08-11.csv"
))
write_once(decision, file.path(
  output_dir, "mv05ab-configuration-decision-2026-08-11.csv"
))
message("MV5-AB independent artifact validation passed: groups=150 pairs=70700 outcomes=0")
