...
if (length(args) != 4L) {
  stop(paste("usage: validate_mv05al_outcome_execution.R EXTERNAL_METADATA_PATH",
...
rankings <- readc("docs/audits/mv05al-nested256-rankings-2026-08-11.csv.gz")
nested256_public <- readc(paths("mv05al-nested256-query-method-outcomes-2026-08-11.csv"))
long_public <- readc(paths("mv05al-nested256-query-endpoints-2026-08-11.csv"))
paired_public <- readc(paths("mv05al-paired-query-method-endpoints-2026-08-11.csv"))
query_public <- readc(paths("mv05al-query-estimands-2026-08-11.csv"))
sample_public <- readc(paths("mv05al-sample-estimands-2026-08-11.csv"))
tissue_public <- readc(paths("mv05al-tissue-estimands-2026-08-11.csv"))
macro_public <- readc(paths("mv05al-macro-estimands-2026-08-11.csv"))
interval_public <- readc(paths("mv05al-estimand-intervals-2026-08-11.csv"))
primary_public <- readc(paths("mv05al-primary-contrasts-2026-08-11.csv"))
bootstrap_public <- readc(paths("mv05al-bootstrap-audit-2026-08-11.csv"))
random_public <- readc(paths("mv05al-randomization-audit-2026-08-11.csv"))
completion <- readc(paths("mv05al-outcome-unit-completion-2026-08-11.csv"))
production <- readc(paths("mv05al-production-summary-2026-08-11.csv"))
label_provenance <- readc(paths("mv05al-label-source-provenance-2026-08-11.csv"))
source_validation <- readc(paths("mv05al-outcome-source-validation-2026-08-11.csv"))
registry <- readc("docs/audits/mv05ak-estimand-registry-2026-08-11.csv")
method_map <- readc("docs/audits/mv05ak-method-map-2026-08-11.csv")
...
for (index in groups) {
  ...
  family <- method_map$family_id[
    method_map$representation == part$representation[[1L]] &
      method_map$nested256_method_id == part$method_id[[1L]]]
...
nested256 <- do.call(rbind, endpoint_rows)
...
nested256_public <- nested256_public[match(key(nested256), key(nested256_public)), ]
if (rank_rows != 282800L || nrow(nested256) != 3600L || anyNA(nested256_public$method_id) ||
    !close_num(nested256$reciprocal_rank, nested256_public$reciprocal_rank) ||
    !identical(as.logical(nested256$one_nn_correct), as.logical(nested256_public$one_nn_correct)) ||
    !identical(as.integer(nested256$first_same_tissue_rank),
               as.integer(nested256_public$first_same_tissue_rank))) {
  stop("Independent endpoint reconstruction failed.")
}
long_expected <- rbind(
  data.frame(
    nested256[kfields], endpoint_id = "cross_study_tissue_mrr_v1",
    value = nested256$reciprocal_rank, stringsAsFactors = FALSE),
  data.frame(
    nested256[kfields],
    endpoint_id = "cross_study_tissue_1nn_balanced_accuracy_v1",
    value = as.numeric(nested256$one_nn_correct), stringsAsFactors = FALSE))
long_fields <- c(kfields, "endpoint_id")
long_public <- long_public[match(key(long_expected, long_fields),
                                 key(long_public, long_fields)), ]
if (nrow(long_expected) != 7200L || anyNA(long_public$endpoint_id) ||
    !close_num(long_expected$value, long_public$value)) {
  stop("Independent long-endpoint reconstruction failed.")
}
...
baseline <- baseline[match(pk(nested256), pk(baseline)), ]
direct_rr <- nested256$reciprocal_rank - baseline$reciprocal_rank
direct_nn <- as.numeric(nested256$one_nn_correct) - as.numeric(baseline$one_nn_correct)
paired_public <- paired_public[match(pk(nested256), pk(paired_public)), ]
...
direct <- do.call(rbind, lapply(seq_len(nrow(nested256)), function(i) {
  rbind(
    data.frame(nested256[i, pair_fields], endpoint_id = "cross_study_tissue_mrr_v1",
               estimand_type = "direct_nested256_cells_minus_384_cells", value = direct_rr[[i]]),
    data.frame(nested256[i, pair_fields],
               endpoint_id = "cross_study_tissue_1nn_balanced_accuracy_v1",
               estimand_type = "direct_nested256_cells_minus_384_cells", value = direct_nn[[i]]))
}))
did <- do.call(rbind, lapply(c("h0", "h1"), function(family) {
  top <- nested256$family_id == family; energy <- nested256$family_id == "energy"
  did_fields <- setdiff(pair_fields, "family_id")
  ek <- key(nested256[energy, ], did_fields)
  energy_index <- which(energy)[match(key(nested256[top, ], did_fields), ek)]
  top_index <- which(top)
  rbind(
    data.frame(nested256[top_index, pair_fields], endpoint_id = "cross_study_tissue_mrr_v1",
      estimand_type = "topology_increment_nested256_minus_384_cells_difference_in_differences",
      value = direct_rr[top_index] - direct_rr[energy_index]),
    data.frame(nested256[top_index, pair_fields],
      endpoint_id = "cross_study_tissue_1nn_balanced_accuracy_v1",
      estimand_type = "topology_increment_nested256_minus_384_cells_difference_in_differences",
...
primary_registry <- ordered_registry[
  ordered_registry$estimand_role == "confirmatory_nested256_sensitivity", ]
...
for (i in seq_len(nrow(completion))) {
  stem <- sub("^mv05ak_eval_v1:", "", completion$evaluation_unit_id[[i]])
...
if (!all(private_ok) || !all(status_ok) || nrow(completion) != 150L ||
    !all(completion$state == "completed") ||
    !all(completion$labels_opened_after_prediction_lock) ||
    !all(completion$outcomes_computed) ||
    any(completion$method_selection_executed) ||
    nrow(source_validation) != 196L || !all(source_validation$matched) ||
    !all(source_validation$validated_before_label_access) ||
    nrow(label_provenance) != 1L ||
    label_provenance$parsed_label_columns != "Tissue.x_only" ||
    label_provenance$approach_or_other_label_columns_parsed ||
    nrow(production) != 1L || production$prediction_groups != 150L ||
    production$ranking_rows != 282800L || production$outcome_groups != 150L ||
    production$query_method_rows != 3600L ||
    production$long_query_endpoint_rows != 7200L ||
    production$macro_estimands != 24L || production$intervals != 24L ||
    production$primary_tests != 4L || production$clustering_executed ||
    production$other_configuration_executed ||
    production$refit_or_rerank_executed ||
    production$method_selection_executed ||
    production$equivalence_claim_authorized) {
  stop("Private outcome identity or execution boundary failed.")
}

validation <- data.frame(
  contract_id = "mv05al_outcome_independent_validation_v1",
  validation_id = c(
    "canonical_rankings", "tissue_label_join", "query_method_outcomes",
    "long_query_endpoints",
    ...
    "complete_reporting_axes", "source_and_label_boundary",
    "zero_prohibited_operations"),
  passed = TRUE,
  evidence = c(
    "282800_rows", "90_samples_15_studies_5_tissues_tissue_only",
    "3600_query_method_rows", "7200_long_endpoint_rows", "3600_exact_pairs",
    "10800_query_rows_24_estimands",
    ...
    "7200_endpoints_24_intervals_4_tests",
    "196_sources_revalidated_before_Tissue.x_only",
    "no_clustering_other_config_refit_rerank_selection_equivalence"),
...
message("MV5-AL independent outcome validation passed: 17 categories, 24 estimands, 4 tests")
