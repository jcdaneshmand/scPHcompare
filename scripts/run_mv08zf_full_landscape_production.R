...
# Run the committed MV8-ZF landscape queue serially. Scientific outputs and
...
if (!(length(args) %in% c(12L, 13L))) stop(paste(
  "usage: run_mv08zf_full_landscape_production.R <mv08zf-prefreeze>",
  "<mv08z-prefreeze> <private-bindings> <mv08s-private> <mv08v-private>",
  "<rust-library> <private-output> <public-output> <expected-head>",
  "<mv08zb-prefreeze> <mv08zc-prefreeze> <mv08zd-prefreeze> [--resume]"
), call. = FALSE)
...
recovery_roots <- vapply(args[10:12], normalizePath, character(1L), mustWork = TRUE)
resume <- length(args) == 13L && identical(args[[13L]], "--resume")
if (length(args) == 13L && !resume) stop("unknown MV8-ZF runner flag", call. = FALSE)
environment_head <- tolower(trimws(Sys.getenv("MV08ZF_GIT_HEAD", unset = "")))
if (!grepl("^[0-9a-f]{40}$", expected_head) || expected_head != environment_head) {
  stop("MV8-ZF exact committed HEAD binding failed", call. = FALSE)
}
if ((dir.exists(private_root) || dir.exists(public_root)) && !resume) {
  stop("MV8-ZF output roots exist; explicit --resume required", call. = FALSE)
}
if (resume && (!dir.exists(private_root) || !dir.exists(public_root))) {
  stop("MV8-ZF resume requires both existing roots", call. = FALSE)
}
...
for (root in recovery_roots) {
  manifest <- list.files(root, pattern = "artifact-manifest[.]csv$", full.names = FALSE)
  if (length(manifest) != 1L) stop("MV8-ZF recovery manifest drift", call. = FALSE)
...
if (!all(file.exists(source_paths)) ||
  ...
  stop("MV8-ZF PH input rehash failed", call. = FALSE)
}

free_disk <- as.numeric(Sys.getenv("MV08ZF_FREE_DISK_BYTES", unset = ""))
available_memory <- as.numeric(Sys.getenv("MV08ZF_AVAILABLE_MEMORY_BYTES", unset = ""))
if (!is.finite(free_disk) || !is.finite(available_memory) ||
  ...
  stop("MV8-ZF launch headroom gate failed", call. = FALSE)
}
...
if (resume) {
  if (nrow(completed) > nrow(queue) ||
    ...
    stop("MV8-ZF resume is not an exact completed strict prefix", call. = FALSE)
  }
  if (nrow(completed)) for (index in seq_len(nrow(completed))) {
    ...
    if (!all(file.exists(paths)) || nrow(status) != 1L ||
      ...
      stop("MV8-ZF completed prefix artifact drift at order ", index, call. = FALSE)
    }
  }
  ...
  if (!setequal(normalizePath(actual_logs, mustWork = TRUE),
    ...
    stop("MV8-ZF resume has ambiguous logs beyond completed prefix", call. = FALSE)
  }
} else if (any(file.exists(c(ledger_path, completion_path, progress_path)))) {
  stop("MV8-ZF public output unexpectedly pre-exists", call. = FALSE)
}

publish_progress <- function(state) {
  value <- data.frame(
    contract_id = "mv08zf_progress_v1", execution_head = expected_head,
...
run_child <- function(index, row) {
  ...
  if (any(file.exists(paths)) || dir.exists(dirname(paths[[1L]]))) {
    stop("MV8-ZF unowned chunk output exists at order ", index, call. = FALSE)
  }
  ...
  if (any(file.exists(c(stdout, stderr)))) {
    stop("MV8-ZF ambiguous prior logs at order ", index, call. = FALSE)
  }
  dir.create(dirname(dirname(paths[[1L]])), recursive = TRUE, showWarnings = FALSE)
  Sys.setenv(
    MV08Z_RECOVERY_CHAIN = paste(c(recovery_roots, zf_root), collapse = "|"),
    MV08ZF_PREFREEZE = zf_root,
    MV08ZF_GIT_HEAD = expected_head
  )
  started <- Sys.time()
  ...
  metric <- data.frame(
    contract_id = "mv08zf_resource_ledger_v1", execution_head = expected_head,
  ...
  if (!valid) stop("MV8-ZF chunk failed closed at production order ", index, call. = FALSE)
  completion <- data.frame(
    contract_id = "mv08zf_chunk_completion_v1", execution_head = expected_head,
    production_order = index, global_chunk_order = row$global_chunk_order,
    group_order = row$group_order, chunk_order = row$chunk_order,
    ...
    level_cap_applied = FALSE, workers = 1L, retries = 0L,
    outcome_label_state = "closed", biological_outcomes_computed = FALSE,
    stringsAsFactors = FALSE
  )
...
if (start <= nrow(queue)) for (index in seq.int(start, nrow(queue))) {
  if ((nrow(ledger) && sum(ledger$elapsed_seconds) >=
    ...
    stop("MV8-ZF aggregate resource cap reached; evidence preserved", call. = FALSE)
  }
  run_child(index, queue[index, , drop = FALSE])
  publish_progress("running")
}
publish_progress("landscape_production_complete_closure_pending")
cat("MV8-ZF full landscape production completed 628/628 chunks; MV8-ZG closure required\n")
