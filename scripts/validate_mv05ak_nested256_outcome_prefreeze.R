...
if (!requireNamespace("digest", quietly = TRUE)) {
  stop("digest is required for MV5-AK validation.", call. = FALSE)
}

args <- commandArgs(trailingOnly = TRUE)
if (length(args) != 4L) {
  stop(paste(
    "usage: validate_mv05ak_nested256_outcome_prefreeze.R",
    "MV05AJ_RESULT_ROOT EXTERNAL_METADATA_PATH AUDIT_DIR OUTPUT_DIR"),
    call. = FALSE)
}
  ...
  function(category = NULL, passed = NULL, observed = NULL, done = FALSE) {
    if (!done) {
      rows[[length(rows) + 1L]] <<- data.frame(
        contract_id = "mv05ak_independent_validation_v1",
        category = category, passed = isTRUE(passed), observed = observed,
        label_values_read = FALSE, nested256_rankings_computed = FALSE,
...
expected_files <- c(
  "mv05ak-acceptance-criteria-2026-08-11.csv",
  "mv05ak-source-freeze-2026-08-11.csv",
  "mv05ak-method-map-2026-08-11.csv",
  "mv05ak-endpoint-registry-2026-08-11.csv",
  "mv05ak-estimand-registry-2026-08-11.csv",
  "mv05ak-prediction-axis-compatibility-2026-08-11.csv",
  "mv05ak-label-join-prefreeze-2026-08-11.csv",
  "mv05ak-prediction-lock-contract-2026-08-11.csv",
  "mv05ak-evaluation-queue-2026-08-11.csv",
  "mv05ak-clustering-compatibility-disposition-2026-08-11.csv",
  "mv05ak-validation-plan-2026-08-11.csv",
  "mv05ak-abort-rules-2026-08-11.csv",
  "mv05ak-resource-envelope-2026-08-11.csv",
  "mv05ak-reporting-contract-2026-08-11.csv",
  "mv05ak-prefreeze-decision-2026-08-11.csv")
paths <- file.path(audit_dir, expected_files)
if (any(!file.exists(paths))) stop("MV5-AK public assembly is incomplete.", call. = FALSE)
...
prohibited <- c(
  "tissue", "tissue.x", "approach", "approach.x", "reciprocal_rank",
  "one_nn_correct", "estimate", "p_value", "adjusted_p_value", "winner",
  "method_rank")
  ...
  !any(tolower(names(value)) %in% prohibited) &&
    all(vapply(intersect(
      c("outcomes_computed", "evaluation_executed", "ranking_executed",
        "rankings_computed", "method_selection_executed", "labels_opened",
        "label_values_read", "label_join_executed"), names(value)),
      function(column) !any(truth(value[[column]])), logical(1L)))
}, logical(1L)))
add_check("public_outcome_closure", safe_public,
          "labels_0_nested256_rankings_0_outcomes_0")

criteria <- public[["mv05ak-acceptance-criteria-2026-08-11.csv"]]
...
source <- public[["mv05ak-source-freeze-2026-08-11.csv"]]
...
queue <- public[["mv05ak-evaluation-queue-2026-08-11.csv"]]
queue_ok <- nrow(queue) == 150L && !anyDuplicated(queue$evaluation_unit_id) &&
  ...
  all(truth(queue$nested192_prefix_identity_exact)) &&
  all(truth(queue$nested256_subset_384_identity_exact)) &&
  all(queue$baseline_coordinate_source_sha256 ==
        queue$nested256_coordinate_source_sha256) &&
  !any(truth(queue$execution_authorized))
add_check(
  "evaluation_queue", queue_ok,
  "150_groups_282800_pairside_rows_exact_192_256_384_nesting_unauthorized")

method_map <- public[["mv05ak-method-map-2026-08-11.csv"]]
methods_expected <- c(
...
for (index in seq_len(nrow(queue))) {
  ...
  if (!all(file.exists(c(manifest_path, method_path, identity_path, status_path)))) {
    stop("Missing private MV5-AJ group: ", unit$robustness_group_id, call. = FALSE)
  }
  identity <- read_csv(identity_path); status <- read_csv(status_path)
  rows <- read_csv(method_path)
  manifest_source <- grepl("^nested256_private_group_manifest_", source$source_id) &
...
for (representation in names(baseline_frames)) {
  ...
  mapped <- map$nested256_method_id[match(frame$method_id, map$baseline_method_id)]
...
axis_public <- public[["mv05ak-prediction-axis-compatibility-2026-08-11.csv"]]
axis_ok <- axis_ok && nrow(axis_public) == 8L &&
  all(truth(axis_public$fold_seed_query_reference_axis_exact)) &&
  all(axis_public$missing_nested256_rows == 0L) &&
  all(axis_public$excess_nested256_rows == 0L)
...
label_join <- public[["mv05ak-label-join-prefreeze-2026-08-11.csv"]]
...
endpoints <- public[["mv05ak-endpoint-registry-2026-08-11.csv"]]
estimands <- public[["mv05ak-estimand-registry-2026-08-11.csv"]]
registry_ok <- nrow(method_map) == 8L && nrow(endpoints) == 2L &&
  nrow(estimands) == 24L && !anyDuplicated(estimands$estimand_id) &&
  sum(estimands$estimand_role == "confirmatory_nested256_sensitivity") == 4L &&
...
lock <- public[["mv05ak-prediction-lock-contract-2026-08-11.csv"]]
...
clustering <- public[["mv05ak-clustering-compatibility-disposition-2026-08-11.csv"]]
clustering_ok <- nrow(clustering) == 1L && clustering$nested256_within_training_pairs == 0L &&
  clustering$missing_within_training_biological_pairs_both_representations == 525350L &&
  !truth(clustering$reuse_mv05aj_for_clustering_authorized) &&
...
decision <- public[["mv05ak-prefreeze-decision-2026-08-11.csv"]]
decision_ok <- nrow(decision) == 1L && truth(decision$prediction_axes_pairable) &&
  truth(decision$retrieval_evaluation_identifiable) &&
  !truth(decision$clustering_evaluation_identifiable_from_mv05aj) &&
  !truth(decision$execution_authorized_now) &&
  !truth(decision$other_configurations_authorized)
add_check("prefreeze_decision", decision_ok,
          "retrieval_identifiable_execution_0_clustering_0_other_configurations_0")

validation <- add_check(done = TRUE)
if (!all(validation$passed)) {
  stop("MV5-AK independent validation failed: ",
       paste(validation$category[!validation$passed], collapse = ", "), call. = FALSE)
}
path <- file.path(output_dir, "mv05ak-independent-validation-2026-08-11.csv")
if (file.exists(path)) stop("Refusing to overwrite: ", path, call. = FALSE)
utils::write.csv(validation, path, row.names = FALSE, na = "")
message("MV5-AK independent validation passed: ", nrow(validation), " categories")
