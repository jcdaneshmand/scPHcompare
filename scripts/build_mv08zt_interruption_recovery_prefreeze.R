...
# Prospectively freeze no-retry adoption of the completed but unreceipted
# MV8-ZT engine-v2 child at production order 326. This builder is read-only
# with respect to both production roots.

args <- commandArgs(trailingOnly = TRUE)
if (length(args) != 4L) stop(paste(
  "usage: build_mv08zt_interruption_recovery_prefreeze.R",
  "<mv08zt-prefreeze> <private-root> <public-root> <output-dir>"
), call. = FALSE)
...
if (dir.exists(output_dir)) stop("refusing to overwrite recovery prefreeze", call. = FALSE)
...
verify_manifest <- function(root, name) {
  ...
  if (!all(file.exists(paths)) ||
    ...
    stop("MV8-ZT recovery prerequisite manifest drift", call. = FALSE)
  }
}
...
if (!all(file.exists(paths))) stop("MV8-ZT orphan evidence incomplete", call. = FALSE)
status <- read_csv(paths[["status"]])
distances <- read_csv(paths[["distance"]])
logs <- list.files(file.path(private_root, "logs"), full.names = TRUE)
expected_logs <- unlist(lapply(seq_len(orphan_n), function(index) {
...
prefix_private_ok <- all(vapply(seq_len(prefix_n), function(index) {
  value <- chunk_paths(queue[index, , drop = FALSE])
  all(file.exists(value)) &&
    sha_file(value[["distance"]]) == completed$distances_sha256[[index]] &&
    sha_file(value[["status"]]) == completed$status_sha256[[index]]
}, logical(1L)))

recovery_files <- c(
  "scripts/build_mv08zt_interruption_recovery_prefreeze.R",
  "scripts/recover_mv08zt_landscape_interruption.R"
)
if (!all(file.exists(recovery_files))) stop("recovery implementation absent", call. = FALSE)
implementation <- data.frame(
  contract_id = "mv08zt_r1_implementation_binding_v1",
  role = c("prefreeze_builder", "recovery_executor"),
  file = recovery_files,
...
snapshot <- data.frame(
  contract_id = "mv08zt_r1_stopped_snapshot_v1",
  execution_head = progress$execution_head,
  completed_prefix = prefix_n,
  unreceipted_complete_order = orphan_n,
  ...
  partial_files = length(partials), active_runner_processes = length(runner_lines),
  stringsAsFactors = FALSE
)
orphan <- data.frame(
  contract_id = "mv08zt_r1_orphan_binding_v1",
  ...
  upper_bound_not_measurement = TRUE,
  scientific_engine_version = 2L, workers = 1L, retries = 0L,
  stringsAsFactors = FALSE
)
policy <- data.frame(
  contract_id = "mv08zt_r1_recovery_policy_v1",
  ...
  scientific_contract_changed = FALSE, workers = 1L,
...
downstream <- c("comparison_jobs", "clustering_jobs", "fusion_jobs", "label_jobs",
                "outcome_jobs", "adoption_jobs", "manuscript_claim_jobs")
validation <- data.frame(
  check_id = c(
    "prefix_cardinality", "prefix_order", "prefix_ledger_completion_parity",
    "prefix_success", "prefix_engine_v2", "prefix_private_rehash",
    "progress_interrupted_running_state", "orphan_exact_next_order",
    "orphan_artifacts_complete", "orphan_status_complete", "orphan_engine_v2",
    "orphan_queue_identity", "orphan_pair_cardinality", "orphan_distance_hash",
    "orphan_expected_stderr", "logs_exact_through_orphan", "zero_partials",
    "runner_absent", "no_recomputation", "zero_retry", "one_worker",
    "elapsed_upper_bound", "rss_upper_bound", "aggregate_cap_feasible",
    "private_cap_feasible", "implementation_bound", "labels_outcomes_closed",
    "downstream_closed"
  ),
  passed = c(
    prefix_n == 325L,
    ...
    nrow(progress) == 1L && progress$state == "running" &&
      progress$completed_chunks == prefix_n && progress$completed_pairs == 80010L,
    orphan_n == 326L && row$group_order == 11L && row$chunk_order == 16L,
    ...
    !policy$landscape_recomputation, !policy$automatic_retry,
    policy$workers == 1L,
...
decision <- data.frame(
  ...
  next_gate = "resume_MV8_ZT_at_327_then_MV8_ZU_closure",
  outcome_label_state = "closed", biological_outcomes_computed = FALSE,
  stringsAsFactors = FALSE
)

dir.create(output_dir, recursive = TRUE)
atomic_csv(snapshot, file.path(output_dir, "mv08zt-r1-stopped-snapshot.csv"))
atomic_csv(orphan, file.path(output_dir, "mv08zt-r1-orphan-binding.csv"))
atomic_csv(policy, file.path(output_dir, "mv08zt-r1-recovery-policy.csv"))
atomic_csv(implementation, file.path(output_dir, "mv08zt-r1-implementation-bindings.csv"))
atomic_csv(validation, file.path(output_dir, "mv08zt-r1-validation.csv"))
atomic_csv(decision, file.path(output_dir, "mv08zt-r1-decision.csv"))
# Freeze the exact public prefix so an interrupted recovery executor is re-entrant.
atomic_csv(ledger, file.path(output_dir, "mv08zt-r1-prefix-ledger.csv"))
atomic_csv(completed, file.path(output_dir, "mv08zt-r1-prefix-completions.csv"))
atomic_csv(progress, file.path(output_dir, "mv08zt-r1-prefix-progress.csv"))
atomic_text(c(
...
), file.path(output_dir, "MV08ZT_ENGINE_V2_INTERRUPTION_RECOVERY_PREFREEZE.md"))
artifacts <- sort(setdiff(basename(list.files(output_dir, full.names = TRUE)),
                          "mv08zt-r1-artifact-manifest.csv"))
manifest <- data.frame(
  artifact = artifacts,
...
atomic_csv(manifest, file.path(output_dir, "mv08zt-r1-artifact-manifest.csv"))
cat("MV8-ZT recovery prefreeze passed 28/28; zero recovery executed\n")
