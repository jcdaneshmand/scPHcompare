#!/usr/bin/env python3
"""Immutable exact all-level landscape chunks for one MV5-D4 fold-seed group."""
...
CONTRACT_ID = "mv05d4_all_active_exact_landscape_distance_v1"
REQUEST_ID = "mv05d4_cell_landscape_pair_v1"
...
def atomic_csv(path, rows):
    if not rows:
        raise ValueError("Refusing to write an empty MV5-D4 artifact.")
...
def validate_requests(rows, manifest_sha, implementation_sha, max_pairs):
    ...
    if not rows or not required.issubset(rows[0]) or len(rows) > max_pairs:
        raise ValueError("MV5-D4 request chunk has an invalid schema or size.")
    if len({row["pair_request_id"] for row in rows}) != len(rows):
        raise ValueError("MV5-D4 request identifiers are duplicated.")
    for row in rows:
        if (
            ...
            raise ValueError("MV5-D4 request violates the frozen boundary.")
    if len(implementation_sha) != 64:
        raise ValueError("MV5-D4 implementation identity is invalid.")
...
def validate_resume(output_path, status_path, rows, manifest_sha,
    ...
    if exists[0] != exists[1]:
        raise RuntimeError("MV5-D4 chunk has a partial output/status pair.")
    ...
    if (
        ...
        raise RuntimeError("Existing MV5-D4 chunk failed identity validation.")
    return True
...
def main():
    ...
    for row in read_csv(args.intervals):
        ...
        if not math.isfinite(birth) or not math.isfinite(death) or death <= birth:
            raise ValueError("MV5-D4 interval input is not finite positive-persistence.")
    ...
    for chunk_id, rows in sorted(
        ...
        for row in sorted(rows, key=lambda item: int(item["chunk_offset"])):
            if time.perf_counter() - started > args.max_seconds:
                raise TimeoutError("MV5-D4 chunk exceeded its elapsed guard.")
            ...
            if first_key not in intervals or second_key not in intervals:
                raise ValueError("MV5-D4 request lacks finite interval input.")
            if interval_hashes[first_key] != row["query_diagram_sha256"] or \
               interval_hashes[second_key] != row["training_diagram_sha256"]:
                raise ValueError("MV5-D4 diagram identity differs from intervals.")
            ...
            output_rows.append({
                ...
                "clustering_jobs_executed": 0, "integration_jobs_executed": 0,
                "gene_view_jobs_executed": 0,
            })
        atomic_csv(output_path, output_rows)
        status_rows = [{
            "contract_id": "mv05d4_chunk_status_v1", "engine_id": ENGINE_ID,
        ...
        built += 1
    print(f"MV5-D4 group complete: built={built} reused={reused} total={len(grouped)}")
