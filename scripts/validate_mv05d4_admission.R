...
if (length(args) != 7L) {
  stop(
    "usage: validate_mv05d4_admission.R REQUESTS INTERVALS OUTPUT_DIR ",
...
read_many <- function(directory, pattern) {
  paths <- sort(list.files(directory, pattern = pattern, full.names = TRUE),
                method = "radix")
  if (!length(paths)) stop("No MV5-D4 files matched ", pattern, call. = FALSE)
...
if (nrow(observed) != nrow(requests) || anyNA(matched) ||
  ...
  stop("MV5-D4 admission output violates identity or numerical gates.",
       call. = FALSE)
}
...
oracle_rows <- lapply(seq_len(nrow(selected)), function(index) {
  ...
  data.frame(
    contract_id = "mv05d4_admission_r_oracle_v1",
...
if (any(!oracle$passed)) stop("MV5-D4 R oracle comparison failed.",
                              call. = FALSE)

completion <- data.frame(
  contract_id = "mv05d4_admission_completion_v1",
...
snapshot <- data.frame(
  contract_id = "mv05d4_admission_artifact_snapshot_v1",
...
message("Validated MV5-D4 admission group and two eligible R exact oracles.")
