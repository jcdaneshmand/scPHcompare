...
if (length(args) != 2L) {
  stop("usage: snapshot_mv05w_resume.R RESULT_ROOT OUTPUT_CSV", call. = FALSE)
}
...
if (length(files) != 11L || anyDuplicated(relative)) {
  stop("MV5-W smoke snapshot requires exactly 11 private files.",
       call. = FALSE)
}
result <- data.frame(
  contract_id = "mv05w_resume_snapshot_v1",
...
message("Snapshotted 11 immutable MV5-W smoke files.")
