...
for (package in c("processx", "ps", "digest")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for monitored MV5-U admission.",
         call. = FALSE)
  }
}
...
if (length(args) != 13L) {
  stop(paste(
    "usage: monitor_mv05u_admission.R EXECUTION_QUEUE PRIVATE_INVENTORY",
...
if (is.na(max_units) || max_units < 1L || max_units > 24L) {
  stop("MV5-U max-units guard is invalid.", call. = FALSE)
}
...
if (file.exists(resource_output)) {
  stop("MV5-U monitor refuses to overwrite a resource output.",
       call. = FALSE)
}
...
for (index in seq_len(nrow(queue))) {
  ...
  stem <- safe_name(unit$admission_unit_id)
  ...
  process <- processx::process$new(
    ...
    args = c(
      "--vanilla", "scripts/run_mv05u_admission_group.R", queue_path,
      inventory_path, d1_root, g_root, result_root,
      unit$admission_unit_id, expected_head, implementation_sha,
  ...
  while (process$is_alive()) {
    ...
    if (elapsed > 600 || stage_elapsed > 7200) {
      guard <- "timeout_guard"
      process$kill_tree()
    } else if (peak_rss > 4 * 1024^3) {
      guard <- "rss_guard"
      process$kill_tree()
    } else if (storage > 2 * 1024^3) {
      guard <- "storage_guard"
      process$kill_tree()
    }
  }
  ...
  rows[[index]] <- data.frame(
    ...
    stage_elapsed_cap_seconds = 7200,
...
if (any(!metrics$disposition %in% c("completed", "reused_validated")) ||
    ...
    any(metrics$landscape_pair_rows != 64L) ||
    any(metrics$energy_pair_rows != 32L) ||
    any(metrics$elapsed_seconds > 600) ||
    sum(metrics$elapsed_seconds) > 7200 ||
    any(metrics$peak_process_tree_rss_bytes > 4 * 1024^3) ||
    max(metrics$cumulative_private_bytes) > 2 * 1024^3) {
  stop("MV5-U admission completed outside its resource contract.",
       call. = FALSE)
}
message("Completed ", nrow(metrics), " MV5-U admission units (", run_mode, ").")
