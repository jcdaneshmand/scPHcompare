...
for (package in c("digest", "pkgload", "ps")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for MV5-AH prediction locking.", call. = FALSE)
  }
}
pkgload::load_all(".", quiet = TRUE, export_all = TRUE)
args <- commandArgs(trailingOnly = TRUE)
if (length(args) != 5L) {
  stop(paste("usage: build_mv05ah_prediction_lock.R MV05AF_RESULT_ROOT",
...
atomic_rds <- function(value, path) {
  ...
  if (file.exists(temporary)) stop("Stale MV5-AH temporary artifact.", call. = FALSE)
  saveRDS(value, temporary, version = 3, compress = "xz")
  if (!file.rename(temporary, path)) {
    unlink(temporary); stop("MV5-AH atomic rename failed.", call. = FALSE)
  }
}
atomic_csv <- function(value, path) {
  ...
  if (!file.rename(temporary, path)) {
    unlink(temporary); stop("MV5-AH atomic CSV rename failed.", call. = FALSE)
  }
}
...
if (!identical(head, expected_head)) {
  stop("MV5-AH prediction lock requires engine HEAD ", expected_head,
...
y_axis_path <- "docs/audits/mv05ag-prediction-axis-compatibility-2026-08-11.csv"
for (path in c(y_freeze_path, y_queue_path, y_axis_path)) {
  if (!file.exists(path)) stop("Missing accepted MV5-AG source: ", path, call. = FALSE)
}
y_freeze <- read_csv(y_freeze_path)
y_queue <- read_csv(y_queue_path)
axis <- read_csv(y_axis_path)
if (nrow(y_freeze) != 188L || nrow(y_queue) != 150L || nrow(axis) != 8L ||
  ...
  stop("Accepted MV5-AG prefreeze axes or unopened state drifted.", call. = FALSE)
}
...
source_checks <- lapply(seq_len(nrow(y_freeze)), function(index) {
  ...
  if (!file.exists(path)) stop("MV5-AH bound source missing: ", locator, call. = FALSE)
  actual <- file_sha(path)
  data.frame(
    contract_id = "mv05ah_source_validation_v1",
...
if (!all(source_checks$matched)) stop("MV5-AH frozen source hash drift.", call. = FALSE)
...
implementation_paths <- c(
  ...
  ranking_runner = "scripts/build_mv05ah_prediction_lock.R",
  ranking_validator = "scripts/validate_mv05ah_prediction_lock.R")
implementation <- data.frame(
  contract_id = "mv05ah_prediction_implementation_freeze_v1",
...
for (index in seq_len(nrow(y_queue))) {
  ...
  if (xor(file.exists(artifact_path), file.exists(status_path))) {
    stop("MV5-AH partial ranking artifact/status pair: ", stem, call. = FALSE)
  }
  reused <- FALSE
  if (both) {
    ...
    if (!identical(status$evaluation_unit_id, unit$evaluation_unit_id[[1L]]) ||
        !identical(status$mv05ag_source_freeze_sha256, y_source_id) ||
      ...
      stop("MV5-AH immutable ranking resume failed: ", stem, call. = FALSE)
    }
    reused <- TRUE
  } else {
    ...
    ranking <- mv05ah_rank_group_v1(methods, unit)
    elapsed <- proc.time()[["elapsed"]] - started
    if (elapsed > 300) stop("MV5-AH ranking group elapsed cap exceeded.", call. = FALSE)
    atomic_rds(ranking, artifact_path)
    status <- list(
      contract_id = "mv05ah_ranking_unit_status_v1",
      ...
      mv05ag_source_freeze_sha256 = y_source_id,
  ...
  completion[[index]] <- data.frame(
    contract_id = "mv05ah_ranking_unit_completion_v1",
...
if (nrow(rankings) != 282800L || anyDuplicated(rankings$ranking_id) ||
  ...
  stop("MV5-AH prediction ranking completion/resource contract failed.", call. = FALSE)
}

ranking_path <- file.path(audit_dir, "mv05ah-nested192-rankings-2026-08-11.csv.gz")
...
prediction_lock <- data.frame(
  contract_id = "mv05ah_prediction_lock_v1",
  accepted_mv05ag_commit = "6f36741",
  ...
  stringsAsFactors = FALSE)
resource <- data.frame(
  contract_id = "mv05ah_prediction_resource_summary_v1",
  ...
  stringsAsFactors = FALSE)
decision <- data.frame(
  contract_id = "mv05ah_prediction_lock_decision_v1",
  all_188_sources_valid = all(source_checks$matched),
  all_150_groups_complete = all(completion$state == "completed"),
  ranking_rows = nrow(rankings), canonical_ties_valid = TRUE,
  label_open_authorized = TRUE,
  authorization_scope = "mv05ah_nested192_tissue_retrieval_robustness_only",
  clustering_authorized = FALSE, nested256_authorized = FALSE,
  labels_opened = FALSE, outcomes_computed = FALSE,
  stringsAsFactors = FALSE)

atomic_csv(source_checks, file.path(audit_dir,
  "mv05ah-prediction-source-validation-2026-08-11.csv"))
atomic_csv(implementation, file.path(audit_dir,
  "mv05ah-prediction-implementation-freeze-2026-08-11.csv"))
atomic_csv(completion, file.path(audit_dir,
  "mv05ah-ranking-unit-completion-2026-08-11.csv"))
atomic_csv(prediction_lock, file.path(audit_dir,
  "mv05ah-prediction-lock-2026-08-11.csv"))
atomic_csv(resource, file.path(audit_dir,
  "mv05ah-prediction-resource-summary-2026-08-11.csv"))
atomic_csv(decision, file.path(audit_dir,
  "mv05ah-prediction-lock-decision-2026-08-11.csv"))
message("MV5-AH prediction lock passed: groups=150 rankings=282800 labels=0 outcomes=0")
