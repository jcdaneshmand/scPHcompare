...
if (length(args) != 6L) {
  stop(paste("usage: validate_mv05ad_prediction_lock.R MV05AB_RESULT_ROOT",
...
if (nrow(rankings) != 282800L || nrow(completion) != 150L || nrow(lock) != 1L ||
    ...
    any(completion$state != "completed")) {
  stop("MV5-AD prediction-lock public identity failed.", call. = FALSE)
}
...
for (index in seq_len(nrow(completion))) {
  ...
  stem <- sub("^mv05ac_eval_v1:", "", row$evaluation_unit_id[[1L]])
...
if (checked != 282800L || !all(private_hashes) || !all(status_hashes)) {
  stop("MV5-AD private ranking identity reconstruction failed.", call. = FALSE)
}
validation <- data.frame(
  contract_id = "mv05ad_prediction_independent_validation_v1",
  validation_id = c(
...
message("MV5-AD independent prediction validation passed: rows=282800 groups=150 labels=0 outcomes=0")
