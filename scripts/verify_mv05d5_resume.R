...
if (length(args) != 4L) {
  stop(
    "usage: verify_mv05d5_resume.R BUNDLE_DIR AUDIT_DIR SNAPSHOT_CSV ",
...
if (length(bundle_paths) != 75L || length(audit_paths) != 75L) {
  stop("MV5-D5 resume verification requires 75 bundles and 75 audits.",
       call. = FALSE)
}
current <- data.frame(
  relative_file = c(
...
if (!file.exists(snapshot_path)) {
  ...
  message("Captured the pre-resume MV5-D5 file snapshot.")
} else {
  ...
  result <- data.frame(
    contract_id = "mv05d5_resume_validation_v1",
  ...
  if (!unchanged) stop("MV5-D5 resume changed a completed artifact.",
                       call. = FALSE)
  write_provenance_csv(result, summary_path)
  message("MV5-D5 immutable resume validation passed.")
}
