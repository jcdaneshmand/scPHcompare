...
if (length(args) != 5L) {
  stop(
    "usage: assemble_mv05d5_public_artifacts.R BUNDLE_DIR AUDIT_DIR ",
...
if (nrow(metrics) != 75L || any(metrics$disposition != "completed")) {
  stop("MV5-D5 public assembly requires 75 accepted production groups.",
       call. = FALSE)
}
...
for (index in seq_len(nrow(metrics))) {
  ...
  completion_rows[[index]] <- bundle$payload$failures
  index_rows[[index]] <- data.frame(
    contract_id = "mv05d5_public_group_index_v1",
    ...
    mv05d4_group_sha256 = bundle$identity$mv05d4_group_sha256,
    implementation_sha256 = bundle$identity$implementation_sha256,
    outcome_label_state = "closed", biological_outcomes_computed = FALSE,
    stringsAsFactors = FALSE
  )
  scale_rows[[index]] <- data.frame(
    contract_id = "mv05d5_component_scale_disposition_v1",
    group_id = bundle$identity$group_id, fold_id = bundle$identity$fold_id,
    fit_scope_id = bundle$identity$fit_scope_id,
    seed = bundle$identity$seed,
    method_id = c(
...
registry$contract_id <- "mv05d5_method_registry_v1"
...
if (nrow(pairs) != 176750L || anyDuplicated(pairs$pair_id) ||
  ...
  stop("MV5-D5 public assembly violates count or label invariants.",
       call. = FALSE)
}
pairs <- pairs[order(
...
write_gzip <- function(data, name) {
  ...
  if (file.exists(csv) || file.exists(paste0(csv, ".gz"))) {
    stop("MV5-D5 build directory contains a stale compressed artifact.",
         call. = FALSE)
  }
  ...
  if (!identical(status, 0L) || !file.exists(gz) || !file.rename(gz, final)) {
    stop("Failed to deterministically compress a public MV5-D5 artifact.",
         call. = FALSE)
  }
  final
}
...
ranking_path <- write_gzip(
  pairs, "mv05d5-cell-retrieval-rankings-2026-08-08.csv.gz"
)
completion_path <- write_plain(
  completion, "mv05d5-method-completion-2026-08-08.csv"
)
index_path <- write_plain(
  group_index, "mv05d5-group-bundle-index-2026-08-08.csv"
)
registry_path <- write_plain(
  registry, "mv05d5-method-registry-2026-08-08.csv"
)
...
summary <- data.frame(
  contract_id = "mv05d5_public_assembly_summary_v1",
...
write_plain(summary, "mv05d5-public-assembly-summary-2026-08-08.csv")
message("Assembled deterministic public MV5-D5 retrieval artifacts.")
