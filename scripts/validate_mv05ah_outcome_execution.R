...
if (length(args) != 4L) {
  stop(paste("usage: validate_mv05ah_outcome_execution.R EXTERNAL_METADATA_PATH",
...
rankings <- readc("docs/audits/mv05ah-nested192-rankings-2026-08-11.csv.gz")
nested192_public <- readc(paths("mv05ah-nested192-query-method-outcomes-2026-08-11.csv"))
paired_public <- readc(paths("mv05ah-paired-query-method-endpoints-2026-08-11.csv"))
query_public <- readc(paths("mv05ah-query-estimands-2026-08-11.csv"))
sample_public <- readc(paths("mv05ah-sample-estimands-2026-08-11.csv"))
tissue_public <- readc(paths("mv05ah-tissue-estimands-2026-08-11.csv"))
macro_public <- readc(paths("mv05ah-macro-estimands-2026-08-11.csv"))
interval_public <- readc(paths("mv05ah-estimand-intervals-2026-08-11.csv"))
primary_public <- readc(paths("mv05ah-primary-contrasts-2026-08-11.csv"))
bootstrap_public <- readc(paths("mv05ah-bootstrap-audit-2026-08-11.csv"))
random_public <- readc(paths("mv05ah-randomization-audit-2026-08-11.csv"))
completion <- readc(paths("mv05ah-outcome-unit-completion-2026-08-11.csv"))
registry <- readc("docs/audits/mv05ag-estimand-registry-2026-08-11.csv")
method_map <- readc("docs/audits/mv05ag-method-map-2026-08-11.csv")
...
for (index in groups) {
  ...
  family <- method_map$family_id[
    method_map$representation == part$representation[[1L]] &
      method_map$nested192_method_id == part$method_id[[1L]]]
...
nested192 <- do.call(rbind, endpoint_rows)
...
nested192_public <- nested192_public[match(key(nested192), key(nested192_public)), ]
if (rank_rows != 282800L || nrow(nested192) != 3600L || anyNA(nested192_public$method_id) ||
    !close_num(nested192$reciprocal_rank, nested192_public$reciprocal_rank) ||
    !identical(as.logical(nested192$one_nn_correct), as.logical(nested192_public$one_nn_correct)) ||
    !identical(as.integer(nested192$first_same_tissue_rank),
               as.integer(nested192_public$first_same_tissue_rank))) {
...
baseline <- baseline[match(pk(nested192), pk(baseline)), ]
direct_rr <- nested192$reciprocal_rank - baseline$reciprocal_rank
direct_nn <- as.numeric(nested192$one_nn_correct) - as.numeric(baseline$one_nn_correct)
paired_public <- paired_public[match(pk(nested192), pk(paired_public)), ]
...
direct <- do.call(rbind, lapply(seq_len(nrow(nested192)), function(i) {
  rbind(
    data.frame(nested192[i, pair_fields], endpoint_id = "cross_study_tissue_mrr_v1",
               estimand_type = "direct_nested192_cells_minus_384_cells", value = direct_rr[[i]]),
    data.frame(nested192[i, pair_fields],
               endpoint_id = "cross_study_tissue_1nn_balanced_accuracy_v1",
               estimand_type = "direct_nested192_cells_minus_384_cells", value = direct_nn[[i]]))
}))
did <- do.call(rbind, lapply(c("h0", "h1"), function(family) {
  top <- nested192$family_id == family; energy <- nested192$family_id == "energy"
  did_fields <- setdiff(pair_fields, "family_id")
  ek <- key(nested192[energy, ], did_fields)
  energy_index <- which(energy)[match(key(nested192[top, ], did_fields), ek)]
  top_index <- which(top)
  rbind(
    data.frame(nested192[top_index, pair_fields], endpoint_id = "cross_study_tissue_mrr_v1",
      estimand_type = "topology_increment_nested192_minus_384_cells_difference_in_differences",
      value = direct_rr[top_index] - direct_rr[energy_index]),
    data.frame(nested192[top_index, pair_fields],
      endpoint_id = "cross_study_tissue_1nn_balanced_accuracy_v1",
      estimand_type = "topology_increment_nested192_minus_384_cells_difference_in_differences",
...
primary_registry <- ordered_registry[
  ordered_registry$estimand_role == "confirmatory_nested192_sensitivity", ]
...
validation <- data.frame(
  contract_id = "mv05ah_outcome_independent_validation_v1",
  validation_id = c(
...
message("MV5-AH independent outcome validation passed: 15 categories, 24 estimands, 4 tests")
