...
for (package in c("digest", "pkgload", "ps")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for MV5-AL outcome execution.", call. = FALSE)
  }
}
pkgload::load_all(".", quiet = TRUE, export_all = TRUE)
args <- commandArgs(trailingOnly = TRUE)
if (length(args) != 5L) {
  stop(paste("usage: run_mv05al_outcome_execution.R EXTERNAL_METADATA_PATH",
...
atomic_rds <- function(value, path) {
  ...
  if (file.exists(temporary)) stop("Stale MV5-AL outcome temporary artifact.", call. = FALSE)
  saveRDS(value, temporary, version = 3, compress = "xz")
  if (!file.rename(temporary, path)) {
    unlink(temporary); stop("MV5-AL outcome atomic rename failed.", call. = FALSE)
  }
}
atomic_csv <- function(value, path) {
  ...
  if (!file.rename(temporary, path)) {
    unlink(temporary); stop("MV5-AL outcome atomic CSV rename failed.", call. = FALSE)
  }
}
...
if (!identical(head, expected_head)) {
  stop("MV5-AL outcomes require committed prediction lock ", expected_head,
...
prediction_lock_path <- "docs/audits/mv05al-prediction-lock-2026-08-11.csv"
prediction_completion_path <- "docs/audits/mv05al-ranking-unit-completion-2026-08-11.csv"
prediction_validation_path <-
  "docs/audits/mv05al-prediction-independent-validation-2026-08-11.csv"
...
if (any(!file.exists(required_paths))) stop("MV5-AL outcome source missing.", call. = FALSE)
...
if (nrow(y_freeze) != 196L || nrow(queue) != 150L ||
    ...
    prediction_lock$accepted_mv05ak_commit !=
  ...
  stop("MV5-AL durable prediction lock failed before label access.", call. = FALSE)
}

# Revalidate every MV5-AK source before parsing any label or accepted endpoint.
source_checks <- lapply(seq_len(nrow(y_freeze)), function(index) {
  ...
  actual <- sha(path)
  data.frame(
    contract_id = "mv05al_outcome_source_validation_v1",
...
if (!all(source_checks$matched)) stop("MV5-AL source drift before labels.", call. = FALSE)
...
if (length(expected_sct_sha) != 1L || length(expected_integrated_sha) != 1L ||
  ...
  stop("MV5-AL accepted baseline endpoint hash drifted.", call. = FALSE)
}
...
implementation_paths <- c(
  ...
  outcome_runner = "scripts/run_mv05al_outcome_execution.R",
  outcome_validator = "scripts/validate_mv05al_outcome_execution.R",
  ...
  estimand_registry = "docs/audits/mv05ak-estimand-registry-2026-08-11.csv")
implementation <- data.frame(
  contract_id = "mv05al_outcome_implementation_freeze_v1",
...
# Only now, after the durable lock and 196 fresh source checks, parse sample,
...
if (!all(required_labels %in% names(header))) stop("MV5-AL label schema drifted.", call. = FALSE)
...
if (nrow(labels) != 90L || length(unique(labels$study)) != 15L ||
  ...
  stop("MV5-AL exact tissue-label join design drifted.", call. = FALSE)
}
label_provenance <- data.frame(
  contract_id = "mv05al_label_source_provenance_v1",
  ...
  label_open_boundary =
    paste0("after_committed_mv05al_prediction_lock_", substr(expected_head, 1L, 7L)),
...
if (nrow(rankings) != 282800L || any(rankings$labels_opened) ||
    ...
    !setequal(unique(c(rankings$query_sample_id, rankings$training_sample_id)),
              labels$sample_id)) {
  stop("MV5-AL ranking content drifted after label open.", call. = FALSE)
}
...
for (index in seq_len(nrow(queue))) {
  ...
  if (xor(file.exists(artifact), file.exists(status_path))) {
    stop("MV5-AL partial outcome artifact/status pair: ", stem, call. = FALSE)
  }
  reused <- FALSE
  if (file.exists(artifact)) {
    ...
    if (!identical(status$evaluation_unit_id, unit$evaluation_unit_id[[1L]]) ||
      ...
      stop("MV5-AL immutable outcome resume failed: ", stem, call. = FALSE)
    }
    reused <- TRUE
  } else {
    ...
    outcome <- mv05al_evaluate_group_v1(group_rankings, labels)
    elapsed <- proc.time()[["elapsed"]] - started
    if (elapsed > 300) stop("MV5-AL outcome group elapsed cap exceeded.", call. = FALSE)
    atomic_rds(outcome, artifact)
    status <- list(
      contract_id = "mv05al_outcome_unit_status_v1",
  ...
  completion[[index]] <- data.frame(
    contract_id = "mv05al_outcome_unit_completion_v1",
...
completion <- do.call(rbind, completion); nested256 <- do.call(rbind, results)
nested256 <- nested256[order(nested256$representation, nested256$family_id, nested256$fold_id,
                    nested256$seed, nested256$query_sample_id, method = "radix"), ]
rownames(nested256) <- NULL
if (nrow(completion) != 150L || nrow(nested256) != 3600L ||
    !all(completion$state == "completed") ||
    any(nested256$endpoint_status != "estimable")) {
  stop("MV5-AL outcome group completion failed.", call. = FALSE)
}

# Accepted outcome files are opened only after every NESTED256 outcome unit is
...
paired <- mv05al_pair_baseline_v1(nested256, baseline_sct, baseline_integrated)
query_estimands <- mv05al_query_estimands_v1(paired)
summaries <- mv05al_summarize_estimands_v1(query_estimands)
inference <- mv05al_block_inference_v1(summaries$sample)

long_endpoints <- rbind(
  data.frame(
    contract_id = "mv05al_nested256_query_endpoint_v1",
    nested256[c("fold_id", "held_out_study", "seed", "representation", "family_id",
           "method_id", "query_sample_id", "query_tissue", "training_samples",
           "training_studies", "endpoint_status")],
    endpoint_id = .mv05al_endpoints[[1L]], value = nested256$reciprocal_rank,
    stringsAsFactors = FALSE),
  data.frame(
    contract_id = "mv05al_nested256_query_endpoint_v1",
    nested256[c("fold_id", "held_out_study", "seed", "representation", "family_id",
           "method_id", "query_sample_id", "query_tissue", "training_samples",
           "training_studies", "endpoint_status")],
    endpoint_id = .mv05al_endpoints[[2L]], value = as.numeric(nested256$one_nn_correct),
    stringsAsFactors = FALSE))
long_endpoints <- long_endpoints[order(
  match(long_endpoints$endpoint_id, .mv05al_endpoints),
...
if (nrow(long_endpoints) != 7200L) stop("MV5-AL long endpoint completion failed.", call. = FALSE)

production <- data.frame(
  contract_id = "mv05al_production_summary_v1",
  prediction_groups = 150L, ranking_rows = 282800L,
  outcome_groups = nrow(completion), query_method_rows = nrow(nested256),
  ...
  clustering_executed = FALSE, other_configuration_executed = FALSE,
  refit_or_rerank_executed = FALSE, method_selection_executed = FALSE,
  equivalence_claim_authorized = FALSE,
  outcomes_computed = TRUE, evaluation_executed = TRUE,
  stringsAsFactors = FALSE)
resource <- data.frame(
  contract_id = "mv05al_outcome_resource_summary_v1",
  outcome_groups = 150L,
...
if (resource$aggregate_group_seconds > 7200 ||
  ...
  stop("MV5-AL outcome resource cap failed.", call. = FALSE)
}

public <- list(
  "mv05al-outcome-source-validation-2026-08-11.csv" = source_checks,
  "mv05al-outcome-implementation-freeze-2026-08-11.csv" = implementation,
  "mv05al-label-source-provenance-2026-08-11.csv" = label_provenance,
  "mv05al-outcome-unit-completion-2026-08-11.csv" = completion,
  "mv05al-nested256-query-method-outcomes-2026-08-11.csv" = nested256,
  "mv05al-nested256-query-endpoints-2026-08-11.csv" = long_endpoints,
  "mv05al-paired-query-method-endpoints-2026-08-11.csv" = paired,
  "mv05al-query-estimands-2026-08-11.csv" = query_estimands,
  "mv05al-sample-estimands-2026-08-11.csv" = summaries$sample,
  "mv05al-tissue-estimands-2026-08-11.csv" = summaries$tissue,
  "mv05al-macro-estimands-2026-08-11.csv" = summaries$macro,
  "mv05al-estimand-intervals-2026-08-11.csv" = inference$intervals,
  "mv05al-primary-contrasts-2026-08-11.csv" = inference$primary,
  "mv05al-bootstrap-audit-2026-08-11.csv" = inference$bootstrap_audit,
  "mv05al-randomization-audit-2026-08-11.csv" = inference$randomization_audit,
  "mv05al-production-summary-2026-08-11.csv" = production,
  "mv05al-outcome-resource-summary-2026-08-11.csv" = resource)
...
if (file.exists(inference_path)) {
  if (!identical(readRDS(inference_path), inference_payload)) {
    stop("MV5-AL immutable inference-matrix resume failed.", call. = FALSE)
  }
} else {
  atomic_rds(inference_payload, inference_path)
}
message("MV5-AL outcomes passed: groups=150 query_methods=3600 endpoints=7200 estimands=24 primary_tests=4")
