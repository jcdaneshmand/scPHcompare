...
if (!requireNamespace("digest", quietly = TRUE)) {
  stop("digest is required for MV5-AC validation.", call. = FALSE)
}

args <- commandArgs(trailingOnly = TRUE)
if (length(args) != 4L) {
  stop(paste(
    "usage: validate_mv05ac_cosine_outcome_prefreeze.R",
    "MV05AB_RESULT_ROOT EXTERNAL_METADATA_PATH AUDIT_DIR OUTPUT_DIR"),
    call. = FALSE)
}
  ...
  function(category = NULL, passed = NULL, observed = NULL, done = FALSE) {
    if (!done) {
      rows[[length(rows) + 1L]] <<- data.frame(
        contract_id = "mv05ac_independent_validation_v1",
        category = category, passed = isTRUE(passed), observed = observed,
        label_values_read = FALSE, cosine_rankings_computed = FALSE,
...
expected_files <- c(
  "mv05ac-source-freeze-2026-08-11.csv",
  "mv05ac-method-map-2026-08-11.csv",
  "mv05ac-endpoint-registry-2026-08-11.csv",
  "mv05ac-estimand-registry-2026-08-11.csv",
  "mv05ac-prediction-axis-compatibility-2026-08-11.csv",
  "mv05ac-label-join-prefreeze-2026-08-11.csv",
  "mv05ac-prediction-lock-contract-2026-08-11.csv",
  "mv05ac-evaluation-queue-2026-08-11.csv",
  "mv05ac-clustering-compatibility-disposition-2026-08-11.csv",
  "mv05ac-validation-plan-2026-08-11.csv",
  "mv05ac-abort-rules-2026-08-11.csv",
  "mv05ac-resource-envelope-2026-08-11.csv",
  "mv05ac-reporting-contract-2026-08-11.csv",
  "mv05ac-prefreeze-decision-2026-08-11.csv")
paths <- file.path(audit_dir, expected_files)
if (any(!file.exists(paths))) stop("MV5-AC public assembly is incomplete.", call. = FALSE)
public <- setNames(lapply(paths, read_csv), expected_files)
add_check("public_assembly", length(public) == 14L,
          paste0(length(public), "_of_14_ledgers"))
...
add_check("public_outcome_closure", safe_public,
          "labels_0_cosine_rankings_0_outcomes_0")

source <- public[["mv05ac-source-freeze-2026-08-11.csv"]]
...
queue <- public[["mv05ac-evaluation-queue-2026-08-11.csv"]]
queue_ok <- nrow(queue) == 150L && !anyDuplicated(queue$evaluation_unit_id) &&
  ...
  all(queue$configuration_id == "cells384_pc30_cosine_chord_v1") &&
...
method_map <- public[["mv05ac-method-map-2026-08-11.csv"]]
methods_expected <- c(
...
for (index in seq_len(nrow(queue))) {
  ...
  if (!all(file.exists(c(manifest_path, method_path, identity_path, status_path)))) {
    stop("Missing private MV5-AB group: ", unit$robustness_group_id, call. = FALSE)
  }
  identity <- read_csv(identity_path); status <- read_csv(status_path)
  rows <- read_csv(method_path)
  manifest_source <- grepl("^cosine_private_group_manifest_", source$source_id) &
...
for (representation in names(baseline_frames)) {
  ...
  mapped <- map$cosine_method_id[match(frame$method_id, map$baseline_method_id)]
...
axis_public <- public[["mv05ac-prediction-axis-compatibility-2026-08-11.csv"]]
axis_ok <- axis_ok && nrow(axis_public) == 8L &&
  all(truth(axis_public$fold_seed_query_reference_axis_exact)) &&
  all(axis_public$missing_cosine_rows == 0L) &&
  all(axis_public$excess_cosine_rows == 0L)
...
label_join <- public[["mv05ac-label-join-prefreeze-2026-08-11.csv"]]
...
endpoints <- public[["mv05ac-endpoint-registry-2026-08-11.csv"]]
estimands <- public[["mv05ac-estimand-registry-2026-08-11.csv"]]
registry_ok <- nrow(method_map) == 8L && nrow(endpoints) == 2L &&
  nrow(estimands) == 24L && !anyDuplicated(estimands$estimand_id) &&
  sum(estimands$estimand_role == "confirmatory_cosine_sensitivity") == 4L &&
...
lock <- public[["mv05ac-prediction-lock-contract-2026-08-11.csv"]]
...
clustering <- public[["mv05ac-clustering-compatibility-disposition-2026-08-11.csv"]]
clustering_ok <- nrow(clustering) == 1L && clustering$cosine_within_training_pairs == 0L &&
  clustering$missing_within_training_biological_pairs_both_representations == 525350L &&
  !truth(clustering$reuse_mv05ab_for_clustering_authorized) &&
...
decision <- public[["mv05ac-prefreeze-decision-2026-08-11.csv"]]
decision_ok <- nrow(decision) == 1L && truth(decision$prediction_axes_pairable) &&
  truth(decision$retrieval_evaluation_identifiable) &&
  !truth(decision$clustering_evaluation_identifiable_from_mv05ab) &&
...
if (!all(validation$passed)) {
  stop("MV5-AC independent validation failed: ",
       paste(validation$category[!validation$passed], collapse = ", "), call. = FALSE)
}
path <- file.path(output_dir, "mv05ac-independent-validation-2026-08-11.csv")
if (file.exists(path)) stop("Refusing to overwrite: ", path, call. = FALSE)
utils::write.csv(validation, path, row.names = FALSE, na = "")
message("MV5-AC independent validation passed: ", nrow(validation), " categories")
