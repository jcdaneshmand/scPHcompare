...
if (length(args) != 5L) {
  stop(
    "usage: verify_mv05g_deterministic_repeat.R RESULT_A RESULT_B AUDIT_A ",
    "AUDIT_B OUTPUT_CSV", call. = FALSE
  )
}
...
result <- data.frame(
  contract_id = "mv05g_deterministic_repeat_v1",
  group_id = audit_a$group_id, result_a_sha256 = file_sha(paths[[1L]]),
  ...
  audit_scientific_fields_identical = identical(
    audit_a[setdiff(names(audit_a), exclude)],
    audit_b[setdiff(names(audit_b), exclude)]
  ),
...
if (!all(pass)) stop("MV5-G deterministic repeat failed.", call. = FALSE)
dir.create(dirname(args[[5L]]), recursive = TRUE, showWarnings = FALSE)
source("R/provenance_utils.R")
write_provenance_csv(result, args[[5L]])
message("MV5-G full-group repeat is byte identical.")
