...
for (package in c("digest", "pkgload", "ps")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for MV5-AD outcome execution.", call. = FALSE)
  }
}
pkgload::load_all(".", quiet = TRUE, export_all = TRUE)
args <- commandArgs(trailingOnly = TRUE)
if (length(args) != 4L) {
  stop(paste("usage: run_mv05ad_outcome_execution.R EXTERNAL_METADATA_PATH",
...
atomic_rds <- function(value, path) {
  ...
  if (file.exists(temporary)) stop("Stale MV5-AD outcome temporary artifact.", call. = FALSE)
  saveRDS(value, temporary, version = 3, compress = "xz")
  if (!file.rename(temporary, path)) {
    unlink(temporary); stop("MV5-AD outcome atomic rename failed.", call. = FALSE)
  }
}
atomic_csv <- function(value, path) {
  ...
  if (!file.rename(temporary, path)) {
    unlink(temporary); stop("MV5-AD outcome atomic CSV rename failed.", call. = FALSE)
  }
}

expected_head <- "2e2f9f3"
head <- trimws(system2("git", c("rev-parse", "--short=7", "HEAD"), stdout = TRUE))
if (!identical(head, expected_head)) {
  stop("MV5-AD outcomes require committed prediction lock ", expected_head,
...
prediction_lock_path <- "docs/audits/mv05ad-prediction-lock-2026-08-11.csv"
prediction_completion_path <- "docs/audits/mv05ad-ranking-unit-completion-2026-08-11.csv"
prediction_validation_path <-
  "docs/audits/mv05ad-prediction-independent-validation-2026-08-11.csv"
...
if (any(!file.exists(required_paths))) stop("MV5-AD outcome source missing.", call. = FALSE)
...
if (nrow(y_freeze) != 187L || nrow(queue) != 150L ||
  ...
  stop("MV5-AD durable prediction lock failed before label access.", call. = FALSE)
}

# Revalidate every MV5-AC source before parsing any label or accepted endpoint.
source_checks <- lapply(seq_len(nrow(y_freeze)), function(index) {
  ...
  actual <- sha(path)
  data.frame(
    contract_id = "mv05ad_outcome_source_validation_v1",
...
if (!all(source_checks$matched)) stop("MV5-AD source drift before labels.", call. = FALSE)
...
if (length(expected_sct_sha) != 1L || length(expected_integrated_sha) != 1L ||
  ...
  stop("MV5-AD accepted baseline endpoint hash drifted.", call. = FALSE)
}
...
implementation_paths <- c(
  ...
  outcome_runner = "scripts/run_mv05ad_outcome_execution.R",
  outcome_validator = "scripts/validate_mv05ad_outcome_execution.R",
  prediction_lock = prediction_lock_path,
  prediction_validation = prediction_validation_path,
  estimand_registry = "docs/audits/mv05ac-estimand-registry-2026-08-11.csv")
implementation <- data.frame(
  contract_id = "mv05ad_outcome_implementation_freeze_v1",
...
# Only now, after the durable lock and 187 fresh source checks, parse sample,
...
if (!all(required_labels %in% names(header))) stop("MV5-AD label schema drifted.", call. = FALSE)
...
if (nrow(labels) != 90L || length(unique(labels$study)) != 15L ||
  ...
  stop("MV5-AD exact tissue-label join design drifted.", call. = FALSE)
}
label_provenance <- data.frame(
  contract_id = "mv05ad_label_source_provenance_v1",
  ...
  label_open_boundary =
    "after_committed_mv05ad_prediction_lock_2e2f9f3",
...
if (nrow(rankings) != 282800L || any(rankings$labels_opened) ||
    ...
    !setequal(unique(c(rankings$query_sample_id, rankings$training_sample_id)),
              labels$sample_id)) {
  stop("MV5-AD ranking content drifted after label open.", call. = FALSE)
}
...
for (index in seq_len(nrow(queue))) {
  ...
  if (xor(file.exists(artifact), file.exists(status_path))) {
    stop("MV5-AD partial outcome artifact/status pair: ", stem, call. = FALSE)
  }
  reused <- FALSE
  if (file.exists(artifact)) {
    ...
    if (!identical(status$evaluation_unit_id, unit$evaluation_unit_id[[1L]]) ||
      ...
      stop("MV5-AD immutable outcome resume failed: ", stem, call. = FALSE)
    }
    reused <- TRUE
  } else {
    ...
    outcome <- mv05ad_evaluate_group_v1(group_rankings, labels)
    elapsed <- proc.time()[["elapsed"]] - started
    if (elapsed > 300) stop("MV5-AD outcome group elapsed cap exceeded.", call. = FALSE)
    atomic_rds(outcome, artifact)
    status <- list(
      contract_id = "mv05ad_outcome_unit_status_v1",
  ...
  completion[[index]] <- data.frame(
    contract_id = "mv05ad_outcome_unit_completion_v1",
...
completion <- do.call(rbind, completion); cosine <- do.call(rbind, results)
cosine <- cosine[order(cosine$representation, cosine$family_id, cosine$fold_id,
                    cosine$seed, cosine$query_sample_id, method = "radix"), ]
rownames(cosine) <- NULL
if (nrow(completion) != 150L || nrow(cosine) != 3600L ||
    !all(completion$state == "completed") ||
    any(cosine$endpoint_status != "estimable")) {
  stop("MV5-AD outcome group completion failed.", call. = FALSE)
}

# Accepted outcome files are opened only after every COSINE outcome unit is
...
paired <- mv05ad_pair_baseline_v1(cosine, baseline_sct, baseline_integrated)
query_estimands <- mv05ad_query_estimands_v1(paired)
summaries <- mv05ad_summarize_estimands_v1(query_estimands)
inference <- mv05ad_block_inference_v1(summaries$sample)

long_endpoints <- rbind(
  data.frame(
    contract_id = "mv05ad_cosine_query_endpoint_v1",
    cosine[c("fold_id", "held_out_study", "seed", "representation", "family_id",
           "method_id", "query_sample_id", "query_tissue", "training_samples",
           "training_studies", "endpoint_status")],
    endpoint_id = .mv05ad_endpoints[[1L]], value = cosine$reciprocal_rank,
    stringsAsFactors = FALSE),
  data.frame(
    contract_id = "mv05ad_cosine_query_endpoint_v1",
    cosine[c("fold_id", "held_out_study", "seed", "representation", "family_id",
           "method_id", "query_sample_id", "query_tissue", "training_samples",
           "training_studies", "endpoint_status")],
    endpoint_id = .mv05ad_endpoints[[2L]], value = as.numeric(cosine$one_nn_correct),
    stringsAsFactors = FALSE))
long_endpoints <- long_endpoints[order(
  match(long_endpoints$endpoint_id, .mv05ad_endpoints),
...
if (nrow(long_endpoints) != 7200L) stop("MV5-AD long endpoint completion failed.", call. = FALSE)

production <- data.frame(
  contract_id = "mv05ad_production_summary_v1",
  prediction_groups = 150L, ranking_rows = 282800L,
  outcome_groups = nrow(completion), query_method_rows = nrow(cosine),
  ...
  stringsAsFactors = FALSE)
resource <- data.frame(
  contract_id = "mv05ad_outcome_resource_summary_v1",
  outcome_groups = 150L,
...
if (resource$aggregate_group_seconds > 7200 ||
  ...
  stop("MV5-AD outcome resource cap failed.", call. = FALSE)
}

public <- list(
  "mv05ad-outcome-source-validation-2026-08-11.csv" = source_checks,
  "mv05ad-outcome-implementation-freeze-2026-08-11.csv" = implementation,
  "mv05ad-label-source-provenance-2026-08-11.csv" = label_provenance,
  "mv05ad-outcome-unit-completion-2026-08-11.csv" = completion,
  "mv05ad-cosine-query-method-outcomes-2026-08-11.csv" = cosine,
  "mv05ad-cosine-query-endpoints-2026-08-11.csv" = long_endpoints,
  "mv05ad-paired-query-method-endpoints-2026-08-11.csv" = paired,
  "mv05ad-query-estimands-2026-08-11.csv" = query_estimands,
  "mv05ad-sample-estimands-2026-08-11.csv" = summaries$sample,
  "mv05ad-tissue-estimands-2026-08-11.csv" = summaries$tissue,
  "mv05ad-macro-estimands-2026-08-11.csv" = summaries$macro,
  "mv05ad-estimand-intervals-2026-08-11.csv" = inference$intervals,
  "mv05ad-primary-contrasts-2026-08-11.csv" = inference$primary,
  "mv05ad-bootstrap-audit-2026-08-11.csv" = inference$bootstrap_audit,
  "mv05ad-randomization-audit-2026-08-11.csv" = inference$randomization_audit,
  "mv05ad-production-summary-2026-08-11.csv" = production,
  "mv05ad-outcome-resource-summary-2026-08-11.csv" = resource)
...
message("MV5-AD outcomes passed: groups=150 query_methods=3600 endpoints=7200 estimands=24 primary_tests=4")
