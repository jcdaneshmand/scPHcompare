...
if (length(args) != 2L) {
  stop(
    "usage: run_mv05e_retrieval_evaluation.R FROZEN_METADATA_CSV OUTPUT_DIR",
    call. = FALSE
  )
}
...
source("R/mv05e_retrieval_evaluation.R")

audit_dir <- "docs/audits"
ranking_path <- file.path(
  audit_dir, "mv05d5-cell-retrieval-rankings-2026-08-08.csv.gz"
)
completion_path <- file.path(
  audit_dir, "mv05d5-method-completion-2026-08-08.csv"
)
group_index_path <- file.path(
  audit_dir, "mv05d5-group-bundle-index-2026-08-08.csv"
)
assembly_path <- file.path(
  audit_dir, "mv05d5-public-assembly-summary-2026-08-08.csv"
)
...
prediction_lock <- mv05e_verify_prediction_lock_v1(
  ranking_path, completion_path, group_index_path, assembly_path
)
write_provenance_csv(
  prediction_lock,
  file.path(output_dir, "mv05e-prediction-lock-2026-08-08.csv")
)

label_bundle <- mv05e_open_frozen_labels_v1(metadata_path, prediction_lock)
labels <- label_bundle$labels
write_provenance_csv(
  label_bundle$provenance,
  file.path(output_dir, "mv05e-label-source-provenance-2026-08-08.csv")
)

rankings <- utils::read.csv(
  ranking_path, stringsAsFactors = FALSE, check.names = FALSE
)
rankings_sha_after_read <- .mv05e_file_sha256(ranking_path)
...
observations <- mv05e_evaluate_retrieval_v1(rankings, labels)
summaries <- mv05e_summarize_retrieval_v1(observations)
inference <- mv05e_block_inference_v1(summaries$sample)

tissue_sample_count <- table(labels$tissue)
tissue_study_count <- tapply(labels$study, labels$tissue,
                             function(x) length(unique(x)))
eligibility <- data.frame(
  contract_id = "mv05e_sample_eligibility_v1",
  ...
  cross_study_tissue_eligible = labels$tissue %in% .mv05e_eligible_tissues,
...
disposition <- do.call(rbind, lapply(.mv05e_methods, function(method) {
  ...
  data.frame(
    contract_id = "mv05e_endpoint_disposition_summary_v1",
    method_id = method, method_role = unname(.mv05e_method_roles[method]),
...
outputs <- list(
  "mv05e-sample-eligibility-2026-08-08.csv" = eligibility,
  "mv05e-query-endpoints-2026-08-08.csv" = observations,
  "mv05e-endpoint-dispositions-2026-08-08.csv" = disposition,
  "mv05e-tissue-seed-endpoints-2026-08-08.csv" = summaries$tissue_seed,
  "mv05e-seed-macro-endpoints-2026-08-08.csv" = summaries$seed_macro,
  "mv05e-sample-endpoint-summaries-2026-08-08.csv" = summaries$sample,
  "mv05e-tissue-endpoint-summaries-2026-08-08.csv" = summaries$tissue,
  "mv05e-method-endpoint-summaries-2026-08-08.csv" = summaries$method,
  "mv05e-method-intervals-2026-08-08.csv" = inference$method_intervals,
  "mv05e-paired-contrasts-2026-08-08.csv" = inference$contrasts,
  "mv05e-bootstrap-audit-2026-08-08.csv" = inference$bootstrap_audit,
  "mv05e-randomization-audit-2026-08-08.csv" = inference$randomization_audit
)
...
primary$contract_id <- "mv05e_production_summary_v1"
...
primary <- primary[, c(
  ...
  "fusion_jobs_executed", "biological_outcomes_computed",
  "outcome_label_state"
)]
write_provenance_csv(
  primary, file.path(output_dir, "mv05e-production-summary-2026-08-08.csv")
)
...
message("MV5-E prediction-locked retrieval evaluation completed.")
