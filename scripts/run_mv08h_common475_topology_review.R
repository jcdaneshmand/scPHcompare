...
if (length(args) != 5L) {
  stop("usage: run_mv08h_common475_topology_review.R <filtered.h5> <raw.h5> <reference.rds> <panel.csv> <output-dir>", call. = FALSE)
}
...
panel_order_col <- if (panel_size == 475L) "panel_order_475" else "panel_order"
panel_digest <- if (panel_size == 475L) unique(panel$common475_axis_sha256) else unique(panel$panel_sha256)
profile <- if (panel_size == 475L) "scientific_common475" else "scientific"
stable <- sub("\\.[0-9]+$", "", sub("^.*-", "", panel$feature_id))
reference <- readRDS(reference_path)
if (!identical(as.integer(panel[[panel_order_col]]), seq_len(panel_size)) || !identical(length(panel_digest), 1L) ||
    !identical(panel_size, length(reference$panel$feature_id)) || !identical(panel_digest, reference$identity[[if (panel_size == 475L) "common475_axis_sha256" else "panel_sha256"]])) {
...
source_obj <- new_dual_view_source(
  ...
  contract_profile = profile, expected_genes = panel_size
)
...
summary$panel_sha256 <- panel_digest
summary$panel_genes <- panel_size
...
saveRDS(list(cell_topology_v1 = cell_result$diagram, gene_topology_v1 = gene_result$diagram), file.path(out_dir, "private-topology-diagrams.rds"))
...
identity <- data.frame(
  ...
  panel_genes = panel_size, panel_sha256 = panel_digest,
...
validation <- data.frame(
  ...
  passed = c(sum(eligible) >= 384L, TRUE, identical(length(selected), 384L), inherits(coords, "scph_cell_topology_view_v1"), inherits(gene_view, "scph_gene_topology_view_v1"), nrow(summary) == 4L, summary$positive_intervals[summary$view_id == "cell_topology_v1" & summary$homology_dimension == "H0"] == 383L, summary$positive_intervals[summary$view_id == "gene_topology_v1" & summary$homology_dimension == "H0"] == (panel_size - 1L), all(summary$positive_intervals[summary$homology_dimension == "H1"] >= 0L), TRUE, TRUE),
  evidence = c(paste0("eligible=", sum(eligible)), paste0("all ", panel_size, " stable IDs mapped once"), "seed=20260805", "immutable 30-PC coordinates", "correlation-chord dist", "four rows: two views x H0/H1", "cell H0 positive intervals=383", paste0("gene H0 positive intervals=", panel_size - 1L), "H1 retained even when empty", "labels/outcomes closed", "no landscapes or fusion"),
...
report <- c(
  ...
  paste0("- Cell view uses the immutable 30-PC projection; gene view uses the same ", panel_size, " genes under correlation-chord geometry."),
