...
if (length(args) != 3L) {
  stop(paste("usage: build_mv05aj_nested256_execution.R OUTPUT_DIR",
...
if (!grepl("^[0-9a-f]{40}$", head)) stop("MV5-AJ HEAD is invalid.", call. = FALSE)
observed_head <- trimws(system2("git", c("rev-parse", "HEAD"), stdout = TRUE))
if (!identical(observed_head, head)) {
  stop("MV5-AJ requires its prospectively committed engine HEAD ", head,
...
support_files <- c(
  builder = "scripts/build_mv05aj_nested256_execution.R",
  specification = "docs/specifications/MV05AJ_LABEL_CLOSED_NESTED256_EXECUTION_SPECIFICATION_V1.md",
  tests = "tests/testthat/test-mv05aj-nested256-execution.R",
  ai_queue = "docs/audits/mv05ai-execution-queue-2026-08-11.csv",
  ai_decision = "docs/audits/mv05ai-continuation-decision-2026-08-11.csv",
  ai_source_freeze = "docs/audits/mv05ai-source-freeze-2026-08-11.csv",
  ai_repeat = "docs/audits/mv05ai-deterministic-repeat-2026-08-11.csv",
  ...
  artifact_validator = "scripts/validate_mv05aj_configuration.R",
  geometry_validator = "scripts/validate_mv05aj_geometry_oracles.R",
  landscape_validator = "scripts/validate_mv05aj_landscape_oracles.py",
  snapshot = "scripts/snapshot_mv05aj_results.R")
files <- c(implementation_files, support_files)
if (any(!file.exists(files))) {
  stop("MV5-AJ source roster is incomplete: ",
...
if (length(runtime) != 6L) stop("MV5-AJ runtime identity failed.", call. = FALSE)
...
if (nrow(inventory) != 150L || anyDuplicated(inventory$sha256) ||
  ...
  stop("MV5-AJ private coordinate inventory drifted.", call. = FALSE)
}
private_sources <- data.frame(
  contract_id = "mv05aj_source_freeze_v1",
...
decision <- read_csv(support_files[["ai_decision"]])
gate_sources <- read_csv(support_files[["ai_source_freeze"]])
gate_repeat <- read_csv(support_files[["ai_repeat"]])
if (nrow(decision) != 1L ||
    decision$authorized_configuration_id !=
      "nested_cells_256_pc30_euclidean_v1" ||
    !as.logical(decision$nested_256_calculation_authorized) ||
    as.logical(decision$nested_256_calculation_executed) ||
    as.logical(decision$labels_opened) || as.logical(decision$rankings_computed) ||
    as.logical(decision$outcomes_computed) ||
    nrow(gate_sources) != 43L || nrow(gate_repeat) != 12L ||
    !all(as.logical(gate_repeat$byte_exact)) ||
    any(as.logical(gate_repeat$labels_opened)) ||
    any(as.logical(gate_repeat$outcomes_computed))) {
  stop("MV5-AI authorization evidence is incomplete or contaminated.",
       call. = FALSE)
}
queue <- read_csv(support_files[["ai_queue"]])
if (nrow(queue) != 150L ||
    !identical(as.integer(queue$execution_order), 451:600)) {
  stop("MV5-AJ requires the exact MV5-AI nested-256 queue.", call. = FALSE)
}
...
minimum_row <- which(
  queue$training_samples == min(queue$training_samples) &
    queue$representation == "inductive_integrated" &
    queue$seed == min(queue$seed)
)[1L]
maximum_row <- which(
  queue$training_samples == max(queue$training_samples) &
    queue$representation == "sct_whole" &
    queue$seed == min(queue$seed)
)[1L]
if (anyNA(c(minimum_row, maximum_row))) {
  stop("MV5-AJ could not bind minimum/maximum-fold repeats.", call. = FALSE)
}
queue$deterministic_repeat_required[c(minimum_row, maximum_row)] <- TRUE
...
write_once(sources, file.path(output_dir, "mv05aj-source-freeze-2026-08-11.csv"))
write_once(queue, file.path(output_dir, "mv05aj-nested256-execution-queue-2026-08-11.csv"))
write_once(scope, file.path(output_dir, "mv05aj-nested256-execution-scope-2026-08-11.csv"))
message("MV5-AJ bound 150 nested256 groups: implementation_sha=", implementation_sha)
