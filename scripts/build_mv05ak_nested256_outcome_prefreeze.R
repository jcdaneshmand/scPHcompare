...
for (package in c("digest", "pkgload")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for MV5-AK prefreeze.", call. = FALSE)
  }
}
...
if (length(args) != 4L) {
  stop(paste(
    "usage: build_mv05ak_nested256_outcome_prefreeze.R",
    "MV05AJ_RESULT_ROOT EXTERNAL_METADATA_PATH AUDIT_DIR EXPECTED_HEAD"),
    call. = FALSE)
}
...
closed <- function(value) {
  !any(c("tissue", "tissue.x", "approach", "approach.x",
         "reciprocal_rank", "one_nn_correct",
         "p_value", "estimate") %in% tolower(names(value))) &&
    (!("outcomes_computed" %in% names(value)) ||
       !any(.mv05ak_is_true(value$outcomes_computed)))
}
...
if (!identical(observed_head, expected_head)) {
  stop("MV5-AK must run at its prospectively accepted engine HEAD ", expected_head,
...
if (!identical(file_sha(metadata_path), expected_label_sha)) {
  stop("MV5-AK external label source hash drifted.", call. = FALSE)
}
...
paths <- c(
  ...
  mv05ag_endpoint_registry = "docs/audits/mv05ag-endpoint-registry-2026-08-11.csv",
  mv05ag_estimand_registry = "docs/audits/mv05ag-estimand-registry-2026-08-11.csv",
  mv05ah_spec = "docs/specifications/MV05AH_NESTED192_RETRIEVAL_ROBUSTNESS_EXECUTION_SPECIFICATION_V1.md",
  mv05ah_audit = "docs/audits/MV05AH_NESTED192_RETRIEVAL_ROBUSTNESS_EXECUTION_2026-08-11.md",
  mv05ai_spec = "docs/specifications/MV05AI_SELECTION_RESISTANT_NESTED256_CONTINUATION_GATE_SPECIFICATION_V1.md",
  mv05ai_audit = "docs/audits/MV05AI_SELECTION_RESISTANT_NESTED256_CONTINUATION_GATE_2026-08-11.md",
  ...
  mv05aj_execution_spec = "docs/specifications/MV05AJ_LABEL_CLOSED_NESTED256_EXECUTION_SPECIFICATION_V1.md",
  mv05aj_execution_audit = "docs/audits/MV05AJ_LABEL_CLOSED_NESTED256_CONFIGURATION_EXECUTION_2026-08-11.md",
  mv05ak_spec = "docs/specifications/MV05AK_NESTED256_ROBUSTNESS_OUTCOME_PREFREEZE_SPECIFICATION_V1.md",
  mv05ak_code = "R/mv05ak_nested256_outcome_prefreeze.R",
  mv05ak_builder = "scripts/build_mv05ak_nested256_outcome_prefreeze.R",
  mv05ak_validator = "scripts/validate_mv05ak_nested256_outcome_prefreeze.R",
  mv05ak_tests = "tests/testthat/test-mv05ak-nested256-outcome-prefreeze.R",
  d1_fold_validation = "docs/audits/mv05d1-fold-cache-entry-validation-2026-08-07.csv",
  aj_source_freeze = "docs/audits/mv05aj-source-freeze-2026-08-11.csv",
  aj_scope = "docs/audits/mv05aj-nested256-execution-scope-2026-08-11.csv",
  aj_queue = "docs/audits/mv05aj-nested256-execution-queue-2026-08-11.csv",
  aj_completion = "docs/audits/mv05aj-nested256-unit-completion-2026-08-11.csv",
  aj_resources = "docs/audits/mv05aj-nested256-production-resources-2026-08-11.csv",
  aj_validation = "docs/audits/mv05aj-independent-validation-2026-08-11.csv",
  aj_decision = "docs/audits/mv05aj-configuration-decision-2026-08-11.csv",
  aj_geometry = "docs/audits/mv05aj-independent-geometry-groups-2026-08-11.csv",
  aj_repeat = "docs/audits/mv05aj-deterministic-repeat-2026-08-11.csv",
  aj_resume = "docs/audits/mv05aj-resume-validation-2026-08-11.csv",
  aj_validator_repeat = "docs/audits/mv05aj-validator-repeat-2026-08-11.csv",
...
if (any(!file.exists(paths))) {
  stop("MV5-AK committed source set is incomplete: ",
...
estimands <- mv05ak_estimand_registry_v1(method_map, endpoints)
inherited_endpoints <- read_public(paths[["mv05ag_endpoint_registry"]])
inherited_estimands <- read_public(paths[["mv05ag_estimand_registry"]])
endpoint_fields <- c(
  "endpoint_id", "query_value", "endpoint_role", "aggregation_policy",
  "independent_unit", "blocking_unit")
new_estimand_axis <- sort(paste(
  grepl("^direct_", estimands$estimand_type), estimands$representation,
  estimands$family_id, estimands$endpoint_id, sep = "\r"), method = "radix")
old_estimand_axis <- sort(paste(
  grepl("^direct_", inherited_estimands$estimand_type),
  inherited_estimands$representation, inherited_estimands$family_id,
  inherited_estimands$endpoint_id, sep = "\r"), method = "radix")
if (!identical(endpoints[endpoint_fields], inherited_endpoints[endpoint_fields]) ||
    !identical(new_estimand_axis, old_estimand_axis) ||
    nrow(inherited_estimands) != 24L) {
  stop("MV5-AK changed the inherited endpoint or complete estimand panel.",
       call. = FALSE)
}

aj_queue <- read_public(paths[["aj_queue"]])
aj_queue <- aj_queue[.mv05ak_is_true(aj_queue$execution_authorized), , drop = FALSE]
aj_completion <- read_public(paths[["aj_completion"]])
aj_validation <- read_public(paths[["aj_validation"]])
aj_decision <- read_public(paths[["aj_decision"]])
aj_geometry <- read_public(paths[["aj_geometry"]])
if (nrow(aj_queue) != 150L || nrow(aj_completion) != 150L ||
    nrow(aj_geometry) != 150L ||
    anyDuplicated(aj_queue$robustness_group_id) ||
    !setequal(aj_queue$robustness_group_id, aj_completion$robustness_group_id) ||
    !setequal(aj_queue$robustness_group_id, aj_geometry$robustness_group_id) ||
    !setequal(aj_queue$representation, c("sct_whole", "inductive_integrated")) ||
    any(aj_queue$configuration_id != "nested_cells_256_pc30_euclidean_v1") ||
    any(aj_queue$coordinates != 30L) || any(aj_queue$cells != 256L) ||
    any(aj_queue$point_metric != "euclidean") ||
    nrow(aj_decision) != 1L ||
    !.mv05ak_is_true(aj_decision$configuration_calculation_accepted) ||
    !all(.mv05ak_is_true(aj_validation$passed)) ||
    !all(.mv05ak_is_true(aj_geometry$nested_192_exact_prefix_verified)) ||
    !all(.mv05ak_is_true(aj_geometry$nested_256_subset_of_384_verified)) ||
    any(.mv05ak_is_true(aj_geometry$labels_opened)) ||
    any(.mv05ak_is_true(aj_geometry$rankings_computed)) ||
    any(.mv05ak_is_true(aj_geometry$outcomes_computed)) ||
    any(.mv05ak_is_true(aj_queue$outcomes_computed)) ||
    any(.mv05ak_is_true(aj_completion$outcomes_computed))) {
  stop("MV5-AJ accepted nested-256 group scope failed structural validation.",
       call. = FALSE)
}
...
if (nrow(d5_rankings) != 176750L || nrow(j_rankings) != 176750L ||
    ...
    any(.mv05ak_is_true(d5_rankings$biological_outcomes_computed)) ||
    any(.mv05ak_is_true(j_rankings$biological_outcomes_computed))) {
  stop("Accepted baseline rankings are not complete and outcome-closed.",
       call. = FALSE)
}

group_rows <- vector("list", nrow(aj_queue))
nested256_methods <- vector("list", nrow(aj_queue))
manifest_sources <- vector("list", nrow(aj_queue))
for (index in seq_len(nrow(aj_queue))) {
  unit <- aj_queue[index, , drop = FALSE]
  ...
  if (any(!file.exists(required_files))) {
    stop("MV5-AJ private group is incomplete: ", unit$robustness_group_id,
         call. = FALSE)
  }
  ...
  if (nrow(identity) != 1L || nrow(status) != 1L ||
      ...
      identity$configuration_id != "nested_cells_256_pc30_euclidean_v1" ||
      ...
      !setequal(methods$method_id, unique(method_map$nested256_method_id)) ||
    ...
    stop("MV5-AJ private group schema or identity drifted: ",
  ...
  nested256_methods[[index]] <- methods[c(
  ...
  if (!isTRUE(coordinate_source_exact)) {
    stop("The 384-cell and nested-256 coordinate source identity drifted: ",
         unit$robustness_group_id, call. = FALSE)
  }
  geometry_hit <- aj_geometry$robustness_group_id == unit$robustness_group_id
  if (sum(geometry_hit) != 1L) {
    stop("MV5-AJ nested-256 geometry evidence is not one-to-one.", call. = FALSE)
  }
  geometry <- aj_geometry[geometry_hit, , drop = FALSE]
  nested192_prefix_exact <-
    .mv05ak_is_true(geometry$nested_192_exact_prefix_verified)
  nested256_subset_exact <-
    .mv05ak_is_true(geometry$nested_256_subset_of_384_verified)
  if (!nested192_prefix_exact || !nested256_subset_exact) {
    stop("The frozen 192/256/384 nesting identity failed: ",
         unit$robustness_group_id, call. = FALSE)
  }
  group_rows[[index]] <- data.frame(
    ...
    nested256_group_manifest_sha256 = file_sha(required_files[[4L]]),
    baseline_coordinate_source_sha256 = baseline_coordinate_sha,
    nested256_coordinate_source_sha256 = unit$coordinate_source_sha256,
    coordinate_source_identity_exact = coordinate_source_exact,
    nested192_prefix_identity_exact = nested192_prefix_exact,
    nested256_subset_384_identity_exact = nested256_subset_exact,
    private_result_locator = paste0(
      "ignored_tmp:tmp/mv05aj/production/results/", basename(group_path)),
    stringsAsFactors = FALSE)
  manifest_sources[[index]] <- data.frame(
    contract_id = "mv05ak_source_freeze_v1",
    source_id = paste0("nested256_private_group_manifest_", sprintf("%03d", index)),
...
nested256_methods <- do.call(rbind, nested256_methods)

# Normalize accepted baseline method names to the four nested256 method families,
...
for (representation in c("sct_whole", "inductive_integrated")) {
  ...
  source$nested256_method_id <- map$nested256_method_id[
    match(source$method_id, map$baseline_method_id)]
  source$representation <- representation
  baseline_frames[[representation]] <- source[c(
    "fold_id", "seed", "representation", "query_sample_id",
    "training_sample_id", "nested256_method_id")]
}
baseline_axis <- do.call(rbind, baseline_frames)
names(baseline_axis)[names(baseline_axis) == "nested256_method_id"] <- "method_id"

nested256_methods$key <- axis_key(nested256_methods$representation,
                               nested256_methods$fold_id, nested256_methods$seed,
                               nested256_methods$query_sample_id,
                               nested256_methods$training_sample_id,
                               nested256_methods$method_id)
...
if (anyDuplicated(nested256_methods$key) || anyDuplicated(baseline_axis$key) ||
    !setequal(nested256_methods$key, baseline_axis$key)) {
  stop("The 384-cell and nested-256 prediction axes are not exactly pairable.",
       call. = FALSE)
}

axis_rows <- lapply(seq_len(nrow(method_map)), function(index) {
  map <- method_map[index, , drop = FALSE]
  x <- nested256_methods$representation == map$representation &
    nested256_methods$method_id == map$nested256_method_id
  b <- baseline_axis$representation == map$representation &
    baseline_axis$method_id == map$nested256_method_id
  x_key <- sort(nested256_methods$key[x], method = "radix")
  b_key <- sort(baseline_axis$key[b], method = "radix")
  data.frame(
    contract_id = "mv05ak_prediction_axis_compatibility_v1",
    representation = map$representation, family_id = map$family_id,
    baseline_method_id = map$baseline_method_id,
    nested256_method_id = map$nested256_method_id,
    baseline_rows = length(b_key), nested256_rows = length(x_key),
    missing_nested256_rows = length(setdiff(b_key, x_key)),
    excess_nested256_rows = length(setdiff(x_key, b_key)),
    baseline_axis_sha256 = .mv05ak_digest(b_key),
    nested256_axis_sha256 = .mv05ak_digest(x_key),
...
if (nrow(axis_audit) != 8L ||
    ...
    any(axis_audit$missing_nested256_rows != 0L) ||
    any(axis_audit$excess_nested256_rows != 0L)) {
  stop("MV5-AK method-level axis compatibility failed.", call. = FALSE)
}
...
if (!all(required_metadata %in% names(metadata_header))) {
  stop("MV5-AK external label schema drifted.", call. = FALSE)
}
...
analysis_samples <- sort(unique(c(nested256_methods$query_sample_id,
                                  nested256_methods$training_sample_id)),
                         method = "radix")
query_pairs <- unique(nested256_methods[c("fold_id", "query_sample_id")])
...
if (nrow(metadata_keys) != 124L || anyDuplicated(metadata_keys$sample_id) ||
  ...
  stop("MV5-AK external sample/study key join is incompatible.", call. = FALSE)
}
label_join <- data.frame(
  contract_id = "mv05ak_label_join_prefreeze_v1",
...
source_freeze <- data.frame(
  contract_id = "mv05ak_source_freeze_v1", source_id = names(paths),
  ...
  outcome_bearing_source = names(paths) %in%
    c("e_query_endpoints", "k_query_endpoints", "mv05ah_audit"),
  source_read_for_prefreeze = !names(paths) %in%
    c("e_query_endpoints", "k_query_endpoints", "mv05ah_audit"),
  outcomes_computed = FALSE, evaluation_executed = FALSE,
  stringsAsFactors = FALSE)
source_freeze <- rbind(source_freeze, do.call(rbind, manifest_sources), data.frame(
  contract_id = "mv05ak_source_freeze_v1", source_id = "external_label_source",
  ...
  outcomes_computed = FALSE, evaluation_executed = FALSE,
  stringsAsFactors = FALSE))
source_freeze$source_freeze_sha256 <- .mv05ak_digest(
...
queue <- mv05ak_build_evaluation_queue_v1(group_scope, source_sha)

prediction_lock <- data.frame(
  contract_id = "mv05ak_prediction_lock_v1",
...
clustering <- data.frame(
  contract_id = "mv05ak_clustering_compatibility_disposition_v1",
  question = "can_nested256_clustering_be_evaluated_from_accepted_mv05aj_artifacts",
  disposition = "not_identifiable_from_directed_retrieval_pairs_only",
  nested256_heldout_training_pairs = sum(group_scope$biological_pairs),
  nested256_within_training_pairs = 0L,
  ...
  clustering_requires = paste(
    "new_label_closed_nested256_training_distance_matrices",
    "frozen_k_selection_and_heldout_assignment", sep = ";"),
  reuse_mv05aj_for_clustering_authorized = FALSE,
  new_clustering_calculation_authorized = FALSE,
  outcomes_computed = FALSE, evaluation_executed = FALSE,
  stringsAsFactors = FALSE)

validation <- data.frame(
  contract_id = "mv05ak_validation_plan_v1",
  validation_id = c(
...
abort <- data.frame(
  contract_id = "mv05ak_abort_rules_v1",
  rule_id = sprintf("MV5AK-ABORT-%02d", 1:12),
  trigger = c(
    "source_hash_or_accepted_head_mismatch",
    "nested256_private_group_manifest_or_completion_mismatch",
    "baseline_nested256_fold_seed_query_reference_method_axis_mismatch",
...
resources <- data.frame(
  contract_id = "mv05ak_resource_envelope_v1", worker_limit = 1L,
...
reporting <- data.frame(
  contract_id = "mv05ak_reporting_contract_v1",
  rule_id = c(
  ...
  rule = c(
    ...
    "signed_nested256_minus_384cell_and_did_positive_increase_negative_decrease_zero_stability",
...
criteria <- data.frame(
  contract_id = "mv05ak_acceptance_criteria_v1",
  criterion_id = c(
    "prospective_head_exact", "complete_source_freeze",
    "coordinate_and_nested_identity", "complete_prediction_axis_pairing",
  ...
  requirement = c(
    ...
    paste(
      "all_150_nested256_groups_match_the_exact_accepted_384_cell_coordinate_source",
      "and_verify_the_accepted_192_prefix_and_256_of_384_subset", sep = "_"),
    ...
    "rankings_labels_outcomes_method_selection_other_configurations_and_new_calculation_remain_zero"),
  required = TRUE,
  observed = c(
    expected_head, paste0(nrow(source_freeze), "_sources"),
    paste0(sum(group_scope$coordinate_source_identity_exact),
           "_source_", sum(group_scope$nested192_prefix_identity_exact),
           "_prefix_", sum(group_scope$nested256_subset_384_identity_exact),
           "_subset_of_150_groups"),
    paste0(nrow(axis_audit), "_axes_", sum(axis_audit$nested256_rows), "_rows"),
    ...
    "rankings_0_labels_0_outcomes_0_selection_0_other_configurations_0_calculation_0"),
  passed = TRUE,
  outcomes_computed = FALSE, evaluation_executed = FALSE,
  stringsAsFactors = FALSE)

decision <- data.frame(
  contract_id = "mv05ak_prefreeze_decision_v1",
  scientific_contract_coherent = TRUE,
  prediction_axes_pairable = TRUE,
  retrieval_evaluation_identifiable = TRUE,
  clustering_evaluation_identifiable_from_mv05aj = FALSE,
  decision = "approve_later_separate_prediction_locked_nested256_retrieval_robustness_execution_only",
  execution_authorized_now = FALSE,
  other_configurations_authorized = FALSE,
  labels_joined_to_nested256_predictions = FALSE,
...
for (value in list(
  ...
  .mv05ak_assert_preoutcome(value, "MV5-AK prefreeze artifact")
}

outputs <- list(
  "mv05ak-source-freeze-2026-08-11.csv" = source_freeze,
  "mv05ak-method-map-2026-08-11.csv" = method_map,
  "mv05ak-endpoint-registry-2026-08-11.csv" = endpoints,
  "mv05ak-estimand-registry-2026-08-11.csv" = estimands,
  "mv05ak-prediction-axis-compatibility-2026-08-11.csv" = axis_audit,
  "mv05ak-label-join-prefreeze-2026-08-11.csv" = label_join,
  "mv05ak-prediction-lock-contract-2026-08-11.csv" = prediction_lock,
  "mv05ak-evaluation-queue-2026-08-11.csv" = queue,
  "mv05ak-clustering-compatibility-disposition-2026-08-11.csv" = clustering,
  "mv05ak-validation-plan-2026-08-11.csv" = validation,
  "mv05ak-abort-rules-2026-08-11.csv" = abort,
  "mv05ak-resource-envelope-2026-08-11.csv" = resources,
  "mv05ak-reporting-contract-2026-08-11.csv" = reporting,
  "mv05ak-acceptance-criteria-2026-08-11.csv" = criteria,
  "mv05ak-prefreeze-decision-2026-08-11.csv" = decision)
for (name in names(outputs)) write_once(outputs[[name]], file.path(audit_dir, name))

message(
  "MV5-AK prefreeze passed: sources=", nrow(source_freeze),
  " groups=", nrow(queue), " exact_method_axes=", nrow(axis_audit),
  " estimands=", nrow(estimands),
  " retrieval_identifiable=1 clustering_from_mv05aj=0 outcomes=0")
