...
if (length(args) != 1L) {
  stop("usage: build_mv05d0_artifact_manifest.R OUTPUT_CSV", call. = FALSE)
}
output_path <- args[[1L]]
...
fixed <- c(
  ...
  "docs/specifications/MV05D0_NORMALIZATION_CACHE_SPECIFICATION_V2.md",
  "docs/architecture/ADR-008-MV05D0-NORMALIZATION-SOURCE-AND-IDENTITY-GATE.md",
  "docs/architecture/ADR-009-MV05D0-STAGE1-COMPLETION.md",
  "docs/audits/MV05D0_NORMALIZATION_CACHE_GATE_2026-08-07.md",
  "docs/audits/MV05D0_STAGE1_COMPLETION_2026-08-07.md",
  "PROJECT_PLAN.md",
...
scripts <- sort(list.files(
  "scripts", pattern = "mv05d0", full.names = TRUE
), method = "radix")
audits <- sort(list.files(
  "docs/audits", pattern = "^mv05d0.*\\.csv$", full.names = TRUE
), method = "radix")
...
if (!length(paths) || any(!file.exists(paths)) ||
    any(grepl("(?i)(\\.pdf$|example_run|reviewer|pasted-text|^tmp/)",
              paths, perl = TRUE))) {
  stop("MV5-D0 public artifact scope is missing or contains private files.",
       call. = FALSE)
}
role <- ifelse(
...
result <- data.frame(
  contract_id = "mv05d0_public_artifact_manifest_v1",
...
message("Wrote MV5-D0 public artifact manifest for ", nrow(result), " files.")
