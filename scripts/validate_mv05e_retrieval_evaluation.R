...
if (length(args) != 3L) {
  stop(
    "usage: validate_mv05e_retrieval_evaluation.R FROZEN_METADATA_CSV ",
...
read_result <- function(stem) {
  utils::read.csv(
    file.path(result_dir, paste0(stem, "-2026-08-08.csv")),
...
completion_path <- "docs/audits/mv05d5-method-completion-2026-08-08.csv"
group_path <- "docs/audits/mv05d5-group-bundle-index-2026-08-08.csv"
...
if (nrow(completion) != 375L || nrow(groups) != 75L ||
  ...
  stop("Independent D5 completion validation failed.", call. = FALSE)
}
...
query <- read_result("mv05e-query-endpoints")
...
sample_summary <- read_result("mv05e-sample-endpoint-summaries")
sample_groups <- split(
  independent_query,
...
production <- read_result("mv05e-production-summary")
...
if (any(production$upstream_refits != 0L) ||
  ...
  stop("A forbidden or undeclared MV5-E operation is recorded.", call. = FALSE)
}
...
bootstrap_audit <- read_result("mv05e-bootstrap-audit")
...
intervals <- read_result("mv05e-method-intervals")
...
contrasts <- read_result("mv05e-paired-contrasts")
energy <- "cell_distribution_energy_shared_pca_v1"
topology <- c("cell_landscape_h0_v1", "cell_landscape_h1_v1")
...
randomization_audit <- read_result("mv05e-randomization-audit")
...
for (index in seq_along(topology)) {
  ...
  raw_p[[index]] <-
    (sum(abs(null) >= abs(observed$estimate[[1L]])) + 1) / 10000
...
validation <- data.frame(
  contract_id = "mv05e_independent_validation_v1",
  validation = c(
...
message("MV5-E independent retrieval evaluation validation passed.")
