...
for (package in c("processx", "ps", "digest")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for monitored MV5-D4 execution.",
         call. = FALSE)
  }
}
...
if (length(args) != 12L) {
  stop(
    "usage: monitor_mv05d4_landscape_groups.R INPUT_AUDIT INPUT_ROOT ",
...
if (!file.exists(python)) {
  stop("MV5-D4 Python executable does not exist.", call. = FALSE)
}
...
if (is.na(max_groups) || max_groups < 1L || max_groups > 75L ||
  ...
  stop("MV5-D4 monitor guards are invalid.", call. = FALSE)
}
...
engine_script <- normalizePath(
  "scripts/mv05d4_landscape_group.py", winslash = "/", mustWork = TRUE
)
...
if (nrow(inputs) != 75L || anyDuplicated(inputs$group_id) ||
  ...
  stop("MV5-D4 group-input audit is invalid.", call. = FALSE)
}
...
validate_group <- function(group, output_dir, status_dir) {
  ...
  if (is.null(status) || is.null(observed) ||
      ...
      any(observed$integration_jobs_executed != 0L) ||
      any(observed$gene_view_jobs_executed != 0L)) {
    stop("Completed MV5-D4 group failed output validation.", call. = FALSE)
  }
  list(
...
if (!is.null(existing) &&
  ...
  stop("Existing MV5-D4 group metrics are not safely resumable.",
       call. = FALSE)
}
...
while (nrow(pending) > 0L || length(running) > 0L) {
  ...
  for (id in names(running)) {
    ...
    rows[[length(rows) + 1L]] <- data.frame(
      contract_id = "mv05d4_group_resource_metric_v1",
...
if (sum(metrics$completed_distance_rows) != sum(inputs$request_rows) ||
  ...
  stop("MV5-D4 production metrics violate completion or resource gates.",
       call. = FALSE)
}
message("Completed ", nrow(metrics), " MV5-D4 landscape-distance groups.")
