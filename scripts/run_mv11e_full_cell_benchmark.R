...
if (length(args) != 6L) stop(paste(
  "usage: run_mv11e_full_cell_benchmark.R <prefreeze> <admission>",
  "<matrix-bundle> <private-output> <public-output> <execution-head>"
), call. = FALSE)
prefreeze <- normalizePath(args[[1L]], mustWork = TRUE)
admission <- normalizePath(args[[2L]], mustWork = TRUE)
bundle_path <- normalizePath(args[[3L]], mustWork = TRUE)
private <- args[[4L]]; public <- args[[5L]]
execution_head <- tolower(trimws(args[[6L]]))
if (dir.exists(private) || dir.exists(public)) {
  stop("MV11-E output root already exists", call. = FALSE)
}
...
.mv08z_verify_manifest(admission, "mv11d-artifact-manifest.csv")
...
admit <- readc(file.path(admission, "mv11d-decision.csv"))
if (nrow(contract) != 1L || nrow(queue) != 10L ||
  ...
  stop("MV11-E full-execution binding drift", call. = FALSE)
}
...
worker <- normalizePath("scripts/run_mv11_cell_matrix_worker.R",
                        mustWork = TRUE)
ledger <- NULL; run_started <- Sys.time()
write_progress <- function(state) {
  completed <- if (is.null(ledger)) 0L else sum(ledger$disposition == "completed")
  atomic(data.frame(
    contract_id = "mv11e_progress_v1", state = state,
    authorized_matrices = 10L, completed_matrices = completed,
    remaining_matrices = 10L - completed,
  ...
  ), file.path(public, "mv11e-progress.csv"))
}
...
for (i in seq_len(nrow(queue))) {
  ...
  row <- data.frame(
    contract_id = "mv11e_resource_ledger_v1", execution_order = i,
...
if (nrow(assignments) != 55800L || nrow(quality) != 450L) {
  stop("MV11-E aggregate row contract failed", call. = FALSE)
}
stability <- mv10_seed_stability_v1(assignments)
agreement <- mv10_method_agreement_v1(assignments)
primary_k <- mv11_select_primary_k_v1(assignments)
if (nrow(stability) != 90L || nrow(agreement) != 900L ||
    nrow(primary_k) != 2L) {
  stop("MV11-E aggregate calculation contract failed", call. = FALSE)
}
atomic(assignments, file.path(private, "mv11e-sample-partitions.csv"))
atomic(quality, file.path(public, "mv11e-partition-quality.csv"))
atomic(stability, file.path(public, "mv11e-seed-stability.csv"))
atomic(primary_k, file.path(public, "mv11e-primary-k-selection.csv"))
atomic(agreement, file.path(public, "mv11e-method-agreement.csv"))
...
receipt <- data.frame(
  contract_id = "mv11e_terminal_receipt_v1", execution_head = execution_head,
  completion_state = "complete", matrices = 10L, partition_fits = 450L,
...
if (private_bytes > receipt$private_storage_cap_bytes ||
  ...
  stop("MV11-E aggregate resource cap exceeded", call. = FALSE)
}
atomic(receipt, file.path(public, "mv11e-terminal-receipt.csv"))
...
if (any(vapply(public_frames, function(value) {
  "sample_id" %in% names(value)
}, logical(1L)))) stop("MV11-E public sample-axis privacy failure")
files <- sort(setdiff(list.files(public), "mv11e-artifact-manifest.csv"))
manifest <- data.frame(
  contract_id = "mv11e_artifact_manifest_v1", artifact = files,
  bytes = as.numeric(file.info(file.path(public, files))$size),
  sha256 = vapply(file.path(public, files), sha, character(1L)),
  stringsAsFactors = FALSE
)
atomic(manifest, file.path(public, "mv11e-artifact-manifest.csv"))
cat("Completed MV11-E full cell benchmark; fits=450\n")
