...
for (package in c("processx", "ps", "digest")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for monitored MV5-D5 execution.",
         call. = FALSE)
  }
}
...
if (length(args) != 14L) {
  stop(
    "usage: run_mv05d5_queue.R D1_RESOURCE_CSV FOLD_CACHE_DIR ",
    "MEAN_AUDIT_CSV MEAN_DIR D4_COMPONENT_GZ OUTPUT_DIR AUDIT_DIR LOG_DIR ",
    "METRICS_CSV MAX_GROUPS MAX_WORKERS GROUP_TIMEOUT_SECONDS RSS_CAP_BYTES ",
    "STAGE_CAP_SECONDS", call. = FALSE
  )
}
...
audit_dir <- args[[7L]]
log_dir <- args[[8L]]
metrics_path <- args[[9L]]
max_groups <- as.integer(args[[10L]])
max_workers <- as.integer(args[[11L]])
timeout_seconds <- as.numeric(args[[12L]])
rss_cap_bytes <- as.numeric(args[[13L]])
stage_cap_seconds <- as.numeric(args[[14L]])
if (is.na(max_groups) || max_groups < 1L || max_groups > 75L ||
  ...
  stop("MV5-D5 queue guards are invalid.", call. = FALSE)
}
...
group_script <- normalizePath(
  "scripts/run_mv05d5_group.R", winslash = "/", mustWork = TRUE
)
...
if (nrow(groups) != 75L ||
  ...
  stop("MV5-D1 resource manifest does not admit the MV5-D5 queue.",
       call. = FALSE)
}
...
validate_group <- function(group) {
  ...
  if (!file.exists(paths$output) || !file.exists(paths$audit)) {
    stop("MV5-D5 group output or audit is absent.", call. = FALSE)
  }
  ...
  if (nrow(audit) != 1L ||
      ...
      any(bundle$payload$failures$status != "completed")) {
    stop("MV5-D5 completed group failed queue validation.", call. = FALSE)
  }
...
if (!is.null(existing)) {
  if (anyDuplicated(existing$group_order) ||
    ...
    stop("Existing MV5-D5 queue metrics are not safely resumable.",
         call. = FALSE)
  }
...
launch <- function(group) {
  ...
  process <- processx::process$new(
    ...
    args = c(
      ...
      mean_dir, d4_path, paths$output, paths$audit,
...
while (nrow(pending) > 0L || length(running) > 0L) {
  ...
  for (id in names(running)) {
    ...
    rows[[length(rows) + 1L]] <- data.frame(
      contract_id = "mv05d5_group_resource_metric_v1",
...
message("Completed all 75 MV5-D5 retrieval-input groups.")
