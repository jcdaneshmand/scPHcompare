...
source("R/mv05af_nested_execution.R")
...
implementation_files <- c(
  ...
  mv05af = "R/mv05af_nested_execution.R",
  landscape = "scripts/mv05w_exact_landscape_group.py",
  group_runner = "scripts/run_mv05af_nested192_group.R",
  monitor = "scripts/monitor_mv05af_nested192_configuration.R"
)
...
mv05af_validate_nested_queue_v1(queue)
  ...
  !identical(dim(view$payload), c(as.integer(unit$cells),
    ...
    !identical(
      ...
      mv05t_nested_point_ids_v1(
        view$sample_id, seed, source_views[[view$sample_id]]$point_ids, 192L
      )
    ) ||
    view$transformations$mv05u_robustness$point_selection !=
      "sha256_sample_seed_cell_nested_v1"
}, logical(1L)))) {
  stop("MV5-AF transformed nested-192 view axis is incomplete.", call. = FALSE)
}
