...
for (package in c("processx", "ps", "digest")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for monitored MV5-AJ execution.", call. = FALSE)
  }
}
args <- commandArgs(trailingOnly = TRUE)
if (length(args) != 13L) {
  stop(paste(
    "usage: monitor_mv05aj_nested256_configuration.R EXECUTION_QUEUE PRIVATE_INVENTORY",
...
if (is.na(max_units) || max_units != 150L) {
  stop("MV5-AJ nested256 monitor requires exactly 150 groups.", call. = FALSE)
}
head <- trimws(system2("git", c("rev-parse", "HEAD"), stdout = TRUE))
if (!identical(head, expected_head)) {
  stop("MV5-AJ monitor is not at its bound prospective HEAD.", call. = FALSE)
}
...
if (nrow(queue) != max_units ||
  ...
  stop("MV5-AJ bound nested256 identity is stale.", call. = FALSE)
}
...
if (file.exists(resource_output)) {
  ...
  if (!all(required_metrics %in% names(metrics)) ||
    ...
    stop("MV5-AJ existing resource checkpoint is invalid.", call. = FALSE)
  }
}
...
if (completed_count < nrow(queue)) for (index in seq.int(completed_count + 1L,
  ...
  process <- processx::process$new(
    ...
    args = c(
      "--vanilla", "scripts/run_mv05aj_nested256_group.R", queue_path,
  ...
  row <- data.frame(
    contract_id = "mv05aj_nested256_resource_metric_v1",
...
if (any(!metrics$disposition %in% c("completed", "reused_validated")) ||
    ...
    any(as.logical(metrics$labels_opened)) ||
    any(as.logical(metrics$outcomes_computed))) {
  stop("MV5-AJ nested256 execution completed outside its contract.", call. = FALSE)
}
message("Completed 150 MV5-AJ nested256 groups (", run_mode, ").")
