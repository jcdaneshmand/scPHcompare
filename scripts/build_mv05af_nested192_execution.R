...
if (length(args) != 3L) {
  stop(paste("usage: build_mv05af_nested192_execution.R OUTPUT_DIR",
...
if (!grepl("^[0-9a-f]{40}$", head)) stop("MV5-AF HEAD is invalid.", call. = FALSE)
...
support_files <- c(
  builder = "scripts/build_mv05af_nested192_execution.R",
  specification = "docs/specifications/MV05AF_LABEL_CLOSED_NESTED192_EXECUTION_SPECIFICATION_V1.md",
  tests = "tests/testthat/test-mv05af-nested192-execution.R",
  ae_queue = "docs/audits/mv05ae-execution-queue-2026-08-11.csv",
  ae_decision = "docs/audits/mv05ae-continuation-decision-2026-08-11.csv",
  ae_source_freeze = "docs/audits/mv05ae-source-freeze-2026-08-11.csv",
  ...
  artifact_validator = "scripts/validate_mv05af_configuration.R",
  geometry_validator = "scripts/validate_mv05af_geometry_oracles.R",
  landscape_validator = "scripts/validate_mv05af_landscape_oracles.py",
  snapshot = "scripts/snapshot_mv05af_results.R")
files <- c(implementation_files, support_files)
if (any(!file.exists(files))) {
  stop("MV5-AF source roster is incomplete: ",
...
if (length(runtime) != 6L) stop("MV5-AF runtime identity failed.", call. = FALSE)
...
if (nrow(inventory) != 150L || anyDuplicated(inventory$sha256) ||
  ...
  stop("MV5-AF private coordinate inventory drifted.", call. = FALSE)
}
private_sources <- data.frame(
  contract_id = "mv05af_source_freeze_v1",
...
queue <- read_csv(support_files[["ae_queue"]])
queue$contract_id <- "mv05af_nested192_execution_queue_v1"
...
if (anyNA(c(minimum_row, maximum_row))) {
  stop("MV5-AF could not bind minimum/maximum-fold repeats.", call. = FALSE)
}
...
scope <- mv05af_nested_scope_v1(queue)
...
write_once(sources, file.path(output_dir, "mv05af-source-freeze-2026-08-11.csv"))
write_once(queue, file.path(output_dir, "mv05af-nested192-execution-queue-2026-08-11.csv"))
write_once(scope, file.path(output_dir, "mv05af-nested192-execution-scope-2026-08-11.csv"))
message("MV5-AF bound 150 nested192 groups: implementation_sha=", implementation_sha)
