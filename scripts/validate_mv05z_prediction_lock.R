...
if (length(args) != 6L) {
  stop(paste("usage: validate_mv05z_prediction_lock.R MV05X_RESULT_ROOT",
...
if (nrow(rankings) != 282800L || nrow(completion) != 150L || nrow(lock) != 1L ||
  ...
  stop("MV5-Z prediction-lock public identity failed.", call. = FALSE)
}
...
for (index in seq_len(nrow(completion))) {
  ...
  stem <- sub("^mv05y_eval_v1:", "", row$evaluation_unit_id[[1L]])
...
if (checked != 282800L || !all(private_hashes) || !all(status_hashes)) {
  stop("MV5-Z private ranking identity reconstruction failed.", call. = FALSE)
}
validation <- data.frame(
  contract_id = "mv05z_prediction_independent_validation_v1",
  validation_id = c(
...
message("MV5-Z independent prediction validation passed: rows=282800 groups=150 labels=0 outcomes=0")
