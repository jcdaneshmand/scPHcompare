...
if (length(args) != 2L) {
  stop("usage: snapshot_mv05x_results.R RESULT_ROOT OUTPUT_CSV", call. = FALSE)
}
if (!requireNamespace("digest", quietly = TRUE)) {
  stop("digest is required for MV5-X result snapshots.", call. = FALSE)
}
...
if (length(groups) != 150L || any(grepl("^\\.", basename(groups)))) {
  stop("MV5-X snapshot requires exactly 150 published group directories.",
       call. = FALSE)
}
...
if (length(files) != 1650L || any(!file.exists(files))) {
  stop("MV5-X snapshot requires exactly 1,650 private files.", call. = FALSE)
}
relative <- substring(files, nchar(root) + 2L)
result <- data.frame(
  contract_id = "mv05x_result_snapshot_v1",
...
message("Snapshotted 1,650 MV5-X files across 150 groups.")
