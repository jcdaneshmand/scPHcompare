...
if (length(args) != 11L) {
  stop(paste(
    "usage: validate_mv05af_configuration.R QUEUE INVENTORY RESULT_ROOT",
...
for (package in c("digest")) {
  if (!requireNamespace(package, quietly = TRUE)) {
    stop(package, " is required for MV5-AF validation.", call. = FALSE)
  }
}
...
if (!identical(current_head, execution_head) || nrow(authorized) != 150L ||
  ...
  stop("MV5-AF aggregate execution/resume contract failed.", call. = FALSE)
}
...
for (index in seq_len(nrow(authorized))) {
  ...
  if (!dir.exists(path) || length(files) != 11L || any(file.info(
    ...
    )$isdir)) stop("MV5-AF group file cardinality failed.", call. = FALSE)
  ...
  if (nrow(status) != 1L || nrow(source_identity) != 1L ||
    ...
    stop("MV5-AF group identity/manifest failed at row ", index,
         call. = FALSE)
  }
  ...
  if (!all(manifest_ok) || file_sha(file.path(path, "artifact_manifest.csv")) !=
    ...
    stop("MV5-AF artifact hash validation failed at row ", index,
         call. = FALSE)
  }
  ...
  if (nrow(expected_d1) != 1L || nrow(expected_g) != 1L ||
    ...
    stop("MV5-AF private source identity failed at row ", index,
         call. = FALSE)
  }
  ...
  if (nrow(views) != 90L || any(views$point_count != unit$cells) ||
    ...
    stop("MV5-AF scientific/cardinality contract failed at row ", index,
         call. = FALSE)
  }
  ...
  for (method_id in names(expected)) {
    ...
    if (anyNA(observed$pair_request_id) ||
      ...
      stop("MV5-AF four-method reconstruction failed at row ", index,
           call. = FALSE)
    }
  }

  completion[[index]] <- data.frame(
    contract_id = "mv05af_nested192_unit_completion_v1",
...
if (total_views != 13500L || total_pairs != 70700L ||
  ...
  stop("MV5-AF aggregate scientific cardinalities failed.", call. = FALSE)
}

repeat_units <- authorized[strict(authorized$deterministic_repeat_required),
                           , drop = FALSE]
if (nrow(repeat_units) != 2L) {
  stop("MV5-AF requires two prospectively marked repeat groups.", call. = FALSE)
}
repeat_rows <- list()
cursor <- 0L
...
validation <- data.frame(
  contract_id = "mv05af_independent_validation_v1",
  category = c(
  ...
  passed = TRUE,
  observed = c(
    "150_nested192_only", execution_head, "150_hash_bound",
...
resume_evidence <- data.frame(
  contract_id = "mv05af_resume_validation_v1",
...
decision <- data.frame(
  contract_id = "mv05af_nested192_configuration_decision_v1",
  configuration_id = "nested_cells_192_pc30_euclidean_v1",
  ...
  next_action = "prefreeze_nested192_prediction_locked_outcome_evaluation",
  labels_opened = FALSE, outcomes_computed = FALSE,
  stringsAsFactors = FALSE
)
resources$contract_id <- "mv05af_nested192_production_resource_metric_v1"
write_once(resources, file.path(
  output_dir, "mv05af-nested192-production-resources-2026-08-11.csv"
))
write_once(completion, file.path(
  output_dir, "mv05af-nested192-unit-completion-2026-08-11.csv"
))
write_once(validation, file.path(
  output_dir, "mv05af-independent-validation-2026-08-11.csv"
))
write_once(repeat_evidence, file.path(
  output_dir, "mv05af-deterministic-repeat-2026-08-11.csv"
))
write_once(resume_evidence, file.path(
  output_dir, "mv05af-resume-validation-2026-08-11.csv"
))
write_once(decision, file.path(
  output_dir, "mv05af-configuration-decision-2026-08-11.csv"
))
message("MV5-AF independent artifact validation passed: groups=150 pairs=70700 outcomes=0")
