...
if (length(args) != 4L) {
  stop(paste("usage: validate_mv05z_outcome_execution.R EXTERNAL_METADATA_PATH",
...
rankings <- readc("docs/audits/mv05z-pc20-rankings-2026-08-11.csv.gz")
pc20_public <- readc(paths("mv05z-pc20-query-method-outcomes-2026-08-11.csv"))
paired_public <- readc(paths("mv05z-paired-query-method-endpoints-2026-08-11.csv"))
query_public <- readc(paths("mv05z-query-estimands-2026-08-11.csv"))
sample_public <- readc(paths("mv05z-sample-estimands-2026-08-11.csv"))
tissue_public <- readc(paths("mv05z-tissue-estimands-2026-08-11.csv"))
macro_public <- readc(paths("mv05z-macro-estimands-2026-08-11.csv"))
interval_public <- readc(paths("mv05z-estimand-intervals-2026-08-11.csv"))
primary_public <- readc(paths("mv05z-primary-contrasts-2026-08-11.csv"))
bootstrap_public <- readc(paths("mv05z-bootstrap-audit-2026-08-11.csv"))
random_public <- readc(paths("mv05z-randomization-audit-2026-08-11.csv"))
completion <- readc(paths("mv05z-outcome-unit-completion-2026-08-11.csv"))
registry <- readc("docs/audits/mv05y-estimand-registry-2026-08-11.csv")
method_map <- readc("docs/audits/mv05y-method-map-2026-08-11.csv")
...
for (index in groups) {
  ...
  family <- method_map$family_id[
    method_map$representation == part$representation[[1L]] &
      method_map$pc20_method_id == part$method_id[[1L]]]
...
pc20 <- do.call(rbind, endpoint_rows)
...
pc20_public <- pc20_public[match(key(pc20), key(pc20_public)), ]
if (rank_rows != 282800L || nrow(pc20) != 3600L || anyNA(pc20_public$method_id) ||
    !close_num(pc20$reciprocal_rank, pc20_public$reciprocal_rank) ||
    !identical(as.logical(pc20$one_nn_correct), as.logical(pc20_public$one_nn_correct)) ||
    !identical(as.integer(pc20$first_same_tissue_rank),
               as.integer(pc20_public$first_same_tissue_rank))) {
...
baseline <- baseline[match(pk(pc20), pk(baseline)), ]
direct_rr <- pc20$reciprocal_rank - baseline$reciprocal_rank
direct_nn <- as.numeric(pc20$one_nn_correct) - as.numeric(baseline$one_nn_correct)
paired_public <- paired_public[match(pk(pc20), pk(paired_public)), ]
...
direct <- do.call(rbind, lapply(seq_len(nrow(pc20)), function(i) {
  rbind(
    data.frame(pc20[i, pair_fields], endpoint_id = "cross_study_tissue_mrr_v1",
               estimand_type = "direct_pc20_minus_pc30", value = direct_rr[[i]]),
    data.frame(pc20[i, pair_fields],
               endpoint_id = "cross_study_tissue_1nn_balanced_accuracy_v1",
               estimand_type = "direct_pc20_minus_pc30", value = direct_nn[[i]]))
}))
did <- do.call(rbind, lapply(c("h0", "h1"), function(family) {
  top <- pc20$family_id == family; energy <- pc20$family_id == "energy"
  did_fields <- setdiff(pair_fields, "family_id")
  ek <- key(pc20[energy, ], did_fields)
  energy_index <- which(energy)[match(key(pc20[top, ], did_fields), ek)]
  top_index <- which(top)
  rbind(
    data.frame(pc20[top_index, pair_fields], endpoint_id = "cross_study_tissue_mrr_v1",
      estimand_type = "topology_increment_pc20_minus_pc30_difference_in_differences",
      value = direct_rr[top_index] - direct_rr[energy_index]),
    data.frame(pc20[top_index, pair_fields],
      endpoint_id = "cross_study_tissue_1nn_balanced_accuracy_v1",
      estimand_type = "topology_increment_pc20_minus_pc30_difference_in_differences",
...
signs <- with_rng(20260813L, matrix(sample(c(-1, 1), 9999L * 15L, replace = TRUE),
                                    nrow = 9999L, ncol = 15L,
...
primary_registry <- ordered_registry[
  ordered_registry$estimand_role == "confirmatory_pc20_sensitivity", ]
...
validation <- data.frame(
  contract_id = "mv05z_outcome_independent_validation_v1",
  validation_id = c(
...
message("MV5-Z independent outcome validation passed: 15 categories, 24 estimands, 4 tests")
