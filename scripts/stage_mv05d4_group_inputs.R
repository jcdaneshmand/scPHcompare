...
if (length(args) != 5L) {
  stop(
    "usage: stage_mv05d4_group_inputs.R PAIR_MANIFEST VIEW_METRICS ",
...
write_missing <- function(value, path) {
  if (file.exists(path)) {
    stop("Refusing to overwrite an existing MV5-D4 staged input: ", path,
         call. = FALSE)
  }
...
mv05d3_validate_view_metrics_v1(metrics)
...
for (group_order in seq_along(group_ids)) {
  ...
  if (nrow(group_metrics) != 90L || !setequal(group_metrics$job_id,
                                               expected_jobs)) {
    stop("MV5-D4 group does not resolve to exactly 90 PH records.",
         call. = FALSE)
  }
  if (file.exists(checkpoint_path) || file.exists(request_path) ||
    ...
    if (!all(file.exists(c(checkpoint_path, request_path, interval_path)))) {
      stop("MV5-D4 staged group input is partial; refusing overwrite.",
           call. = FALSE)
    }
    checkpoint <- readRDS(checkpoint_path)
    if (!identical(checkpoint$contract_id,
                   "mv05d4_group_input_checkpoint_v1") ||
      ...
      stop("MV5-D4 staged group input is stale; refusing overwrite.",
           call. = FALSE)
    }
    ...
    for (index in seq_len(nrow(group_metrics))) {
      ...
      if (!file.exists(result_path) ||
        ...
        stop("MV5-D4 found a missing or stale PH result.", call. = FALSE)
      }
      ...
      if (record$cache_key != metric$record_cache_key ||
        ...
        stop("MV5-D4 PH record identity differs from public evidence.",
             call. = FALSE)
      }
    ...
    if (sum(intervals$homology_dimension == "H0") != 90L * 383L ||
      ...
      stop("MV5-D4 finite interval staging violated the landscape boundary.",
           call. = FALSE)
    }
    ...
    checkpoint <- list(
      contract_id = "mv05d4_group_input_checkpoint_v1",
    ...
    if (file.exists(checkpoint_path) || !file.rename(temporary,
                                                     checkpoint_path)) {
      stop("Could not atomically create MV5-D4 input checkpoint.",
           call. = FALSE)
    }
  ...
  rows[[group_order]] <- data.frame(
    contract_id = "mv05d4_group_input_audit_v1",
...
if (file.exists(audit_output)) {
  ...
  unlink(temporary)
  if (!same) stop("Existing MV5-D4 input audit is stale.", call. = FALSE)
} else {
  write_provenance_csv(audit, audit_output)
}
message("Staged and validated 75 MV5-D4 group input bundles.")
