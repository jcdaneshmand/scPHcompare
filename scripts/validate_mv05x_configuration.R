...
if (length(args) != 11L) {
  stop(paste(
    "usage: validate_mv05x_configuration.R QUEUE INVENTORY RESULT_ROOT",
...
closed_frame <- function(value) {
  ...
  flags <- grep("(biological_)?outcomes_computed$", names(value), value = TRUE)
...
if (!identical(current_head, execution_head) || nrow(authorized) != 150L ||
    ...
    sum(resources$elapsed_seconds) > 30 * 60 * 60 ||
    any(resources$peak_process_tree_rss_bytes > 4 * 1024^3) ||
    max(resources$cumulative_private_bytes) > 16 * 1024^3 ||
...
for (index in seq_len(nrow(authorized))) {
  ...
  completion[[index]] <- data.frame(
    contract_id = "mv05x_pc20_unit_completion_v1",
...
for (index in seq_len(nrow(repeat_units))) {
  ...
  for (row in seq_len(nrow(matched))) {
    ...
    repeat_rows[[cursor]] <- data.frame(
      contract_id = "mv05x_deterministic_repeat_v1",
      robustness_group_id = unit$robustness_group_id,
      representation = unit$representation,
...
validation <- data.frame(
  contract_id = "mv05x_independent_validation_v1",
  category = c(
  ...
  passed = TRUE,
  observed = c(
    "150_pc20_and_450_closed", execution_head, "150_hash_bound",
...
resume_evidence <- data.frame(
  contract_id = "mv05x_resume_validation_v1",
...
decision <- data.frame(
  contract_id = "mv05x_pc20_configuration_decision_v1",
  ...
  next_action = "prefreeze_pc20_label_closed_robustness_outcome_evaluation",
  labels_opened = FALSE, outcomes_computed = FALSE,
  stringsAsFactors = FALSE
)
resources$contract_id <- "mv05x_pc20_production_resource_metric_v1"
write_once(resources, file.path(
  output_dir, "mv05x-pc20-production-resources-2026-08-11.csv"
))
write_once(completion, file.path(
  output_dir, "mv05x-pc20-unit-completion-2026-08-11.csv"
))
write_once(validation, file.path(
  output_dir, "mv05x-independent-validation-2026-08-11.csv"
))
write_once(repeat_evidence, file.path(
  output_dir, "mv05x-deterministic-repeat-2026-08-11.csv"
))
write_once(resume_evidence, file.path(
  output_dir, "mv05x-resume-validation-2026-08-11.csv"
))
write_once(decision, file.path(
  output_dir, "mv05x-configuration-decision-2026-08-11.csv"
))
message("MV5-X independent validation passed: groups=150 pairs=70700 outcomes=0")
