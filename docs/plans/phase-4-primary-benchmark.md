...
| P4-02 | Specify matched baselines | Pseudobulk, centroid, composition, distributional/OT, and graph-summary contracts | Same samples, splits, clustering/task, and endpoint used across representations | `in_progress` — MV5-C baselines remain exact; MV5-D0 stage-1 inputs are complete, but folds and endpoint comparison remain closed |
...
| P4-04 | Define validation splits | Study-blocked/leave-one-study-out rules and leakage audit | No sample/study information crosses a prohibited boundary | `in_progress` — 90-sample/15-study/75-job plan remains label-closed; all 450 runtime-complete v2 caches are ready, but fold execution requires a separate MV5-D1 authorization |
