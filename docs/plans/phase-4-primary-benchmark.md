...
|---|---|---|---|---|
| P4-01 | Freeze estimands/endpoints | Statistical analysis plan defining primary/secondary estimands and units | Approved before confirmatory computation | `in_progress` — MV5-E/MV5-K retrieval, MV5-L paired representation DID, clustering, and the MV5-Y PC20 retrieval-sensitivity estimands are frozen; other robustness and external-validation estimands remain open |
...
| P4-08 | Build robustness controls | Outliers, shuffles, composition controls, subsampling, dimensions/features/scales | Each control targets a named alternative explanation | `in_progress` - heterogeneous PC20 sensitivity is complete; MV5-AB completes and independently validates the 150-group cosine-chord radial-scale calculation, while its outcome evaluation and both nested-cell settings remain separately gated |
| P4-09 | Execute benchmark | Immutable result bundle with manifests, logs, failures, and uncertainty | All prespecified methods/results are reported, including nulls | `in_progress` - retrieval/DID, prediction-locked clustering, PC20 outcomes and the label-closed cosine calculation are independently validated; cosine rankings/outcomes require a new prediction-locked prefreeze before evaluation |
