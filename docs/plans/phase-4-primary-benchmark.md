...
| P4-08 | Build robustness controls | Outliers, shuffles, composition controls, subsampling, dimensions/features/scales | Each control targets a named alternative explanation | `not_started` — MV5-M ranks robustness second after the clustering contract/resource gate; known outcomes require sensitivity-only interpretation |
| P4-09 | Execute benchmark | Immutable result bundle with manifests, logs, failures, and uncertainty | All prespecified methods/results are reported, including nulls | `in_progress` — retrieval/DID are complete; MV5-M selects a no-outcome clustering contract/resource gate next; technical mixing is identifiability-blocked and robustness remains second |
