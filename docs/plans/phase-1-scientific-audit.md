...
|---|---|---|---|---|
| P1-01 | Observation-unit trace | Table mapping every input row, cluster label, distance, and metric to cell/sample/study/group | No metric has an ambiguous unit or weighting rule | `in_progress` — PH input conflict traced and historical diagrams invalidated; MV-01 cell/gene point, coordinate, matching, metric, and sample-unit contracts frozen; broader metric/weighting trace remains |
...
| P1-04 | PCA/integration-space trace | Fit/transform provenance, feature sets, dimensions, centering/scaling, shared versus per-sample basis | Every comparison states whether coordinates are commensurate and why | `not_started` |
...
| P1-07 | Statistical reconstruction | Null-generation trace, seeds, blocks, cluster-size rules, p-value correction, multiplicity families, functional-test assumptions | Every p-value/interval has a documented estimand and valid procedure or is withdrawn | `not_started` |
