# MV5-AD prediction-locked cosine retrieval-robustness execution specification v1

Date frozen: 2026-08-11

Contract base: `260ac6d`

Outcome state at engine freeze: not computed

## Authorized analysis

MV5-AD implements exactly the accepted MV5-AC retrieval-only contract. It
first constructs canonical label-closed cosine rankings for all 150 groups,
...
The analysis retains the fixed 384 cells and 30 coordinates and compares the
accepted cosine-chord geometry with the accepted Euclidean geometry. SCT and
...
descriptive; energy is the matched same-geometry baseline.

## Prediction-lock phase

All 187 MV5-AC sources and all 150 MV5-AB group manifests must revalidate
...
and exact tie size from MV5-AB method rows without calling production helpers.
...
- 3,600 cosine query-method outcomes;
...
The two endpoints, 16 direct cosine-chord-minus-Euclidean changes, and eight
topology-increment DIDs are exactly those in the MV5-AC registries. Every row
...
`20260814`. The same block-count matrix applies to both geometries and every
method/estimand.
...
MV5-AD excludes cosine clustering and within-training pairs, both nested-cell
