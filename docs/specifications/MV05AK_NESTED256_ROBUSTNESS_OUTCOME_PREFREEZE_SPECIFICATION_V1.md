# MV5-AK nested-256 robustness-outcome prefreeze specification v1

Date frozen: 2026-08-11

Accepted nested-256 calculation base: `5e88fc149a2a9ef88de8af89372f7ca793b0cf41`

Outcome state: not computed

## Purpose

MV5-AK prospectively freezes a retrieval-only comparison between the accepted
384-cell, 30-coordinate Euclidean benchmark and the accepted nested-256-cell,
30-coordinate Euclidean calculation. The scientific question is whether
reducing deterministic cell representation depth from 384 to 256 changes
...
The 256 cells are the first 256 elements of the frozen sample/seed/cell-ID
SHA-256 order. Their first 192 elements are byte-identical to the accepted
nested-192 selection, and all 256 are a strict subset of the same accepted
384-cell realization. SCT and inductive-integrated views remain separate
strata.

The contract is frozen before any nested256 ranking, tissue-label join, endpoint,
...
The accepted Euclidean and nested256 artifacts share exactly:
...
- the same frozen 384-cell realization, with 384 baseline cells versus its
  deterministic nested 256-cell subset with its exact accepted 192-cell
  prefix, and 30 fold-compatible coordinates;
...
- exact all-active-consecutive-level persistence-landscape L2 distances for
  H0 and H1, retained separately;
...
| Nested 256 | first 256 cells in frozen SHA-256 order | Euclidean |
...
192-prefix, 256-within-384 inclusion, or source-identity verification causes
an abort before label access.

## Exact method pairing

Each representation has four paired families: H0, H1, raw H0/H1 composite,
and energy. The generic nested256 method IDs are mapped to the accepted integrated
...
SCT, the nested-256 source hash must equal the accepted MV5-D1 384-cell private
...
file hash. The private MV5-AJ source identity must independently repeat the
same hash. Thus the 256 cloud is a deterministic subset of the exact accepted
384-cell source, with the accepted 192 cloud as its strict prefix, rather than
merely a cloud with compatible dimensions.
...
outcome are prohibited. Accepted Euclidean endpoint rows and the completed
nested-192 panel are hash-bound but must not be opened until the nested256
prediction lock is durable.
...
`DIRECT(R,m,Y) = Y(R,m,nested256_cells) - Y(R,m,384_cells)`

for two representations, four families, and two endpoints: 16 estimands.

For topology dimension `d` in H0/H1, define:

`DID(R,d,Y) = [Y(R,d,nested256)-Y(R,energy,nested256)] - [Y(R,d,384cell)-Y(R,energy,384cell)]`
...
the nested-256 setting. Positive DID values mean topology's increment relative to its
...
MV5-AJ contains directed held-out-query/training-reference pairs only. It has
zero within-training nested256 distances, while compatible clustering requires
...
Therefore nested256 retrieval sensitivity is identifiable, but nested256 clustering
...
No new clustering calculation is authorized in MV5-AK.
...
MV5-AK may approve only a later, separately committed, prediction-locked
nested-256 retrieval evaluation. It does not execute or authorize rankings,
labels, endpoints, another configuration, clustering, gene topology,
