# Prediction-locked MV5-E SCT cell retrieval evaluation helpers.
...
.mv05e_method_roles <- c(
  cell_landscape_h0_v1 = "confirmatory",
  cell_landscape_h1_v1 = "confirmatory",
  cell_landscape_h0_h1_raw_euclidean_v1 = "descriptive_secondary",
  cell_distribution_energy_shared_pca_v1 = "matched_cell_baseline",
  pseudobulk_shared_panel_euclidean_v1 = "context_baseline"
)

.mv05e_eligible_tissues <- c(
  "bone marrow", "colon", "liver", "pbmc", "testis"
)

.mv05e_seeds <- 20260805:20260809

.mv05e_file_sha256 <- function(path) {
  digest::digest(file = path, algo = "sha256", serialize = FALSE)
}

.mv05e_restore_rng <- function(had_seed, old_seed) {
  if (had_seed) {
...
.mv05e_with_seed <- function(seed, expression) {
  had_seed <- exists(".Random.seed", envir = .GlobalEnv, inherits = FALSE)
  if (had_seed) old_seed <- get(".Random.seed", envir = .GlobalEnv)
  on.exit(.mv05e_restore_rng(had_seed, if (had_seed) old_seed else NULL),
          add = TRUE)
  set.seed(as.integer(seed))
  force(expression)
}

mv05e_verify_prediction_lock_v1 <- function(
    ranking_path, completion_path, group_index_path, assembly_summary_path,
  ...
  if (any(!file.exists(paths))) {
    stop("MV5-D5 prediction-lock input is missing.", call. = FALSE)
  }
  ranking_sha <- .mv05e_file_sha256(ranking_path)
  completion_sha <- .mv05e_file_sha256(completion_path)
  group_sha <- .mv05e_file_sha256(group_index_path)
  if (!identical(ranking_sha, expected_ranking_sha256)) {
    stop("MV5-D5 ranking SHA-256 does not match the frozen prediction lock.",
         call. = FALSE)
  }
  ...
  if (!all(required_completion %in% names(completion)) ||
    ...
    stop("MV5-D5 prediction-lock evidence has an unexpected schema.",
         call. = FALSE)
  }
  ...
  valid <- nrow(completion) == 375L && nrow(groups) == 75L &&
    ...
    identical(sort(unique(as.character(completion$method_id)), method = "radix"),
              sort(.mv05e_methods, method = "radix")) &&
    identical(sort(unique(as.integer(completion$seed))), .mv05e_seeds) &&
  ...
  if (!isTRUE(valid)) {
    stop("MV5-D5 prediction-lock evidence failed identity/completion checks.",
         call. = FALSE)
  }
  data.frame(
    contract_id = "mv05e_prediction_lock_v1",
    source_commit = "d2a15680a1f2f7d0113b34872f6649ec56902df2",
...
mv05e_open_frozen_labels_v1 <- function(metadata_path, prediction_lock,
  ...
  metadata_sha <- .mv05e_file_sha256(metadata_path)
  ...
  eligible <- sort(names(studies_by_tissue)[studies_by_tissue >= 2L],
                   method = "radix")
  if (!identical(eligible, sort(.mv05e_eligible_tissues, method = "radix"))) {
    stop("Frozen cross-study tissue eligibility has changed.", call. = FALSE)
  }
  candidates <- labels[labels$tissue %in% .mv05e_eligible_tissues, , drop = FALSE]
  ...
  list(
    ...
    provenance = data.frame(
      contract_id = "mv05e_frozen_label_source_v1",
      ...
      label_open_boundary = "after_mv05d5_prediction_lock_passed",
...
.mv05e_validate_rankings <- function(rankings, labels,
                                      expected_rows = 176750L,
                                      expected_methods = .mv05e_methods,
                                      expected_seeds = .mv05e_seeds) {
  required <- c(
  ...
  if (!is.data.frame(rankings) || !all(required %in% names(rankings)) ||
    ...
    stop("MV5-D5 rankings have an invalid schema or row count.", call. = FALSE)
  }
  if (anyNA(rankings[, required]) || anyDuplicated(rankings$pair_id) ||
    ...
    stop("MV5-D5 rankings failed immutable-content validation.", call. = FALSE)
  }
  ...
  if (!all(valid_group)) {
    stop("MV5-D5 rankings are not canonical immutable rank sequences.",
         call. = FALSE)
  }
  invisible(TRUE)
}

mv05e_evaluate_retrieval_v1 <- function(
    rankings, labels, expected_rows = 176750L,
    expected_methods = .mv05e_methods, expected_seeds = .mv05e_seeds,
    expected_observations = 2250L, require_all_estimable = TRUE) {
  .mv05e_validate_rankings(
  ...
  observations <- do.call(rbind, lapply(groups, function(index) {
    ...
    eligible <- q_tissue %in% .mv05e_eligible_tissues
    status <- if (!eligible) {
  ...
  observations <- observations[order(
    match(observations$method_id, .mv05e_methods), observations$seed,
  ...
  if (nrow(observations) != expected_observations ||
    ...
    stop("Unexpected MV5-E endpoint estimability/completion.", call. = FALSE)
  }
  observations
}

.mv05e_group_summary <- function(observations, group_fields, contract_id) {
  parts <- split(
    observations,
...
mv05e_summarize_retrieval_v1 <- function(observations) {
  ...
  seed_macro <- do.call(rbind, lapply(split(
    ...
    data.frame(
      contract_id = "mv05e_seed_macro_summary_v1",
  ...
  order_method <- function(x) {
    x[order(match(x$method_id, .mv05e_methods), method = "radix"), , drop = FALSE]
  }
  list(
    tissue_seed = tissue_seed[order(
      match(tissue_seed$method_id, .mv05e_methods), tissue_seed$seed,
      tissue_seed$query_tissue, method = "radix"), , drop = FALSE],
    seed_macro = seed_macro[order(
      match(seed_macro$method_id, .mv05e_methods), seed_macro$seed,
      method = "radix"), , drop = FALSE],
    sample = sample_summary[order(
      match(sample_summary$method_id, .mv05e_methods),
...
mv05e_block_inference_v1 <- function(
  ...
  if (!is.data.frame(sample_summary) ||
      ...
      any(sample_summary$seeds != length(.mv05e_seeds))) {
    stop("Sample summaries are incomplete for MV5-E inference.", call. = FALSE)
  }
  methods <- .mv05e_methods
  ...
  if (length(sample_ids) != 90L ||
    ...
    stop("MV5-E inference requires a complete 90-sample by five-method table.",
         call. = FALSE)
  }
  ...
  metric_arrays <- lapply(c(
    ...
    metric <- .mv05e_metric_column(endpoint)
  ...
  bootstrap <- .mv05e_with_seed(bootstrap_seed, {
  ...
  method_intervals <- do.call(rbind, lapply(names(metric_arrays), function(endpoint) {
    ...
    point <- vapply(seq_along(methods), function(index) {
      ...
      .mv05e_macro_from_sample_rows(rows, "value")
    }, numeric(1L))
    do.call(rbind, lapply(seq_along(methods), function(index) {
      ...
      data.frame(
        contract_id = "mv05e_method_interval_v1", endpoint_id = endpoint,
        method_id = methods[[index]], method_role = unname(.mv05e_method_roles[index]),
        ...
        completed_seeds = length(.mv05e_seeds),
        inference_status = "estimable", stringsAsFactors = FALSE
      )
    }))
  }))
  contrast_methods <- c("cell_landscape_h0_v1", "cell_landscape_h1_v1")
  energy <- "cell_distribution_energy_shared_pca_v1"
  contrasts <- do.call(rbind, lapply(names(metric_arrays), function(endpoint) {
    ...
    do.call(rbind, lapply(contrast_methods, function(method) {
      ...
      point <- .mv05e_macro_from_sample_rows(rows, "value")
      ...
      interval <- stats::quantile(boot_difference, c(0.025, 0.975),
                                  names = FALSE, type = 7)
      data.frame(
        contract_id = "mv05e_paired_contrast_v1",
  ...
  if (length(primary_rows)) {
    ...
    signs <- .mv05e_with_seed(randomization_seed, matrix(
    ...
    for (position in seq_along(primary_rows)) {
      ...
      exceedances <- sum(abs(null) >= abs(contrasts$estimate[[row_index]]))
    ...
    sign_audit <- data.frame(
      contract_id = "mv05e_randomization_audit_v1",
  ...
  bootstrap_audit <- data.frame(
    contract_id = "mv05e_bootstrap_audit_v1",
