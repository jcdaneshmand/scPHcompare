# MV5-Y prospective PC20 robustness-outcome contracts.

.mv05y_digest <- function(value) {
  digest::digest(value, algo = "sha256", serialize = TRUE)
}

.mv05y_is_true <- function(value) {
  tolower(as.character(value)) == "true"
}

.mv05y_hash_ok <- function(value, width = 64L) {
  length(value) > 0L && all(grepl(sprintf("^[0-9a-f]{%d}$", width), value))
}

.mv05y_assert_preoutcome <- function(value, label = "input") {
  ...
  for (column in intersect(c("outcomes_computed", "evaluation_executed",
                           ...
                           names(value))) {
    if (any(.mv05y_is_true(value[[column]]))) {
...
mv05y_method_map_v1 <- function() {
  ...
  data.frame(
    contract_id = "mv05y_method_map_v1",
...
#' Frozen PC20 robustness endpoint registry.
#'
#' @return A two-row endpoint registry.
mv05y_endpoint_registry_v1 <- function() {
  data.frame(
    contract_id = "mv05y_endpoint_registry_v1",
    ...
    direction_policy = c(
      "positive_signed_change_means_higher_pc20_mrr_zero_means_stability",
      "positive_signed_change_means_higher_pc20_balanced_accuracy_zero_means_stability"),
    outcomes_computed = FALSE,
    evaluation_executed = FALSE,
    stringsAsFactors = FALSE)
}

#' Frozen PC20 robustness estimands.
#'
#' @return The complete 24-row estimand registry.
mv05y_estimand_registry_v1 <- function(
  ...
  direct$estimand_type <- "direct_pc20_minus_pc30"
  direct$estimand_role <- ifelse(
    direct$endpoint_role == "primary", "secondary_sensitivity",
    "supportive_sensitivity")
  direct$formula <- "macro_Y_pc20_method_minus_macro_Y_pc30_method"
  ...
  did <- merge(did_map,
               endpoints[c("endpoint_id", "endpoint_role")], all = TRUE)
  did$estimand_type <- "topology_increment_pc20_minus_pc30_difference_in_differences"
  did$estimand_role <- ifelse(
    did$endpoint_role == "primary", "confirmatory_pc20_sensitivity",
    "supportive_pc20_sensitivity")
  did$formula <- paste0(
    "(macro_Y_pc20_topology_minus_macro_Y_pc20_energy)-",
    "(macro_Y_pc30_topology_minus_macro_Y_pc30_energy)")
  ...
  result$point_direction <-
    "positive_means_pc20_increased_the_named_quantity_negative_means_decreased_zero_means_unchanged"
  ...
  result$p_value_policy <- ifelse(
    result$estimand_role == "confirmatory_pc20_sensitivity",
  ...
  result$multiplicity_family <- ifelse(
    result$estimand_role == "confirmatory_pc20_sensitivity",
    "F1_pc20_topology_increment_mrr_four_tests", "none")
  ...
  mv05y_validate_estimand_registry_v1(result)
  result
}

mv05y_validate_estimand_registry_v1 <- function(value) {
  if (!is.data.frame(value) || nrow(value) != 24L ||
      anyDuplicated(value$estimand_id) ||
      sum(value$estimand_type == "direct_pc20_minus_pc30") != 16L ||
      sum(value$estimand_type ==
            "topology_increment_pc20_minus_pc30_difference_in_differences") != 8L ||
      sum(value$multiplicity_family ==
            "F1_pc20_topology_increment_mrr_four_tests") != 4L ||
      any(value$equivalence_or_noninferiority_claim_authorized) ||
      any(.mv05y_is_true(value$outcomes_computed)) ||
      any(.mv05y_is_true(value$evaluation_executed))) {
    stop("MV5-Y estimand registry violates its complete 24-estimand contract.",
         call. = FALSE)
  }
  .mv05y_assert_preoutcome(value, "MV5-Y estimand registry")
  invisible(value)
}

#' Build the frozen 150-group PC20 outcome-evaluation queue.
#'
...
mv05y_build_evaluation_queue_v1 <- function(group_scope,
  ...
  required <- c(
    ...
    "baseline_group_sha256", "pc20_group_manifest_sha256",
  ...
  queue$label_state <- "external_hash_bound_not_joined_to_pc20_predictions"
  ...
  queue <- queue[c(
    ...
    "pc20_group_manifest_sha256", "private_result_locator",
    "source_freeze_sha256", "label_state", "ranking_executed",
    "outcomes_computed", "evaluation_executed",
    "method_selection_executed", "execution_authorized")]
  mv05y_validate_evaluation_queue_v1(queue)
  queue
}

mv05y_validate_evaluation_queue_v1 <- function(queue) {
  if (!is.data.frame(queue) || nrow(queue) != 150L ||
      ...
      any(.mv05y_is_true(queue$ranking_executed)) ||
      any(.mv05y_is_true(queue$outcomes_computed)) ||
      any(.mv05y_is_true(queue$evaluation_executed)) ||
      any(.mv05y_is_true(queue$method_selection_executed)) ||
      any(.mv05y_is_true(queue$execution_authorized))) {
    stop("MV5-Y queue violates its prospective 150-group contract.",
         call. = FALSE)
  }
  .mv05y_assert_preoutcome(queue, "MV5-Y evaluation queue")
  invisible(queue)
}
