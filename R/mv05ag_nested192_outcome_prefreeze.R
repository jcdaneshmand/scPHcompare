# MV5-AG prospective nested-192 cell-depth robustness-outcome contracts.

.mv05ag_digest <- function(value) {
  digest::digest(value, algo = "sha256", serialize = TRUE)
}

.mv05ag_is_true <- function(value) {
  tolower(as.character(value)) == "true"
}

.mv05ag_hash_ok <- function(value, width = 64L) {
  length(value) > 0L && all(grepl(sprintf("^[0-9a-f]{%d}$", width), value))
}

.mv05ag_assert_preoutcome <- function(value, label = "input") {
  ...
  for (column in intersect(c("outcomes_computed", "evaluation_executed",
                           ...
                           names(value))) {
    if (any(.mv05ag_is_true(value[[column]]))) {
...
#' Frozen 384-cell to nested-192 Euclidean method map.
#'
#' @return A data frame defining the two representations and four paired
#'   distance families.
mv05ag_method_map_v1 <- function() {
  ...
  data.frame(
    contract_id = "mv05ag_method_map_v1",
    representation = representation,
    family_id = family,
    baseline_method_id = baseline,
    nested192_method_id = nested192,
    ...
    nested192_coordinate_count = 30L,
    baseline_cells = 384L,
    nested192_cells = 192L,
    baseline_point_metric = "euclidean",
    nested192_point_metric = "euclidean",
...
#' Frozen nested-192 cell-depth robustness endpoint registry.
#'
#' @return A two-row endpoint registry.
mv05ag_endpoint_registry_v1 <- function() {
  data.frame(
    contract_id = "mv05ag_endpoint_registry_v1",
    ...
    direction_policy = c(
      "positive_signed_change_means_higher_nested192_mrr_zero_means_stability",
      "positive_signed_change_means_higher_nested192_balanced_accuracy_zero_means_stability"),
    outcomes_computed = FALSE,
    evaluation_executed = FALSE,
    stringsAsFactors = FALSE)
}

#' Frozen nested-192 cell-depth robustness estimands.
#'
#' @return The complete 24-row estimand registry.
mv05ag_estimand_registry_v1 <- function(
  ...
  direct$estimand_type <- "direct_nested192_cells_minus_384_cells"
  direct$estimand_role <- ifelse(
    direct$endpoint_role == "primary", "secondary_sensitivity",
    "supportive_sensitivity")
  direct$formula <- "macro_Y_nested192_cells_method_minus_macro_Y_384_cells_method"
  ...
  did <- merge(did_map,
               endpoints[c("endpoint_id", "endpoint_role")], all = TRUE)
  did$estimand_type <- "topology_increment_nested192_minus_384_cells_difference_in_differences"
  did$estimand_role <- ifelse(
    did$endpoint_role == "primary", "confirmatory_nested192_sensitivity",
    "supportive_nested192_sensitivity")
  did$formula <- paste0(
    "(macro_Y_nested192_topology_minus_macro_Y_nested192_energy)-",
  ...
  result$point_direction <-
    "positive_means_nested192_cells_increased_the_named_quantity_relative_to_384_cells_negative_means_decreased_zero_means_unchanged"
  ...
  result$p_value_policy <- ifelse(
    result$estimand_role == "confirmatory_nested192_sensitivity",
  ...
  result$multiplicity_family <- ifelse(
    result$estimand_role == "confirmatory_nested192_sensitivity",
    "F1_nested192_topology_increment_mrr_four_tests", "none")
  ...
  mv05ag_validate_estimand_registry_v1(result)
  result
}

mv05ag_validate_estimand_registry_v1 <- function(value) {
  if (!is.data.frame(value) || nrow(value) != 24L ||
      anyDuplicated(value$estimand_id) ||
      sum(value$estimand_type == "direct_nested192_cells_minus_384_cells") != 16L ||
      sum(value$estimand_type ==
            "topology_increment_nested192_minus_384_cells_difference_in_differences") != 8L ||
      sum(value$multiplicity_family ==
            "F1_nested192_topology_increment_mrr_four_tests") != 4L ||
      any(value$equivalence_or_noninferiority_claim_authorized) ||
      any(.mv05ag_is_true(value$outcomes_computed)) ||
      any(.mv05ag_is_true(value$evaluation_executed))) {
    stop("MV5-AG estimand registry violates its complete 24-estimand contract.",
         call. = FALSE)
  }
  .mv05ag_assert_preoutcome(value, "MV5-AG estimand registry")
  invisible(value)
}

#' Build the frozen 150-group nested-192 outcome-evaluation queue.
#'
...
mv05ag_build_evaluation_queue_v1 <- function(group_scope,
  ...
  required <- c(
    ...
    "baseline_group_sha256", "nested192_group_manifest_sha256",
    "baseline_coordinate_source_sha256",
    "nested192_coordinate_source_sha256",
    "coordinate_source_identity_exact", "private_result_locator")
  ...
  queue$label_state <- "external_hash_bound_not_joined_to_nested192_predictions"
  ...
  queue <- queue[c(
    ...
    "nested192_group_manifest_sha256", "private_result_locator",
    "baseline_coordinate_source_sha256",
    "nested192_coordinate_source_sha256",
    "coordinate_source_identity_exact",
    "source_freeze_sha256", "label_state", "ranking_executed",
    "outcomes_computed", "evaluation_executed",
    "method_selection_executed", "execution_authorized")]
  mv05ag_validate_evaluation_queue_v1(queue)
  queue
}

mv05ag_validate_evaluation_queue_v1 <- function(queue) {
  if (!is.data.frame(queue) || nrow(queue) != 150L ||
      ...
      any(!.mv05ag_is_true(queue$coordinate_source_identity_exact)) ||
      any(.mv05ag_is_true(queue$ranking_executed)) ||
      any(.mv05ag_is_true(queue$outcomes_computed)) ||
      any(.mv05ag_is_true(queue$evaluation_executed)) ||
      any(.mv05ag_is_true(queue$method_selection_executed)) ||
      any(.mv05ag_is_true(queue$execution_authorized))) {
    stop("MV5-AG queue violates its prospective 150-group contract.",
         call. = FALSE)
  }
  .mv05ag_assert_preoutcome(queue, "MV5-AG evaluation queue")
  invisible(queue)
}
