# MV5-Z prediction-locked PC20 retrieval robustness execution helpers.

.mv05z_seeds <- 20260805:20260809
.mv05z_endpoints <- c(
  "cross_study_tissue_mrr_v1",
  "cross_study_tissue_1nn_balanced_accuracy_v1")

.mv05z_digest <- function(value) {
  digest::digest(value, algo = "sha256", serialize = TRUE)
}

.mv05z_with_rng <- function(seed, expression) {
...
.mv05z_boundary_exceedances <- function(null, observed) {
  tolerance <- 64 * .Machine$double.eps *
    pmax(1, abs(null), abs(observed))
  sum(abs(null) + tolerance >= abs(observed))
}

#' Construct canonical label-closed PC20 rankings for one frozen group.
#'
#' @param method_rows The accepted four-method MV5-X rows for one group.
#' @param group_row One MV5-Y queue row.
#' @return Canonically ordered prediction-side rankings.
mv05z_rank_group_v1 <- function(method_rows, group_row) {
  ...
  if (!is.data.frame(method_rows) || !all(required %in% names(method_rows)) ||
    ...
    stop("MV5-Z group method rows are malformed or not outcome-closed.",
         call. = FALSE)
  }
  expected_methods <- mv05y_method_map_v1()
  expected_methods <- expected_methods[
    expected_methods$representation == group_row$representation,
    "pc20_method_id"]
  if (!setequal(method_rows$method_id, expected_methods)) {
    stop("MV5-Z group method axis drifted.", call. = FALSE)
  }
  ...
  result <- do.call(rbind, lapply(split(method_rows, keys), function(part) {
    ...
    part$tie_break_policy <- "exact_distance_then_canonical_sample_id_v1"
    part
  }))
  result$contract_id <- "mv05z_pc20_ranking_row_v1"
  ...
  result$ranking_id <- vapply(seq_len(nrow(result)), function(index) {
    paste0("mv05z_ranking_v1:", .mv05z_digest(list(
  ...
  if (nrow(result) != group_row$method_rows ||
    ...
    stop("MV5-Z canonical group ranking completion failed.", call. = FALSE)
  }
  result[c(
...
#' Evaluate one immutable PC20 ranking group after its prediction lock.
mv05z_evaluate_group_v1 <- function(rankings, labels,
  ...
  if (!is.data.frame(rankings) || !all(required %in% names(rankings)) ||
    ...
    stop("MV5-Z ranking or label input is malformed.", call. = FALSE)
  }
  ...
  if (anyNA(label_index) || anyNA(query_index)) {
    stop("MV5-Z ranking-to-label key join failed.", call. = FALSE)
  }
  ...
  if (any(labels$study[query_index] != heldout) ||
    ...
    stop("MV5-Z held-out study isolation failed.", call. = FALSE)
  }
  ...
  observations <- do.call(rbind, lapply(groups, function(index) {
    ...
    if (!identical(as.integer(part$neighbor_rank), seq_len(nrow(part))) ||
        !identical(part$training_sample_id,
                   part$training_sample_id[order(
                     ...
                     method = "radix")])) {
      stop("MV5-Z immutable ranking sequence drifted.", call. = FALSE)
    }
    ...
    family <- map$family_id[match(method, map$pc20_method_id)]
    data.frame(
      contract_id = "mv05z_pc20_query_method_outcome_v1",
  ...
  if (nrow(observations) != expected ||
    ...
    stop("MV5-Z query-method outcome completion failed.", call. = FALSE)
  }
  observations
}

#' Pair accepted PC30 and new PC20 query-method endpoints.
mv05z_pair_baseline_v1 <- function(pc20, baseline_sct, baseline_integrated,
                                    method_map = mv05y_method_map_v1()) {
  ...
  if (!all(required %in% names(pc20)) ||
      !all(required %in% names(baseline_sct)) ||
      !all(required %in% names(baseline_integrated))) {
    stop("MV5-Z paired endpoint schema is incomplete.", call. = FALSE)
  }
  baseline <- rbind(
  ...
  pc20_key <- key(pc20); baseline_key <- key(baseline)
  if (nrow(pc20) != 3600L || nrow(baseline) != 3600L ||
      anyDuplicated(pc20_key) || anyDuplicated(baseline_key) ||
      !setequal(pc20_key, baseline_key)) {
    stop("MV5-Z baseline/PC20 query endpoint pairing failed.", call. = FALSE)
  }
  baseline <- baseline[match(pc20_key, baseline_key), , drop = FALSE]
  common <- c("held_out_study", "query_tissue", "training_samples",
              "training_studies", "endpoint_status")
  if (!all(vapply(common, function(name) identical(pc20[[name]], baseline[[name]]),
                  logical(1L))) ||
      any(pc20$upstream_refit) || any(pc20$reranked_after_label_open) ||
      any(baseline$upstream_refit) || any(baseline$reranked_after_label_open)) {
    stop("MV5-Z paired identity, denominator, or no-refit contract failed.",
         call. = FALSE)
  }
  data.frame(
    contract_id = "mv05z_paired_query_method_endpoint_v1",
    pc20[keys],
    baseline_method_id = baseline$method_id,
    pc20_method_id = pc20$method_id,
    baseline_reciprocal_rank = as.numeric(baseline$reciprocal_rank),
    pc20_reciprocal_rank = as.numeric(pc20$reciprocal_rank),
    direct_reciprocal_rank_change =
      as.numeric(pc20$reciprocal_rank - baseline$reciprocal_rank),
    baseline_one_nn_correct = as.logical(baseline$one_nn_correct),
    pc20_one_nn_correct = as.logical(pc20$one_nn_correct),
    direct_one_nn_change = as.numeric(pc20$one_nn_correct) -
      as.numeric(baseline$one_nn_correct),
    endpoint_status = pc20$endpoint_status,
    outcomes_computed = TRUE, evaluation_executed = TRUE,
    method_selection_executed = FALSE,
    stringsAsFactors = FALSE)
}

#' Construct every frozen query-level PC20 robustness estimand.
mv05z_query_estimands_v1 <- function(
    paired, registry = mv05y_estimand_registry_v1()) {
  keys <- c("fold_id", "held_out_study", "seed", "representation",
  ...
  direct <- do.call(rbind, lapply(seq_len(nrow(paired)), function(index) {
    ...
    rbind(
      data.frame(part[keys], family_id = part$family_id,
        endpoint_id = .mv05z_endpoints[[1L]],
        estimand_type = "direct_pc20_minus_pc30",
        value = part$direct_reciprocal_rank_change, stringsAsFactors = FALSE),
      data.frame(part[keys], family_id = part$family_id,
        endpoint_id = .mv05z_endpoints[[2L]],
        estimand_type = "direct_pc20_minus_pc30",
  ...
  did <- do.call(rbind, lapply(c("h0", "h1"), function(family) {
    ...
    rbind(
      data.frame(topology[match_keys], family_id = family,
        endpoint_id = .mv05z_endpoints[[1L]],
        estimand_type =
          "topology_increment_pc20_minus_pc30_difference_in_differences",
  ...
  if (nrow(result) != 10800L || anyNA(result$estimand_id) ||
    ...
    stop("MV5-Z query estimand construction failed.", call. = FALSE)
  }
  result$contract_id <- "mv05z_query_estimand_v1"
...
mv05z_summarize_estimands_v1 <- function(query) {
  ...
  sample$contract_id <- "mv05z_sample_estimand_v1"
  ...
  tissue$contract_id <- "mv05z_tissue_estimand_v1"
  macro <- stats::aggregate(
    estimate ~ estimand_id + estimand_order + estimand_type + estimand_role +
      endpoint_id + representation + family_id,
    data = tissue, FUN = mean)
  macro$contract_id <- "mv05z_macro_estimand_v1"
  ...
  if (nrow(sample) != 2160L || nrow(tissue) != 120L || nrow(macro) != 24L ||
    ...
    stop("MV5-Z frozen aggregation completion failed.", call. = FALSE)
  }
  list(query = query, sample = sample, tissue = tissue, macro = macro)
}

.mv05z_macro <- function(base, values, weights = NULL) {
...
mv05z_block_inference_v1 <- function(
    sample_summary, registry = mv05y_estimand_registry_v1(),
    bootstrap_replicates = 2000L, bootstrap_seed = 20260812L,
    randomization_replicates = 9999L, randomization_seed = 20260813L) {
  ...
  if (nrow(base) != 90L || anyNA(base) || any(!is.finite(values))) {
    stop("MV5-Z inference sample matrix is incomplete.", call. = FALSE)
  }
  ...
  if (anyDuplicated(study_tissue$held_out_study)) {
    stop("MV5-Z bootstrap requires tissue-homogeneous study blocks.",
         call. = FALSE)
  }
  count_matrix <- .mv05z_with_rng(bootstrap_seed, {
  ...
  for (replicate_id in seq_len(bootstrap_replicates)) {
    ...
    for (column in seq_len(ncol(values))) {
      bootstrap[replicate_id, column] <-
        .mv05z_macro(base, values[, column], weights)
    }
  }
  intervals <- do.call(rbind, lapply(seq_along(estimand_ids), function(index) {
    ...
    ci <- stats::quantile(bootstrap[, index], c(0.025, 0.975),
                          names = FALSE, type = 7)
    data.frame(
      contract_id = "mv05z_estimand_interval_v1",
      ...
      estimate = .mv05z_macro(base, values[, index]),
  ...
  primary <- intervals[intervals$estimand_role ==
                         "confirmatory_pc20_sensitivity", , drop = FALSE]
  primary$contract_id <- "mv05z_primary_contrast_v1"
  primary$multiplicity_family <- "F1_pc20_topology_increment_mrr_four_tests"
  primary$randomization_replicates <- randomization_replicates
  primary$randomization_seed <- randomization_seed
  signs <- .mv05z_with_rng(randomization_seed, matrix(
  ...
  for (index in seq_len(nrow(primary))) {
    ...
    null <- vapply(seq_len(randomization_replicates), function(replicate_id) {
      ...
      .mv05z_macro(base, signed)
    }, numeric(1L))
    exceedances[[index]] <- .mv05z_boundary_exceedances(
      null, primary$estimate[[index]])
    primary$raw_p_value[[index]] <-
      (exceedances[[index]] + 1) / (randomization_replicates + 1)
    null_hash[[index]] <- .mv05z_digest(null)
  }
  ...
  bootstrap_audit <- data.frame(
    contract_id = "mv05z_bootstrap_audit_v1",
    bootstrap_replicates = bootstrap_replicates,
    bootstrap_seed = bootstrap_seed, tissues = 5L, study_blocks = 15L,
    estimands = 24L, block_count_matrix_sha256 = .mv05z_digest(count_matrix),
    replicate_matrix_sha256 = .mv05z_digest(bootstrap),
  ...
  randomization_audit <- data.frame(
    contract_id = "mv05z_randomization_audit_v1",
    multiplicity_family = "F1_pc20_topology_increment_mrr_four_tests",
    ...
    sign_matrix_sha256 = .mv05z_digest(signs),
