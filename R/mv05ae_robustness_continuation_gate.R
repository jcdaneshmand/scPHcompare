# MV5-AE selection-resistant nested-cell continuation helpers.

.mv05ae_true <- function(x) {
  if (is.logical(x)) return(!is.na(x) & x)
  tolower(trimws(as.character(x))) == "true"
}

#' Recover the immutable MV5-V configuration order after two completions.
mv05ae_configuration_order_v1 <- function(queue) {
  required <- c("configuration_id", "configuration_order", "execution_order")
  if (!is.data.frame(queue) || !all(required %in% names(queue)) ||
      nrow(queue) != 600L) {
    stop("MV5-AE requires the complete 600-row MV5-V queue.", call. = FALSE)
  }
  expected <- c(
  ...
  mapping$contract_id <- "mv05ae_configuration_order_v1"
  mapping$position_state <- c("complete", "complete", "next_eligible",
                              "later_closed")
...
mv05ae_bind_complete_evidence_v1 <- function(
  ...
  if (length(prohibited) && any(.mv05ae_true(unlist(production[1L, prohibited])))) {
    stop("A robustness evidence bundle reports a prohibited operation.",
         call. = FALSE)
  }
  data.frame(
    contract_id = "mv05ae_complete_evidence_binding_v1",
    ...
    cell_count_sensitivity_answered = FALSE,
    subgroup_or_method_selection_used = FALSE,
    complete_evidence_bound = TRUE,
    stringsAsFactors = FALSE)
}

#' Frozen whole-analysis post-cosine continuation criteria.
mv05ae_continuation_criteria_v1 <- function() {
  data.frame(
    contract_id = "mv05ae_continuation_criteria_v1",
    ...
    required_state = c(
      "nested_192_is_position_3_after_pc20_and_cosine_complete",
      "both_complete_24_estimand_4_test_panels_bound_unsliced",
      "cell_subsampling_stability_not_answered_by_pc_count_or_geometry",
      "30pc_euclidean_fixed_only_nested_cell_count_changes_384_to_192",
      "all_150_sources_have_deterministic_first_192_of_frozen_384_views",
      "admission_and_two_full_config_precedents_fit_one_worker_caps",
      "no_representation_dimension_tissue_endpoint_seed_estimate_interval_or_pvalue_selection",
      "later_nested_192_outcome_requires_same_24_estimand_complete_panel",
      "stop_before_ranking_labels_outcomes_clustering_or_nested_256"),
    mandatory = TRUE, frozen_before_new_calculation = TRUE,
    stringsAsFactors = FALSE)
}

#' Make the single allowed MV5-AE continuation decision.
mv05ae_decide_v1 <- function(order, evidence, criterion_pass) {
  criteria <- mv05ae_continuation_criteria_v1()
  if (!is.data.frame(order) || nrow(order) != 4L ||
      order$configuration_id[[3L]] !=
        "nested_cells_192_pc30_euclidean_v1" ||
      !identical(order$position_state,
                 c("complete", "complete", "next_eligible", "later_closed")) ||
      !is.data.frame(evidence) || nrow(evidence) != 2L ||
      !setequal(evidence$analysis_id,
                c("pc20_vs_pc30", "cosine_chord_vs_euclidean")) ||
      !all(.mv05ae_true(evidence$complete_evidence_bound)) ||
      any(.mv05ae_true(evidence$cell_count_sensitivity_answered)) ||
      !is.data.frame(criterion_pass) ||
      !identical(criterion_pass$criterion_id, criteria$criterion_id) ||
      !all(.mv05ae_true(criterion_pass$passed))) {
    stop("MV5-AE continuation cannot be authorized.", call. = FALSE)
  }
  data.frame(
    contract_id = "mv05ae_continuation_decision_v1",
    decision = "authorize_later_label_closed_nested_192_calculation_only",
    authorized_configuration_id = order$configuration_id[[3L]],
    ...
    nested_192_calculation_executed = FALSE,
    labels_opened = FALSE, rankings_computed = FALSE,
    outcomes_computed = FALSE, clustering_authorized = FALSE,
    nested_256_authorized = FALSE, stringsAsFactors = FALSE)
}

#' Construct the exact later nested-192 calculation queue.
mv05ae_nested_192_queue_v1 <- function(queue, decision) {
  id <- "nested_cells_192_pc30_euclidean_v1"
  if (!is.data.frame(decision) || nrow(decision) != 1L ||
      decision$authorized_configuration_id[[1L]] != id ||
      .mv05ae_true(decision$nested_192_calculation_executed[[1L]])) {
    stop("MV5-AE decision does not authorize queue construction.", call. = FALSE)
  }
  ...
  if (nrow(result) != 150L || anyDuplicated(result$robustness_group_id) ||
      ...
      any(as.integer(result$cells) != 192L) ||
      any(as.integer(result$coordinates) != 30L) ||
      any(result$point_metric != "euclidean") ||
      any(.mv05ae_true(result$outcomes_computed))) {
    stop("The frozen nested-192 queue is incomplete or contaminated.",
         call. = FALSE)
  }
  result$mv05ae_execution_order <- seq_len(nrow(result))
