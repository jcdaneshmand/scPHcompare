# MV5-AK prospective nested-256 cell-depth robustness-outcome contracts.

.mv05ak_digest <- function(value) {
  digest::digest(value, algo = "sha256", serialize = TRUE)
}

.mv05ak_is_true <- function(value) {
  tolower(as.character(value)) == "true"
}

.mv05ak_hash_ok <- function(value, width = 64L) {
  length(value) > 0L && all(grepl(sprintf("^[0-9a-f]{%d}$", width), value))
}

.mv05ak_assert_preoutcome <- function(value, label = "input") {
  if (!is.data.frame(value)) stop(label, " must be a data frame.", call. = FALSE)
  prohibited <- c(
    "tissue", "tissue.x", "approach", "approach.x",
  ...
  for (column in intersect(c("outcomes_computed", "evaluation_executed",
                              ...
                              "labels_opened", "label_values_read",
                              "label_join_executed"),
                           names(value))) {
    if (any(.mv05ak_is_true(value[[column]]))) {
...
#' Frozen 384-cell to nested-256 Euclidean method map.
#'
#' @return A data frame defining the two representations and four paired
#'   distance families.
mv05ak_method_map_v1 <- function() {
  ...
  data.frame(
    contract_id = "mv05ak_method_map_v1",
    representation = representation,
    family_id = family,
    baseline_method_id = baseline,
    nested256_method_id = nested256,
    ...
    nested256_coordinate_count = 30L,
    baseline_cells = 384L,
    nested256_cells = 256L,
    baseline_point_metric = "euclidean",
    nested256_point_metric = "euclidean",
...
#' Frozen nested-256 cell-depth robustness endpoint registry.
#'
#' @return A two-row endpoint registry.
mv05ak_endpoint_registry_v1 <- function() {
  data.frame(
    contract_id = "mv05ak_endpoint_registry_v1",
    ...
    direction_policy = c(
      "positive_signed_change_means_higher_nested256_mrr_zero_means_stability",
      "positive_signed_change_means_higher_nested256_balanced_accuracy_zero_means_stability"),
    outcomes_computed = FALSE,
    evaluation_executed = FALSE,
    stringsAsFactors = FALSE)
}

#' Frozen nested-256 cell-depth robustness estimands.
#'
#' @return The complete 24-row estimand registry.
mv05ak_estimand_registry_v1 <- function(
  ...
  direct$estimand_type <- "direct_nested256_cells_minus_384_cells"
  direct$estimand_role <- ifelse(
    direct$endpoint_role == "primary", "secondary_sensitivity",
    "supportive_sensitivity")
  direct$formula <- "macro_Y_nested256_cells_method_minus_macro_Y_384_cells_method"
  ...
  did <- merge(did_map,
               endpoints[c("endpoint_id", "endpoint_role")], all = TRUE)
  did$estimand_type <- "topology_increment_nested256_minus_384_cells_difference_in_differences"
  did$estimand_role <- ifelse(
    did$endpoint_role == "primary", "confirmatory_nested256_sensitivity",
    "supportive_nested256_sensitivity")
  did$formula <- paste0(
    "(macro_Y_nested256_topology_minus_macro_Y_nested256_energy)-",
    "(macro_Y_384cell_topology_minus_macro_Y_384cell_energy)")
  ...
  result$point_direction <-
    "positive_means_nested256_cells_increased_the_named_quantity_relative_to_384_cells_negative_means_decreased_zero_means_unchanged"
  ...
  result$p_value_policy <- ifelse(
    result$estimand_role == "confirmatory_nested256_sensitivity",
  ...
  result$multiplicity_family <- ifelse(
    result$estimand_role == "confirmatory_nested256_sensitivity",
    "F1_nested256_topology_increment_mrr_four_tests", "none")
  ...
  mv05ak_validate_estimand_registry_v1(result)
  result
}

mv05ak_validate_estimand_registry_v1 <- function(value) {
  if (!is.data.frame(value) || nrow(value) != 24L ||
      anyDuplicated(value$estimand_id) ||
      sum(value$estimand_type == "direct_nested256_cells_minus_384_cells") != 16L ||
      sum(value$estimand_type ==
            "topology_increment_nested256_minus_384_cells_difference_in_differences") != 8L ||
      sum(value$multiplicity_family ==
            "F1_nested256_topology_increment_mrr_four_tests") != 4L ||
      any(value$equivalence_or_noninferiority_claim_authorized) ||
      any(.mv05ak_is_true(value$outcomes_computed)) ||
      any(.mv05ak_is_true(value$evaluation_executed))) {
    stop("MV5-AK estimand registry violates its complete 24-estimand contract.",
         call. = FALSE)
  }
  .mv05ak_assert_preoutcome(value, "MV5-AK estimand registry")
  invisible(value)
}

#' Build the frozen 150-group nested-256 outcome-evaluation queue.
#'
...
mv05ak_build_evaluation_queue_v1 <- function(group_scope,
  ...
  required <- c(
    ...
    "baseline_group_sha256", "nested256_group_manifest_sha256",
    "baseline_coordinate_source_sha256",
    "nested256_coordinate_source_sha256",
    "coordinate_source_identity_exact", "nested192_prefix_identity_exact",
    "nested256_subset_384_identity_exact", "private_result_locator")
  ...
  queue$label_state <- "external_hash_bound_not_joined_to_nested256_predictions"
  ...
  queue <- queue[c(
    ...
    "nested256_group_manifest_sha256", "private_result_locator",
    "baseline_coordinate_source_sha256",
    "nested256_coordinate_source_sha256",
    "coordinate_source_identity_exact", "nested192_prefix_identity_exact",
    "nested256_subset_384_identity_exact",
    "source_freeze_sha256", "label_state", "ranking_executed",
    "outcomes_computed", "evaluation_executed",
    "method_selection_executed", "execution_authorized")]
  mv05ak_validate_evaluation_queue_v1(queue)
  queue
}

mv05ak_validate_evaluation_queue_v1 <- function(queue) {
  if (!is.data.frame(queue) || nrow(queue) != 150L ||
      ...
      any(!.mv05ak_is_true(queue$coordinate_source_identity_exact)) ||
      any(!.mv05ak_is_true(queue$nested192_prefix_identity_exact)) ||
      any(!.mv05ak_is_true(queue$nested256_subset_384_identity_exact)) ||
      any(.mv05ak_is_true(queue$ranking_executed)) ||
      any(.mv05ak_is_true(queue$outcomes_computed)) ||
      any(.mv05ak_is_true(queue$evaluation_executed)) ||
      any(.mv05ak_is_true(queue$method_selection_executed)) ||
      any(.mv05ak_is_true(queue$execution_authorized))) {
    stop("MV5-AK queue violates its prospective 150-group contract.",
         call. = FALSE)
  }
  .mv05ak_assert_preoutcome(queue, "MV5-AK evaluation queue")
  invisible(queue)
}
